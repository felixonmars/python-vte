...
/**
 * SECTION: deepinvte-terminal
 * @short_description: A terminal widget implementation
 *
 * A DeepinvteTerminal is a terminal emulator implemented as a GTK2 widget.
 */
...
#include "matcher.h"
#include "pty.h"
#include "deepinvteaccess.h"
#include "deepinvteint.h"
...
static void deepinvte_terminal_set_visibility (DeepinvteTerminal *terminal, GdkVisibilityState state);
static void deepinvte_terminal_set_termcap(DeepinvteTerminal *terminal, const char *path,
				     gboolean reset);
static void deepinvte_terminal_paste(DeepinvteTerminal *terminal, GdkAtom board);
static void deepinvte_terminal_real_copy_clipboard(DeepinvteTerminal *terminal);
static void deepinvte_terminal_real_paste_clipboard(DeepinvteTerminal *terminal);
static gboolean deepinvte_terminal_io_read(GIOChannel *channel,
				     GIOCondition condition,
				     DeepinvteTerminal *terminal);
static gboolean deepinvte_terminal_io_write(GIOChannel *channel,
				      GIOCondition condition,
				      DeepinvteTerminal *terminal);
static void deepinvte_terminal_match_hilite_clear(DeepinvteTerminal *terminal);
static void deepinvte_terminal_match_hilite_hide(DeepinvteTerminal *terminal);
static void deepinvte_terminal_match_hilite_show(DeepinvteTerminal *terminal, long x, long y);
static void deepinvte_terminal_match_hilite_update(DeepinvteTerminal *terminal, long x, long y);
static void deepinvte_terminal_match_contents_clear(DeepinvteTerminal *terminal);
static gboolean deepinvte_terminal_background_update(DeepinvteTerminal *data);
static void deepinvte_terminal_queue_background_update(DeepinvteTerminal *terminal);
static void deepinvte_terminal_process_incoming(DeepinvteTerminal *terminal);
static void deepinvte_terminal_emit_pending_signals(DeepinvteTerminal *terminal);
static gboolean deepinvte_cell_is_selected(DeepinvteTerminal *terminal,
				     glong col, glong row, gpointer data);
static char *deepinvte_terminal_get_text_range_maybe_wrapped(DeepinvteTerminal *terminal,
						       glong start_row,
						       glong start_col,
						       glong end_row,
						       glong end_col,
...
static char *deepinvte_terminal_get_text_maybe_wrapped(DeepinvteTerminal *terminal,
						 gboolean wrap,
						 DeepinvteSelectionFunc is_selected,
						 gpointer data,
						 GArray *attributes,
						 gboolean include_trailing_spaces);
static void _deepinvte_terminal_disconnect_pty_read(DeepinvteTerminal *terminal);
static void _deepinvte_terminal_disconnect_pty_write(DeepinvteTerminal *terminal);
static void deepinvte_terminal_stop_processing (DeepinvteTerminal *terminal);

static inline gboolean deepinvte_terminal_is_processing (DeepinvteTerminal *terminal);
static inline void deepinvte_terminal_start_processing (DeepinvteTerminal *terminal);
static void deepinvte_terminal_add_process_timeout (DeepinvteTerminal *terminal);
static void add_update_timeout (DeepinvteTerminal *terminal);
static void remove_update_timeout (DeepinvteTerminal *terminal);
static void reset_update_regions (DeepinvteTerminal *terminal);
static void deepinvte_terminal_set_cursor_blinks_internal(DeepinvteTerminal *terminal, gboolean blink);
static void deepinvte_terminal_set_font_full_internal(DeepinvteTerminal *terminal,
                                                const PangoFontDescription *font_desc,
                                                DeepinvteTerminalAntiAlias antialias);
static void _deepinvte_check_cursor_blink(DeepinvteTerminal *terminal);
...
static struct _deepinvte_incoming_chunk *free_chunks;
static struct _deepinvte_incoming_chunk *
get_chunk (void)
{
	struct _deepinvte_incoming_chunk *chunk = NULL;
	if (free_chunks) {
		chunk = free_chunks;
		free_chunks = free_chunks->next;
	}
	if (chunk == NULL) {
		chunk = g_new (struct _deepinvte_incoming_chunk, 1);
	}
	chunk->next = NULL;
	chunk->len = 0;
	return chunk;
...
static void
release_chunk (struct _deepinvte_incoming_chunk *chunk)
{
...
prune_chunks (guint len)
{
	...
	if (len && free_chunks != NULL) {
		...
		struct _deepinvte_incoming_chunk *last;
		chunk = free_chunks;
	...
	while (chunk != NULL) {
		struct _deepinvte_incoming_chunk *next = chunk->next;
		g_free (chunk);
		chunk = next;
	}
}
static void
_deepinvte_incoming_chunks_release (struct _deepinvte_incoming_chunk *chunk)
{
	while (chunk) {
		struct _deepinvte_incoming_chunk *next = chunk->next;
		release_chunk (chunk);
...
static gsize
_deepinvte_incoming_chunks_length (struct _deepinvte_incoming_chunk *chunk)
{
	gsize len = 0;
	while (chunk) {
		len += chunk->len;
...
static gsize
_deepinvte_incoming_chunks_count (struct _deepinvte_incoming_chunk *chunk)
{
	gsize cnt = 0;
	while (chunk) {
		cnt ++;
...
static struct _deepinvte_incoming_chunk *
_deepinvte_incoming_chunks_reverse(struct _deepinvte_incoming_chunk *chunk)
{
	struct _deepinvte_incoming_chunk *prev = NULL;
	while (chunk) {
		struct _deepinvte_incoming_chunk *next = chunk->next;
		chunk->next = prev;
		prev = chunk;
...
G_DEFINE_TYPE_WITH_CODE(DeepinvteTerminal, deepinvte_terminal, GTK_TYPE_WIDGET,
                        g_type_add_class_private (g_define_type_id, sizeof (DeepinvteTerminalClassPrivate));
                        G_IMPLEMENT_INTERFACE(GTK_TYPE_SCROLLABLE, NULL)
                        if (_deepinvte_debug_on(DEEPINVTE_DEBUG_LIFECYCLE)) {
                                g_printerr("deepinvte_terminal_get_type()\n");
                        })
#else
G_DEFINE_TYPE_WITH_CODE(DeepinvteTerminal, deepinvte_terminal, GTK_TYPE_WIDGET,
                        g_type_add_class_private (g_define_type_id, sizeof (DeepinvteTerminalClassPrivate));
...
#else
G_DEFINE_TYPE(DeepinvteTerminal, deepinvte_terminal, GTK_TYPE_WIDGET)
#endif
#endif /* GTK 3.0 */

/* Indexes in the "palette" color array for the dim colors.
 * Only the first %DEEPINVTE_LEGACY_COLOR_SET_SIZE colors have dim versions.  */
static const guchar corresponding_dim_index[] = {16,88,28,100,18,90,30,102};

static void
deepinvte_g_array_fill(GArray *array, gconstpointer item, guint final_size)
{
...
DeepinvteRowData *
_deepinvte_terminal_ring_insert (DeepinvteTerminal *terminal, glong position, gboolean fill)
{
	DeepinvteRowData *row;
	DeepinvteRing *ring = terminal->pvt->screen->row_data;
	while (G_UNLIKELY (_deepinvte_ring_next (ring) < position)) {
		row = _deepinvte_ring_append (ring);
		_deepinvte_row_data_fill (row, &terminal->pvt->screen->fill_defaults, terminal->column_count);
	}
	row = _deepinvte_ring_insert (ring, position);
	if (fill)
		_deepinvte_row_data_fill (row, &terminal->pvt->screen->fill_defaults, terminal->column_count);
	return row;
}
...
DeepinvteRowData *
_deepinvte_terminal_ring_append (DeepinvteTerminal *terminal, gboolean fill)
{
	return _deepinvte_terminal_ring_insert (terminal, _deepinvte_ring_next (terminal->pvt->screen->row_data), fill);
}

void
_deepinvte_terminal_ring_remove (DeepinvteTerminal *terminal, glong position)
{
	_deepinvte_ring_remove (terminal->pvt->screen->row_data, position);
}

/* Reset defaults for character insertion. */
void
_deepinvte_terminal_set_default_attributes(DeepinvteTerminal *terminal)
{
...
void
_deepinvte_invalidate_cells(DeepinvteTerminal *terminal,
		      glong column_start, gint column_count,
		      glong row_start, gint row_count)
{
	DeepinvteRegionRectangle rect;
	glong i;
	...
	_deepinvte_debug_print (DEEPINVTE_DEBUG_UPDATES,
			"Invalidating cells at (%ld,%ld+%ld)x(%d,%d).\n",
			column_start, row_start,
			(long)terminal->pvt->screen->scroll_delta,
			column_count, row_count);
	_deepinvte_debug_print (DEEPINVTE_DEBUG_WORK, "?");
...
static void
_deepinvte_invalidate_region (DeepinvteTerminal *terminal,
...
void
_deepinvte_invalidate_all(DeepinvteTerminal *terminal)
{
	DeepinvteRegionRectangle rect;
	...
	_deepinvte_debug_print (DEEPINVTE_DEBUG_WORK, "*");
	_deepinvte_debug_print (DEEPINVTE_DEBUG_UPDATES, "Invalidating all.\n");
...
void
_deepinvte_terminal_scroll_region (DeepinvteTerminal *terminal,
...
static inline const DeepinvteRowData *
_deepinvte_terminal_find_row_data (DeepinvteTerminal *terminal, glong row)
{
	const DeepinvteRowData *rowdata = NULL;
	DeepinvteScreen *screen = terminal->pvt->screen;
	if (G_LIKELY (_deepinvte_ring_contains (screen->row_data, row))) {
		rowdata = _deepinvte_ring_index (screen->row_data, row);
	}
	return rowdata;
}
...
_deepinvte_terminal_find_row_data_writable (DeepinvteTerminal *terminal, glong row)
{
	DeepinvteRowData *rowdata = NULL;
	DeepinvteScreen *screen = terminal->pvt->screen;
	if (G_LIKELY (_deepinvte_ring_contains (screen->row_data, row))) {
...
static const DeepinvteCell *
deepinvte_terminal_find_charcell(DeepinvteTerminal *terminal, gulong col, glong row)
{
	const DeepinvteRowData *rowdata;
	const DeepinvteCell *ret = NULL;
	DeepinvteScreen *screen;
	screen = terminal->pvt->screen;
	if (_deepinvte_ring_contains (screen->row_data, row)) {
		rowdata = _deepinvte_ring_index (screen->row_data, row);
		ret = _deepinvte_row_data_get (rowdata, col);
	}
	return ret;
}

static glong
find_start_column (DeepinvteTerminal *terminal, glong col, glong row)
{
	const DeepinvteRowData *row_data = _deepinvte_terminal_find_row_data (terminal, row);
	if (G_UNLIKELY (col < 0))
		return col;
	if (row_data != NULL) {
		const DeepinvteCell *cell = _deepinvte_row_data_get (row_data, col);
		while (col > 0 && cell != NULL && cell->attr.fragment) {
			cell = _deepinvte_row_data_get (row_data, --col);
		}
	}
	return MAX(col, 0);
}
static glong
find_end_column (DeepinvteTerminal *terminal, glong col, glong row)
{
	const DeepinvteRowData *row_data = _deepinvte_terminal_find_row_data (terminal, row);
	gint columns = 0;
	if (G_UNLIKELY (col < 0))
...
static gssize
deepinvte_terminal_preedit_width(DeepinvteTerminal *terminal, gboolean left_only)
{
	gunichar c;
	int i;
	gssize ret = 0;
	...
	if (terminal->pvt->im_preedit != NULL) {
		...
		for (i = 0;
			...
			ret += _deepinvte_iso2022_unichar_width(terminal->pvt->iso2022, c);
...
static gssize
deepinvte_terminal_preedit_length(DeepinvteTerminal *terminal, gboolean left_only)
{
	int i = 0;
	const char *preedit = NULL;
...
void
_deepinvte_invalidate_cell(DeepinvteTerminal *terminal, glong col, glong row)
{
	const DeepinvteRowData *row_data;
	int columns;
	...
	columns = 1;
	row_data = _deepinvte_terminal_find_row_data(terminal, row);
	if (row_data != NULL) {
		const DeepinvteCell *cell;
		cell = _deepinvte_row_data_get (row_data, col);
		if (cell != NULL) {
...
void
_deepinvte_invalidate_cursor_once(DeepinvteTerminal *terminal, gboolean periodic)
{
	DeepinvteScreen *screen;
	const DeepinvteCell *cell;
	...
	if (terminal->pvt->cursor_visible && gtk_widget_is_drawable (&terminal->widget)) {
		preedit_width = deepinvte_terminal_preedit_width(terminal, FALSE);
		...
		cell = deepinvte_terminal_find_charcell(terminal, column, row);
		if (cell != NULL) {
			columns = cell->attr.columns;
			if (cell->c != 0 &&
					_deepinvte_draw_get_char_width (
...
static gboolean
deepinvte_invalidate_cursor_periodic (DeepinvteTerminal *terminal)
{
        DeepinvteTerminalPrivate *pvt = terminal->pvt;

	pvt->cursor_blink_state = !pvt->cursor_blink_state;
	pvt->cursor_blink_time += pvt->cursor_blink_cycle;

	_deepinvte_invalidate_cursor_once(terminal, TRUE);
	...
	pvt->cursor_blink_tag = g_timeout_add_full(G_PRIORITY_LOW,
						   ...
						   (GSourceFunc)deepinvte_invalidate_cursor_periodic,
						   terminal,
						   NULL);
	return FALSE;
}
...
static void
deepinvte_terminal_emit_selection_changed(DeepinvteTerminal *terminal)
{
...
static void
deepinvte_terminal_emit_commit(DeepinvteTerminal *terminal, const gchar *text, guint length)
{
...
static void
deepinvte_terminal_emit_emulation_changed(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
static void
deepinvte_terminal_emit_encoding_changed(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
static void
deepinvte_terminal_emit_child_exited(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
static void
deepinvte_terminal_emit_contents_changed(DeepinvteTerminal *terminal)
{
	if (terminal->pvt->contents_changed_pending) {
		/* Update dingus match set. */
		deepinvte_terminal_match_contents_clear(terminal);
		if (terminal->pvt->mouse_cursor_visible) {
			deepinvte_terminal_match_hilite_update(terminal,
...
void
_deepinvte_terminal_queue_contents_changed(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
static void
deepinvte_terminal_emit_cursor_moved(DeepinvteTerminal *terminal)
{
...
static void
deepinvte_terminal_queue_cursor_moved(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
static gboolean
deepinvte_terminal_emit_eof(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
			"Emitting `eof'.\n");
	GDK_THREADS_ENTER ();
...
/* Emit a "eof" signal. */
static void
deepinvte_terminal_queue_eof(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
			"Queueing `eof'.\n");
	g_idle_add_full (G_PRIORITY_HIGH,
		(GSourceFunc) deepinvte_terminal_emit_eof,
...
static void
deepinvte_terminal_emit_char_size_changed(DeepinvteTerminal *terminal,
...
static void
_deepinvte_terminal_emit_status_line_changed(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
static void
deepinvte_terminal_emit_increase_font_size(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
static void
deepinvte_terminal_emit_decrease_font_size(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
void
_deepinvte_terminal_emit_text_inserted(DeepinvteTerminal *terminal)
{
...
void
_deepinvte_terminal_emit_text_deleted(DeepinvteTerminal *terminal)
{
	if (!terminal->pvt->accessible_emit) {
		return;
	}
...
static void
deepinvte_terminal_emit_text_modified(DeepinvteTerminal *terminal)
{
	if (!terminal->pvt->accessible_emit) {
		return;
	}
...
static void
deepinvte_terminal_emit_text_scrolled(DeepinvteTerminal *terminal, gint delta)
{
	if (!terminal->pvt->accessible_emit) {
		return;
	}
...
static void
deepinvte_terminal_deselect_all(DeepinvteTerminal *terminal)
{
	if (terminal->pvt->has_selection) {
		gint sx, sy, ex, ey;

		_deepinvte_debug_print(DEEPINVTE_DEBUG_SELECTION,
		...
		deepinvte_terminal_emit_selection_changed(terminal);
		...
		_deepinvte_invalidate_region(terminal,
				MIN (sx, ex), MAX (sx, ex),
				MIN (sy, ey),   MAX (sy, ey),
				FALSE);
	}
...
/* Remove a tabstop. */
void
_deepinvte_terminal_clear_tabstop(DeepinvteTerminal *terminal, int column)
{
	g_assert(DEEPINVTE_IS_TERMINAL(terminal));
...
gboolean
_deepinvte_terminal_get_tabstop(DeepinvteTerminal *terminal, int column)
{
	gpointer hash;
	g_assert(DEEPINVTE_IS_TERMINAL(terminal));
	if (terminal->pvt->tabstops != NULL) {
...
void
_deepinvte_terminal_set_tabstop(DeepinvteTerminal *terminal, int column)
{
	g_assert(DEEPINVTE_IS_TERMINAL(terminal));
	if (terminal->pvt->tabstops != NULL) {
...
/* Reset the set of tab stops to the default. */
static void
deepinvte_terminal_set_default_tabstops(DeepinvteTerminal *terminal)
{
	int i, width = 0;
	if (terminal->pvt->tabstops != NULL) {
	...
	if (terminal->pvt->termcap != NULL) {
		width = _deepinvte_termcap_find_numeric(terminal->pvt->termcap,
						  terminal->pvt->emulation,
						  "it");
	}
	if (width == 0) {
		width = DEEPINVTE_TAB_WIDTH;
	}
	for (i = 0; i <= DEEPINVTE_TAB_MAX; i += width) {
		_deepinvte_terminal_set_tabstop(terminal, i);
	}
}

/* Clear the cache of the screen contents we keep. */
static void
deepinvte_terminal_match_contents_clear(DeepinvteTerminal *terminal)
{
	g_assert(DEEPINVTE_IS_TERMINAL(terminal));
	...
	deepinvte_terminal_match_hilite_clear(terminal);
}

/* Refresh the cache of the screen contents we keep. */
static gboolean
always_selected(DeepinvteTerminal *terminal, glong column, glong row, gpointer data)
{
	return TRUE;
}

static void
deepinvte_terminal_match_contents_refresh(DeepinvteTerminal *terminal)
{
	GArray *array;
	deepinvte_terminal_match_contents_clear(terminal);
	array = g_array_new(FALSE, TRUE, sizeof(struct _DeepinvteCharAttributes));
	terminal->pvt->match_contents = deepinvte_terminal_get_text(terminal,
							      always_selected,
							      NULL,
							      array);
...
static void
regex_match_clear_cursor (struct deepinvte_match_regex *regex)
{
        switch (regex->cursor_mode) {
                case DEEPINVTE_REGEX_CURSOR_GDKCURSOR:
                        if (regex->cursor.cursor != NULL) {
                                gdk_cursor_unref(regex->cursor.cursor);
                                regex->cursor.cursor = NULL;
                        }
                        break;
                case DEEPINVTE_REGEX_CURSOR_GDKCURSORTYPE:
                        break;
                case DEEPINVTE_REGEX_CURSOR_NAME:
...
static void
regex_match_clear (struct deepinvte_match_regex *regex)
{
        regex_match_clear_cursor(regex);

        if (regex->mode == DEEPINVTE_REGEX_GREGEX) {
                g_regex_unref(regex->regex.gregex.regex);
                regex->regex.gregex.regex = NULL;
        } else if (regex->mode == DEEPINVTE_REGEX_DEEPINVTE) {
                _deepinvte_regex_free(regex->regex.reg);
...
static void
deepinvte_terminal_set_cursor_from_regex_match(DeepinvteTerminal *terminal, struct deepinvte_match_regex *regex)
{
...
/**
 * deepinvte_terminal_match_clear_all:
 ...
 */
void
deepinvte_terminal_match_clear_all(DeepinvteTerminal *terminal)
{
	struct deepinvte_match_regex *regex;
	guint i;
...
/**
 * deepinvte_terminal_match_remove:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_match_remove(DeepinvteTerminal *terminal, int tag)
{
	struct deepinvte_match_regex *regex;
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
...
static GdkCursor *
deepinvte_terminal_cursor_new(DeepinvteTerminal *terminal, GdkCursorType cursor_type)
{
	GdkDisplay *display;
	GdkCursor *cursor;
 ...
 *
 * Deprecated: 0.17.1: Use deepinvte_terminal_match_add_gregex() instead
 */
int
deepinvte_terminal_match_add(DeepinvteTerminal *terminal, const char *match)
{
	struct deepinvte_match_regex new_regex, *regex;
	guint ret;
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), -1);
        g_return_val_if_fail(terminal->pvt->match_regex_mode != DEEPINVTE_REGEX_GREGEX, -1);
	g_return_val_if_fail(match != NULL, -1);
	g_return_val_if_fail(strlen(match) > 0, -1);

        terminal->pvt->match_regex_mode = DEEPINVTE_REGEX_DEEPINVTE;

	memset(&new_regex, 0, sizeof(new_regex));
        new_regex.mode = DEEPINVTE_REGEX_DEEPINVTE;
	new_regex.regex.reg = _deepinvte_regex_compile(match);
	...
	new_regex.tag = ret;
        new_regex.cursor_mode = DEEPINVTE_REGEX_CURSOR_GDKCURSORTYPE;
        new_regex.cursor.cursor_type = DEEPINVTE_DEFAULT_CURSOR;
...
/**
 * deepinvte_terminal_match_add_gregex:
 * @terminal: a #DeepinvteTerminal
 * @regex: a #GRegex
 ...
 */
int
deepinvte_terminal_match_add_gregex(DeepinvteTerminal *terminal, GRegex *regex, GRegexMatchFlags flags)
{
	DeepinvteTerminalPrivate *pvt;
	struct deepinvte_match_regex new_regex_match, *regex_match;
	guint ret, len;

	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), -1);
        g_return_val_if_fail(terminal->pvt->match_regex_mode != DEEPINVTE_REGEX_DEEPINVTE, -1);
	g_return_val_if_fail(regex != NULL, -1);

        pvt = terminal->pvt;
        pvt->match_regex_mode = DEEPINVTE_REGEX_GREGEX;
	...
	/* Set the tag to the insertion point. */
        new_regex_match.mode = DEEPINVTE_REGEX_GREGEX;
        new_regex_match.regex.gregex.regex = g_regex_ref(regex);
        new_regex_match.regex.gregex.flags = flags;
	new_regex_match.tag = ret;
        new_regex_match.cursor_mode = DEEPINVTE_REGEX_CURSOR_GDKCURSORTYPE;
        new_regex_match.cursor.cursor_type = DEEPINVTE_DEFAULT_CURSOR;
...
/**
 * deepinvte_terminal_match_set_cursor:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_match_set_cursor(DeepinvteTerminal *terminal, int tag, GdkCursor *cursor)
{
	struct deepinvte_match_regex *regex;
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
	...
	regex = &g_array_index(terminal->pvt->match_regexes,
			       ...
			       tag);
        regex_match_clear_cursor(regex);
        regex->cursor_mode = DEEPINVTE_REGEX_CURSOR_GDKCURSOR;
	regex->cursor.cursor = cursor ? gdk_cursor_ref(cursor) : NULL;
	deepinvte_terminal_match_hilite_clear(terminal);
}

/**
 * deepinvte_terminal_match_set_cursor_type:
 * @terminal: a #DeepinvteTerminal
 * @tag: the tag of the regex which should use the specified cursor
 ...
 */
void
deepinvte_terminal_match_set_cursor_type(DeepinvteTerminal *terminal,
				   int tag, GdkCursorType cursor_type)
{
	...
	regex = &g_array_index(terminal->pvt->match_regexes,
			       ...
			       tag);
        regex_match_clear_cursor(regex);
        regex->cursor_mode = DEEPINVTE_REGEX_CURSOR_GDKCURSORTYPE;
	regex->cursor.cursor_type = cursor_type;
	deepinvte_terminal_match_hilite_clear(terminal);
}

/**
 * deepinvte_terminal_match_set_cursor_name:
 * @terminal: a #DeepinvteTerminal
 * @tag: the tag of the regex which should use the specified cursor
 ...
 */
void
deepinvte_terminal_match_set_cursor_name(DeepinvteTerminal *terminal,
				   int tag, const char *cursor_name)
{
	...
	regex = &g_array_index(terminal->pvt->match_regexes,
			       ...
			       tag);
        regex_match_clear_cursor(regex);
        regex->cursor_mode = DEEPINVTE_REGEX_CURSOR_NAME;
 ...
 * argument. */
static char *
deepinvte_terminal_match_check_internal_deepinvte(DeepinvteTerminal *terminal,
                                      long column, glong row,
                                      int *tag, int *start, int *end)
{
	struct _deepinvte_regex_match matches[256];
	guint i, j;
	gint k;
	gint start_blank, end_blank;
	int ret, offset;
	struct deepinvte_match_regex *regex = NULL;
	struct _DeepinvteCharAttributes *attr = NULL;
	gssize sattr, eattr;
	gchar *line, eol;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS,
	...
	*tag = -1;
	...
	for (i = 0; i < terminal->pvt->match_regexes->len; i++) {
		...
		k = 0;
		ret = _deepinvte_regex_exec(regex->regex.reg,
				      line + k,
		...
		while (ret == 0) {
			...
			for (j = 0;
				...
				_DEEPINVTE_DEBUG_IF(DEEPINVTE_DEBUG_MISC) {
					gchar *match;
					struct _DeepinvteCharAttributes *_sattr, *_eattr;
				...
				if (ko >= matches[j].rm_so &&
					...
					if (end != NULL) {
						*end = sattr + k + matches[j].rm_eo - 1;
                                        ...
                                        deepinvte_terminal_set_cursor_from_regex_match(terminal, regex);
 ...
 * argument. */
static char *
deepinvte_terminal_match_check_internal_gregex(DeepinvteTerminal *terminal,
                                         long column, glong row,
                                         int *tag, int *start, int *end)
{
	gint start_blank, end_blank;
...
static char *
deepinvte_terminal_match_check_internal(DeepinvteTerminal *terminal,
                                  long column, glong row,
                                  int *tag, int *start, int *end)
{
	if (terminal->pvt->match_contents == NULL) {
		deepinvte_terminal_match_contents_refresh(terminal);
	}

        if (terminal->pvt->match_regex_mode == DEEPINVTE_REGEX_GREGEX)
                return deepinvte_terminal_match_check_internal_gregex(terminal, column, row, tag, start, end);
        if (terminal->pvt->match_regex_mode == DEEPINVTE_REGEX_DEEPINVTE)
                return deepinvte_terminal_match_check_internal_deepinvte(terminal, column, row, tag, start, end);
        return NULL;
}

static gboolean
rowcol_inside_match (DeepinvteTerminal *terminal, glong row, glong col)
{
...
/**
 * deepinvte_terminal_match_check:
 * @terminal: a #DeepinvteTerminal
 ...
 * regular expressions previously set using deepinvte_terminal_match_add().  If a
 * match exists, the text string is returned and if @tag is not %NULL, the number
 * associated with the matched regular expression will be stored in @tag.
 *
 * If more than one regular expression has been set with
 * deepinvte_terminal_match_add(), then expressions are checked in the order in
 ...
 */
char *
deepinvte_terminal_match_check(DeepinvteTerminal *terminal, glong column, glong row,
			 int *tag)
{
	long delta;
	char *ret;
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
	...
	if (rowcol_inside_match (terminal, row + delta, column)) {
		...
		ret = deepinvte_terminal_match_check_internal(terminal,
				column, row + delta,
				tag, NULL, NULL);
	}
	_DEEPINVTE_DEBUG_IF(DEEPINVTE_DEBUG_EVENTS) {
...
static void
deepinvte_terminal_emit_adjustment_changed(DeepinvteTerminal *terminal)
{
	if (terminal->pvt->adjustment_changed_pending) {
		...
		v = _deepinvte_ring_delta (screen->row_data);
		current = gtk_adjustment_get_lower(terminal->adjustment);
		if (current != v) {
			_deepinvte_debug_print(DEEPINVTE_DEBUG_ADJ,
		...
		v = MAX(_deepinvte_ring_next(screen->row_data),
...
static inline void
deepinvte_terminal_queue_adjustment_changed(DeepinvteTerminal *terminal)
{
...
static void
deepinvte_terminal_queue_adjustment_value_changed(DeepinvteTerminal *terminal, glong v)
{
	if (v != terminal->pvt->screen->scroll_delta) {
		terminal->pvt->screen->scroll_delta = v;
...
static void
deepinvte_terminal_queue_adjustment_value_changed_clamped(DeepinvteTerminal *terminal, glong v)
{
	gdouble lower, upper;
	...
	deepinvte_terminal_queue_adjustment_value_changed (terminal, v);
}


void
_deepinvte_terminal_adjust_adjustments(DeepinvteTerminal *terminal)
{
	DeepinvteScreen *screen;
	long delta;
	...
	deepinvte_terminal_queue_adjustment_changed(terminal);

	/* The lower value should be the first row in the buffer. */
	screen = terminal->pvt->screen;
	delta = _deepinvte_ring_delta(screen->row_data);
	...
	if (screen->scroll_delta > screen->insert_delta) {
		deepinvte_terminal_queue_adjustment_value_changed(terminal,
				screen->insert_delta);
	}
}
...
static void
_deepinvte_terminal_adjust_adjustments_full (DeepinvteTerminal *terminal)
{
	gboolean changed = FALSE;
	gdouble v;
...
static void
deepinvte_terminal_scroll_lines(DeepinvteTerminal *terminal, gint lines)
{
	glong destination;
	_deepinvte_debug_print(DEEPINVTE_DEBUG_ADJ, "Scrolling %d lines.\n", lines);
	/* Calculate the ideal position where we want to be before clamping. */
	destination = terminal->pvt->screen->scroll_delta;
	destination += lines;
	/* Tell the scrollbar to adjust itself. */
	deepinvte_terminal_queue_adjustment_value_changed_clamped (terminal, destination);
}

/* Scroll a fixed number of pages up or down, in the current screen. */
static void
deepinvte_terminal_scroll_pages(DeepinvteTerminal *terminal, gint pages)
{
	deepinvte_terminal_scroll_lines(terminal, pages * terminal->row_count);
}

/* Scroll so that the scroll delta is the minimum value. */
static void
deepinvte_terminal_maybe_scroll_to_top(DeepinvteTerminal *terminal)
{
	deepinvte_terminal_queue_adjustment_value_changed (terminal,
			_deepinvte_ring_delta(terminal->pvt->screen->row_data));
}

static void
deepinvte_terminal_maybe_scroll_to_bottom(DeepinvteTerminal *terminal)
{
	glong delta;
	delta = terminal->pvt->screen->insert_delta;
	deepinvte_terminal_queue_adjustment_value_changed (terminal, delta);
	_deepinvte_debug_print(DEEPINVTE_DEBUG_ADJ,
			"Snapping to bottom of screen\n");
}

static void
_deepinvte_terminal_setup_utf8 (DeepinvteTerminal *terminal)
{
        DeepinvteTerminalPrivate *pvt = terminal->pvt;
        GError *error = NULL;

        if (!deepinvte_pty_set_utf8(pvt->pty,
...
/**
 * deepinvte_terminal_set_encoding:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_encoding(DeepinvteTerminal *terminal, const char *codeset)
{
        DeepinvteTerminalPrivate *pvt;
        GObject *object;
	...
	conv = _deepinvte_conv_open(codeset, "UTF-8");
	if (conv == DEEPINVTE_INVALID_CONV) {
		g_warning(_("Unable to convert characters from %s to %s."),
			  "UTF-8", codeset);
		/* fallback to no conversion */
		conv = _deepinvte_conv_open(codeset = "UTF-8", "UTF-8");
	}
	if (terminal->pvt->outgoing_conv != DEEPINVTE_INVALID_CONV) {
		_deepinvte_conv_close(terminal->pvt->outgoing_conv);
	}
	...
	if ((_deepinvte_buffer_length(terminal->pvt->outgoing) > 0) &&
	    (old_codeset != NULL)) {
		/* Convert back to UTF-8. */
		obuf1 = g_convert((gchar *)terminal->pvt->outgoing->data,
				  _deepinvte_buffer_length(terminal->pvt->outgoing),
				  "UTF-8",
				  old_codeset,
				  NULL,
				  &bytes_written,
		...
		if (obuf1 != NULL) {
			...
			obuf2 = g_convert(obuf1,
					  ...
					  NULL);
			if (obuf2 != NULL) {
				_deepinvte_buffer_clear(terminal->pvt->outgoing);
				_deepinvte_buffer_append(terminal->pvt->outgoing,
						   obuf2, bytes_written);
				g_free(obuf2);
			}
			g_free(obuf1);
	...
	_deepinvte_iso2022_state_set_codeset(terminal->pvt->iso2022,
				       terminal->pvt->encoding);

	_deepinvte_debug_print(DEEPINVTE_DEBUG_IO,
			"Set terminal encoding to `%s'.\n",
			terminal->pvt->encoding);
	deepinvte_terminal_emit_encoding_changed(terminal);

        g_object_thaw_notify(object);
}

/**
 * deepinvte_terminal_get_encoding:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
const char *
deepinvte_terminal_get_encoding(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
...
static inline DeepinvteRowData *
deepinvte_terminal_insert_rows (DeepinvteTerminal *terminal, guint cnt)
{
	DeepinvteRowData *row;
	do {
		row = _deepinvte_terminal_ring_append (terminal, FALSE);
	} while(--cnt);
	return row;
}
 ...
 * cursor position. */
DeepinvteRowData *
_deepinvte_terminal_ensure_row (DeepinvteTerminal *terminal)
{
	DeepinvteRowData *row;
	DeepinvteScreen *screen;
	gint delta;
	...
	delta = v - _deepinvte_ring_next(screen->row_data) + 1;
	if (delta > 0) {
		row = deepinvte_terminal_insert_rows (terminal, delta);
		_deepinvte_terminal_adjust_adjustments(terminal);
	} else {
		/* Find the row the cursor is in. */
		row = _deepinvte_ring_index_writable (screen->row_data, v);
	}
	g_assert(row != NULL);
...
static DeepinvteRowData *
deepinvte_terminal_ensure_cursor(DeepinvteTerminal *terminal)
{
	DeepinvteRowData *row;

	row = _deepinvte_terminal_ensure_row (terminal);
	_deepinvte_row_data_fill (row, &basic_cell.cell, terminal->pvt->screen->cursor_current.col);
...
void
_deepinvte_terminal_update_insert_delta(DeepinvteTerminal *terminal)
{
	long delta, rows;
	DeepinvteScreen *screen;
	...
	rows = _deepinvte_ring_next (screen->row_data);
	delta = screen->cursor_current.row - rows + 1;
	if (G_UNLIKELY (delta > 0)) {
		deepinvte_terminal_insert_rows (terminal, delta);
		rows = _deepinvte_ring_next (screen->row_data);
	}
	...
	delta = MAX(delta, _deepinvte_ring_delta(screen->row_data));
...
void
_deepinvte_terminal_set_pointer_visible(DeepinvteTerminal *terminal, gboolean visible)
{
	GdkWindow *window;
	struct deepinvte_match_regex *regex = NULL;
	...
	if (visible || !terminal->pvt->mouse_autohide) {
		if (terminal->pvt->mouse_tracking_mode) {
			_deepinvte_debug_print(DEEPINVTE_DEBUG_CURSOR,
...
/**
 * deepinvte_terminal_new:
 *
 * Creates a new terminal widget.
 *
 * Returns: (transfer full) (type Deepinvte.Terminal): a new #DeepinvteTerminal object
 */
GtkWidget *
deepinvte_terminal_new(void)
{
	return g_object_new(DEEPINVTE_TYPE_TERMINAL, NULL);
}

/* Set up a palette entry with a more-or-less match for the requested color. */
static void
deepinvte_terminal_set_color_internal(DeepinvteTerminal *terminal, int entry,
				const GdkColor *proposed)
{
	...
	if (entry == DEEPINVTE_DEF_BG) {
		deepinvte_terminal_queue_background_update(terminal);
	}

	/* and redraw */
	if (entry == DEEPINVTE_CUR_BG)
...
static void
deepinvte_terminal_generate_bold(const PangoColor *foreground,
...
/**
 * deepinvte_terminal_set_color_bold:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_color_bold(DeepinvteTerminal *terminal, const GdkColor *bold)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
	...
	_deepinvte_debug_print(DEEPINVTE_DEBUG_MISC,
			...
			bold->red, bold->green, bold->blue);
	deepinvte_terminal_set_color_internal(terminal, DEEPINVTE_BOLD_FG, bold);
}

/**
 * deepinvte_terminal_set_color_dim:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_color_dim(DeepinvteTerminal *terminal, const GdkColor *dim)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
	...
	deepinvte_terminal_set_color_internal(terminal, DEEPINVTE_DIM_FG, dim);
}
 ...
 */
void
deepinvte_terminal_set_color_foreground(DeepinvteTerminal *terminal,
				  const GdkColor *foreground)
{
	...
	_deepinvte_debug_print(DEEPINVTE_DEBUG_MISC,
			...
			foreground->red, foreground->green, foreground->blue);
	deepinvte_terminal_set_color_internal(terminal, DEEPINVTE_DEF_FG, foreground);
}
 ...
 */
void
deepinvte_terminal_set_color_background(DeepinvteTerminal *terminal,
				  const GdkColor *background)
{
	...
	deepinvte_terminal_set_color_internal(terminal, DEEPINVTE_DEF_BG, background);
}
 ...
 */
void
deepinvte_terminal_set_color_cursor(DeepinvteTerminal *terminal,
			      const GdkColor *cursor_background)
{
	...
	if (cursor_background != NULL) {
		...
		deepinvte_terminal_set_color_internal(terminal, DEEPINVTE_CUR_BG,
						cursor_background);
 ...
 */
void
deepinvte_terminal_set_color_highlight(DeepinvteTerminal *terminal,
				 const GdkColor *highlight_background)
{
	...
	if (highlight_background != NULL) {
		...
		deepinvte_terminal_set_color_internal(terminal, DEEPINVTE_DEF_HL,
 ...
 */
void
deepinvte_terminal_set_colors(DeepinvteTerminal *terminal,
			const GdkColor *foreground,
			const GdkColor *background,
	...
	for (i=0; i < G_N_ELEMENTS(terminal->pvt->palette); i++) {
		if (i < 16) {
			...
			case DEEPINVTE_DEF_FG:
				...
				break;
			case DEEPINVTE_BOLD_FG:
				deepinvte_terminal_generate_bold(&terminal->pvt->palette[DEEPINVTE_DEF_FG],
							   &terminal->pvt->palette[DEEPINVTE_DEF_BG],
							   1.8,
							   &color);
				break;
			case DEEPINVTE_DIM_FG:
				deepinvte_terminal_generate_bold(&terminal->pvt->palette[DEEPINVTE_DEF_FG],
							   &terminal->pvt->palette[DEEPINVTE_DEF_BG],
							   0.5,
							   &color);
				break;
			case DEEPINVTE_DEF_HL:
				color.red = 0xc000;
				color.blue = 0xc000;
				color.green = 0xc000;
				break;
			case DEEPINVTE_CUR_BG:
				color.red = 0x0000;
				color.blue = 0x0000;
				color.green = 0x0000;
				break;
		...
		deepinvte_terminal_set_color_internal(terminal, i, &color);
	}
...
/**
 * deepinvte_terminal_set_color_bold_rgba:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_color_bold_rgba(DeepinvteTerminal *terminal,
                                 const GdkRGBA *bold)
{
	GdkColor color;
	...
	deepinvte_terminal_set_color_bold(terminal, &color);
}

/**
 * deepinvte_terminal_set_color_dim_rgba:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_color_dim_rgba(DeepinvteTerminal *terminal,
                                const GdkRGBA *dim)
{
	GdkColor color;
	...
	deepinvte_terminal_set_color_dim(terminal, &color);
}

/**
 * deepinvte_terminal_set_color_foreground_rgba:
 * @terminal: a #DeepinvteTerminal
 * @foreground: the new foreground color
 *
 * Sets the foreground color used to draw normal text.
 ...
 */
void
deepinvte_terminal_set_color_foreground_rgba(DeepinvteTerminal *terminal,
				       const GdkRGBA *foreground)
{
	GdkColor color;

	deepinvte_terminal_set_color_foreground(terminal,
                                          gdk_color_from_rgba(&color, foreground));
}

/**
 * deepinvte_terminal_set_color_background_rgba:
 * @terminal: a #DeepinvteTerminal
 * @background: the new background color
 *
 * Sets the background color for text which does not have a specific background
 ...
 */
void
deepinvte_terminal_set_color_background_rgba(DeepinvteTerminal *terminal,
				       const GdkRGBA *background)
{
	GdkColor color;

	deepinvte_terminal_set_color_background(terminal,
                                          gdk_color_from_rgba (&color, background));
}

/**
 * deepinvte_terminal_set_color_cursor_rgba:
 * @terminal: a #DeepinvteTerminal
 * @cursor_background: (allow-none): the new color to use for the text cursor, or %NULL
 *
 * Sets the background color for text which is under the cursor.  If %NULL, text
 ...
 */
void
deepinvte_terminal_set_color_cursor_rgba(DeepinvteTerminal *terminal,
				   const GdkRGBA *cursor_background)
{
        GdkColor color;

	deepinvte_terminal_set_color_cursor(terminal,
                                      gdk_color_from_rgba(&color, cursor_background));
}

/**
 * deepinvte_terminal_set_color_highlight_rgba:
 * @terminal: a #DeepinvteTerminal
 * @highlight_background: (allow-none): the new color to use for highlighted text, or %NULL
 *
 * Sets the background color for text which is highlighted.  If %NULL,
 ...
 */
void
deepinvte_terminal_set_color_highlight_rgba(DeepinvteTerminal *terminal,
				      const GdkRGBA *highlight_background)
{
	GdkColor color;

	deepinvte_terminal_set_color_highlight(terminal,
                                         gdk_color_from_rgba(&color, highlight_background));
}

/**
 * deepinvte_terminal_set_colors_rgba:
 * @terminal: a #DeepinvteTerminal
 * @foreground: (allow-none): the new foreground color, or %NULL
 * @background: (allow-none): the new background color, or %NULL
 ...
 */
void
deepinvte_terminal_set_colors_rgba(DeepinvteTerminal *terminal,
			     const GdkRGBA *foreground,
			     const GdkRGBA *background,
			     ...
			     gsize palette_size)
{
	...
	deepinvte_terminal_set_colors(terminal,
...
/**
 * deepinvte_terminal_set_opacity:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_opacity(DeepinvteTerminal *terminal, guint16 opacity)
{
        DeepinvteTerminalPrivate *pvt;
...
/**
 * deepinvte_terminal_set_default_colors:
 * @terminal: a #DeepinvteTerminal
 *
 * Reset the terminal palette to reasonable compiled-in default color.
 */
void
deepinvte_terminal_set_default_colors(DeepinvteTerminal *terminal)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
	deepinvte_terminal_set_colors(terminal, NULL, NULL, NULL, 0);
}


/* Cleanup smart-tabs.  See deepinvte_sequence_handler_ta() */
void
_deepinvte_terminal_cleanup_tab_fragments_at_cursor (DeepinvteTerminal *terminal)
{
	DeepinvteRowData *row = _deepinvte_terminal_ensure_row (terminal);
	DeepinvteScreen *screen = terminal->pvt->screen;
	long col = screen->cursor_current.col;
	const DeepinvteCell *pcell = _deepinvte_row_data_get (row, col);

	if (G_UNLIKELY (pcell != NULL && pcell->c == '\t')) {
		long i, num_columns;
		DeepinvteCell *cell = _deepinvte_row_data_get_writable (row, col);
		...
		while (cell->attr.fragment && col > 0)
			cell = _deepinvte_row_data_get_writable (row, --col);
...
void
_deepinvte_terminal_cursor_down (DeepinvteTerminal *terminal)
{
	long start, end;
	DeepinvteScreen *screen;
	...
	if (screen->cursor_current.row == end) {
		...
		if (screen->scrolling_restricted) {
			if (start == screen->insert_delta) {
				...
				_deepinvte_terminal_ring_insert (terminal, screen->cursor_current.row, FALSE);
				/* Force the areas below the region to be
				 * redrawn -- they've moved. */
				_deepinvte_terminal_scroll_region(terminal, start,
							    end - start + 1, 1);
				/* Force scroll. */
				_deepinvte_terminal_adjust_adjustments(terminal);
			} else {
			...
			screen->cursor_current.row++;
			_deepinvte_terminal_update_insert_delta(terminal);
		}
...
gboolean
_deepinvte_terminal_insert_char(DeepinvteTerminal *terminal, gunichar c,
			 gboolean insert, gboolean invalidate_now)
{
	...
	if (G_UNLIKELY (screen->alternate_charset)) {
		...
		c = _deepinvte_iso2022_process_single(terminal->pvt->iso2022, c, '0');
	}
	...
	if (G_UNLIKELY (DEEPINVTE_ISO2022_HAS_ENCODED_WIDTH(c))) {
		columns = _deepinvte_iso2022_get_encoded_width(c);
		c &= ~DEEPINVTE_ISO2022_ENCODED_WIDTH_MASK;
	} else {
		columns = _deepinvte_iso2022_unichar_width(terminal->pvt->iso2022, c);
	}
	...
	if (G_UNLIKELY (columns == 0)) {
		...
		_deepinvte_debug_print(DEEPINVTE_DEBUG_PARSE, "combining U+%04X", c);
		...
		if (G_UNLIKELY (col == 0)) {
			...
			if (G_LIKELY (row_num > 0)) {
				row_num--;
				row = _deepinvte_terminal_find_row_data_writable (terminal, row_num);

				if (row) {
					if (!row->attr.soft_wrapped)
						row = NULL;
					else
						col = _deepinvte_row_data_length (row);
				}
			}
		} else {
			row = _deepinvte_terminal_find_row_data_writable (terminal, row_num);
		...
		c = _deepinvte_unistr_append_unichar (cell->c, c);
	...
	row = deepinvte_terminal_ensure_cursor (terminal);
	g_assert(row != NULL);
	...
	if (insert) {
		for (i = 0; i < columns; i++)
			_deepinvte_row_data_insert (row, col + i, &screen->color_defaults);
	} else {
		_deepinvte_row_data_fill (row, &basic_cell.cell, col + columns);
	}
	...
	if (G_LIKELY (col > 0)) {
		glong col2 = col - 1;
		DeepinvteCell *cell = _deepinvte_row_data_get_writable (row, col2);
		while (col2 > 0 && cell != NULL && cell->attr.fragment)
			cell = _deepinvte_row_data_get_writable (row, --col2);
	...
	{
		DeepinvteCell *pcell = _deepinvte_row_data_get_writable (row, col);
		pcell->c = c;
		pcell->attr = attr;
		col++;
	}
	...
	_deepinvte_row_data_shrink (row, terminal->column_count);
...
not_inserted:
	_deepinvte_debug_print(DEEPINVTE_DEBUG_ADJ|DEEPINVTE_DEBUG_PARSE,
			"insertion delta => %ld.\n",
			(long)screen->insert_delta);
	return line_wrapped;
}

/* Catch a DeepinvteReaper child-exited signal, and if it matches the one we're
 * looking for, emit one of our own. */
static void
deepinvte_terminal_catch_child_exited(DeepinvteReaper *reaper, int pid, int status,
				DeepinvteTerminal *terminal)
{
	if (pid == terminal->pvt->pty_pid) {
		...
		_DEEPINVTE_DEBUG_IF (DEEPINVTE_DEBUG_LIFECYCLE) {
		...
		/* Close out the PTY. */
                deepinvte_terminal_set_pty_object(terminal, NULL);

		/* Tell observers what's happened. */
                terminal->pvt->child_exit_status = status;
		deepinvte_terminal_emit_child_exited(terminal);
...
static void mark_input_source_invalid(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print (DEEPINVTE_DEBUG_IO, "removed poll of deepinvte_terminal_io_read\n");
	terminal->pvt->pty_input_source = 0;
}
static void
_deepinvte_terminal_connect_pty_read(DeepinvteTerminal *terminal)
{
	...
	if (terminal->pvt->pty_input_source == 0) {
		_deepinvte_debug_print (DEEPINVTE_DEBUG_IO, "polling deepinvte_terminal_io_read\n");
		terminal->pvt->pty_input_source =
			g_io_add_watch_full(terminal->pvt->pty_channel,
					    DEEPINVTE_CHILD_INPUT_PRIORITY,
					    G_IO_IN | G_IO_HUP,
					    (GIOFunc) deepinvte_terminal_io_read,
					    terminal,
					    (GDestroyNotify) mark_input_source_invalid);
	}
}

static void mark_output_source_invalid(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print (DEEPINVTE_DEBUG_IO, "removed poll of deepinvte_terminal_io_write\n");
	terminal->pvt->pty_output_source = 0;
}
static void
_deepinvte_terminal_connect_pty_write(DeepinvteTerminal *terminal)
{
        DeepinvteTerminalPrivate *pvt = terminal->pvt;
	...
	if (terminal->pvt->pty_channel == NULL) {
		pvt->pty_channel =
			g_io_channel_unix_new(deepinvte_pty_get_fd(pvt->pty));
	}

	if (terminal->pvt->pty_output_source == 0) {
		if (deepinvte_terminal_io_write (terminal->pvt->pty_channel,
					     G_IO_OUT,
					     terminal))
		{
			_deepinvte_debug_print (DEEPINVTE_DEBUG_IO, "polling deepinvte_terminal_io_write\n");
			terminal->pvt->pty_output_source =
				g_io_add_watch_full(terminal->pvt->pty_channel,
						    DEEPINVTE_CHILD_OUTPUT_PRIORITY,
						    G_IO_OUT,
						    (GIOFunc) deepinvte_terminal_io_write,
						    terminal,
...
static void
_deepinvte_terminal_disconnect_pty_read(DeepinvteTerminal *terminal)
{
	if (terminal->pvt->pty_input_source != 0) {
		_deepinvte_debug_print (DEEPINVTE_DEBUG_IO, "disconnecting poll of deepinvte_terminal_io_read\n");
...
static void
_deepinvte_terminal_disconnect_pty_write(DeepinvteTerminal *terminal)
{
	if (terminal->pvt->pty_output_source != 0) {
		_deepinvte_debug_print (DEEPINVTE_DEBUG_IO, "disconnecting poll of deepinvte_terminal_io_write\n");
...
/**
 * deepinvte_terminal_pty_new:
 * @terminal: a #DeepinvteTerminal
 * @flags: flags from #DeepinvtePtyFlags
 * @error: (allow-none): return location for a #GError, or %NULL
 *
 * Creates a new #DeepinvtePty, and sets the emulation property
 * from #DeepinvteTerminal:emulation.
 *
 * See deepinvte_pty_new() for more information.
 *
 * Returns: (transfer full): a new #DeepinvtePty
 * Since: 0.26
 */
DeepinvtePty *
deepinvte_terminal_pty_new(DeepinvteTerminal *terminal,
                     ...
                     GError **error)
{
        ...
        pty = deepinvte_pty_new(flags, error);
        if (pty == NULL)
                return NULL;

        deepinvte_pty_set_term(pty, deepinvte_terminal_get_emulation(terminal));

        return pty;
}

/**
 * deepinvte_terminal_watch_child:
 * @terminal: a #DeepinvteTerminal
 * @child_pid: a #GPid
 *
 * Watches @child_pid. When the process exists, the #DeepinvteReaper::child-exited
 * signal will be called. Use deepinvte_terminal_get_child_exit_status() to
 * retrieve the child's exit status.
 *
 * Prior to calling this function, a #DeepinvtePty must have been set in @terminal
 * using deepinvte_terminal_set_pty_object().
 * When the child exits, the terminal's #DeepinvtePty will be set to %NULL.
 *
 ...
 */
void
deepinvte_terminal_watch_child (DeepinvteTerminal *terminal,
                          GPid child_pid)
{
        DeepinvteTerminalPrivate *pvt;
        GObject *object;
        ...
        deepinvte_reaper_add_child(child_pid);
        if (reaper != pvt->pty_reaper) {
                ...
                g_signal_connect(reaper, "child-exited",
                                G_CALLBACK(deepinvte_terminal_catch_child_exited),
                                terminal);
        ...
        /* FIXMEchpe: call deepinvte_terminal_set_size here? */

        g_object_thaw_notify(object);
}

/**
 * deepinvte_get_user_shell:
 *
...
static char *
_deepinvte_terminal_get_user_shell_with_fallback (void)
{
        char *command;
        const gchar *env;

        if ((command = deepinvte_get_user_shell ()))
                return command;
 ...
 * Like __deepinvte_pty_get_argv(), but returns the argument vector to spawn
 * the user's shell if @command is %NULL.
 *
 * Returns: a newly allocated array of strings. Free with g_strfreev()
 */
static char **
_deepinvte_terminal_get_argv (const char *command,
                        char **argv,
                        GSpawnFlags *flags /* inout */)
{
	char **argv2;
        ...
        argv2 = __deepinvte_pty_get_argv(command ? command : (shell = _deepinvte_terminal_get_user_shell_with_fallback ()),
                                   argv,
                                   flags);
        g_free(shell);
        return argv2;
 ...
 *
 * Deprecated: 0.26: Use deepinvte_terminal_fork_command_full()
 */
pid_t
deepinvte_terminal_fork_command(DeepinvteTerminal *terminal,
			  const char *command,
                          char **argv,
                          char **envv,
                          ...
                          gboolean wtmp)
{
        ...
        spawn_flags = G_SPAWN_CHILD_INHERITS_STDIN |
                      G_SPAWN_SEARCH_PATH;
        real_argv = _deepinvte_terminal_get_argv (command, argv, &spawn_flags);

        ret = deepinvte_terminal_fork_command_full(terminal,
                                             __deepinvte_pty_get_pty_flags(lastlog, utmp, wtmp),
                                             working_directory,
                                             real_argv,
                                             envv,
                                             spawn_flags,
        ...
        if (error) {
                _deepinvte_debug_print(DEEPINVTE_DEBUG_MISC,
                                "deepinvte_terminal_fork_command failed: %s\n", error->message);
                g_error_free(error);
        }
#endif
 ...
 * @terminal: a #DeepinvteTerminal
 * @pty_flags: flags from #DeepinvtePtyFlags
 ...
 * in the child.
 *
 * See deepinvte_pty_new(), g_spawn_async() and deepinvte_terminal_watch_child() for more information.
 *
 ...
 */
gboolean
deepinvte_terminal_fork_command_full(DeepinvteTerminal *terminal,
                               ...
                               GError **error)
{
        ...
        g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), FALSE);
        g_return_val_if_fail(argv != NULL, FALSE);
        g_return_val_if_fail(child_setup_data == NULL || child_setup, FALSE);
        g_return_val_if_fail(error == NULL || *error == NULL, FALSE);

        pty = deepinvte_pty_new(pty_flags, error);
        if (pty == NULL)
                return FALSE;
        ...
        deepinvte_terminal_set_pty_object(terminal, pty);
        deepinvte_terminal_watch_child(terminal, pid);
        g_object_unref (pty);
 ...
 * Since: 0.11.11
 *
 * Deprecated: 0.26: Use #DeepinvtePty and fork() instead
 */
pid_t
deepinvte_terminal_forkpty(DeepinvteTerminal *terminal,
...
static void
deepinvte_terminal_eof(GIOChannel *channel, DeepinvteTerminal *terminal)
{
        GObject *object = G_OBJECT(terminal);
	...
	deepinvte_terminal_queue_eof(terminal);
...
static void
deepinvte_terminal_im_reset(DeepinvteTerminal *terminal)
{
	if (gtk_widget_get_realized (&terminal->widget)) {
...
static void
deepinvte_terminal_emit_pending_text_signals(DeepinvteTerminal *terminal, GQuark quark)
{
	static struct {
		const char *name;
		GQuark quark;
	...
	if (terminal->pvt->text_modified_flag) {
		...
		deepinvte_terminal_emit_text_modified(terminal);
...
static void
deepinvte_terminal_process_incoming(DeepinvteTerminal *terminal)
{
	DeepinvteScreen *screen;
	DeepinvteVisualPosition cursor;
	...
	GArray *unichars;
	struct _deepinvte_incoming_chunk *chunk, *next_chunk, *achunk = NULL;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_IO,
			"Handler processing %"G_GSIZE_FORMAT" bytes over %"G_GSIZE_FORMAT" chunks + %d bytes pending.\n",
			_deepinvte_incoming_chunks_length(terminal->pvt->incoming),
			_deepinvte_incoming_chunks_count(terminal->pvt->incoming),
	...
	for (chunk = _deepinvte_incoming_chunks_reverse (terminal->pvt->incoming);
			chunk != NULL;
			chunk = next_chunk) {
		gsize processed;
		...
		processed = _deepinvte_iso2022_process(terminal->pvt->iso2022,
...
skip_chunk:
	...
	while (start < wcount && !leftovers) {
		...
		_deepinvte_matcher_match(terminal->pvt->matcher,
				   &wbuf[start],
				   wcount - start,
				   &match,
				   &next,
		...
		if ((match != NULL) && (match[0] != '\0')) {
			 ...
			 * behavior. */
			_deepinvte_terminal_handle_sequence(terminal,
						      match,
						      quark,
						      params);
			...
			if (invalidated_text &&
					(screen->cursor_current.col > bbox_bottomright.x + DEEPINVTE_CELL_BBOX_SLACK ||
					 screen->cursor_current.col < bbox_topleft.x - DEEPINVTE_CELL_BBOX_SLACK     ||
					 screen->cursor_current.row > bbox_bottomright.y + DEEPINVTE_CELL_BBOX_SLACK ||
					 screen->cursor_current.row < bbox_topleft.y - DEEPINVTE_CELL_BBOX_SLACK)) {
		...
		if (match == NULL) {
			...
			_DEEPINVTE_DEBUG_IF(DEEPINVTE_DEBUG_PARSE) {
				gunichar cc = c & ~DEEPINVTE_ISO2022_ENCODED_WIDTH_MASK;
				if (cc > 255) {
					g_printerr("U+%04lx\n", (long) cc);
				} else {
					if (cc > 127) {
			...
			if (G_UNLIKELY (_deepinvte_terminal_insert_char(terminal, c,
						 FALSE, FALSE))) {
		...
		g_assert(screen->insert_delta >=
			 _deepinvte_ring_delta(screen->row_data));
...
next_match:
		if (G_LIKELY(params != NULL)) {
			...
			_deepinvte_matcher_free_params_array(terminal->pvt->matcher,
					params);
		}
	}
	...
	if (modified) {
		...
		if (terminal->pvt->scroll_on_output || bottom) {
			deepinvte_terminal_maybe_scroll_to_bottom(terminal);
		}
	...
	deepinvte_terminal_emit_pending_signals (terminal);
	...
	if ((cursor.col != terminal->pvt->screen->cursor_current.col) ||
		...
		if (cursor_visible)
			_deepinvte_invalidate_cell(terminal, cursor.col, cursor.row);
		_deepinvte_invalidate_cursor_once(terminal, FALSE);
	...
	_deepinvte_debug_print (DEEPINVTE_DEBUG_IO,
			...
			(long) unichars->len,
			(long) _deepinvte_incoming_chunks_length(terminal->pvt->incoming),
			_deepinvte_incoming_chunks_count(terminal->pvt->incoming));
}

static inline void
_deepinvte_terminal_enable_input_source (DeepinvteTerminal *terminal)
{
	if (terminal->pvt->pty_channel == NULL) {
		return;
	}
...
static void
_deepinvte_terminal_feed_chunks (DeepinvteTerminal *terminal, struct _deepinvte_incoming_chunk *chunks)
{
	struct _deepinvte_incoming_chunk *last;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_IO, "Feed %"G_GSIZE_FORMAT" bytes, in %"G_GSIZE_FORMAT" chunks.\n",
			_deepinvte_incoming_chunks_length(chunks),
			_deepinvte_incoming_chunks_count(chunks));
...
static gboolean
deepinvte_terminal_io_read(GIOChannel *channel,
		     GIOCondition condition,
		     DeepinvteTerminal *terminal)
{
	int err = 0;
	...
	if (condition & G_IO_IN) {
		struct _deepinvte_incoming_chunk *chunk, *chunks = NULL;
		...
		if (max_bytes) {
			...
			max_bytes = DEEPINVTE_MAX_INPUT_READ;
		}
...
out:
		...
		if (chunks != NULL) {
			_deepinvte_terminal_feed_chunks (terminal, chunks);
		}
		if (!deepinvte_terminal_is_processing (terminal)) {
			GDK_THREADS_ENTER ();
			deepinvte_terminal_add_process_timeout (terminal);
			GDK_THREADS_LEAVE ();
		}
		...
		_deepinvte_debug_print (DEEPINVTE_DEBUG_IO, "read %d/%d bytes, again? %s, active? %s\n",
				bytes, max_bytes,
				again ? "yes" : "no",
	...
	if (eof) {
		...
		if (!deepinvte_terminal_is_processing (terminal)) {
			GDK_THREADS_ENTER ();
			deepinvte_terminal_eof (channel, terminal);
			GDK_THREADS_LEAVE ();
		} else {
			deepinvte_terminal_eof (channel, terminal);
		}
 ...
 */
void
deepinvte_terminal_feed(DeepinvteTerminal *terminal, const char *data, glong length)
{
	...
	if (length > 0) {
		...
		deepinvte_terminal_start_processing (terminal);
	}
}

/* Send locally-encoded characters to the child. */
static gboolean
deepinvte_terminal_io_write(GIOChannel *channel,
		      GIOCondition condition,
		      DeepinvteTerminal *terminal)
{
	gssize count;
	...
	count = write(fd, terminal->pvt->outgoing->data,
		      _deepinvte_buffer_length(terminal->pvt->outgoing));
	if (count != -1) {
		_DEEPINVTE_DEBUG_IF (DEEPINVTE_DEBUG_IO) {
			gssize i;
			for (i = 0; i < count; i++) {
		...
		_deepinvte_buffer_consume(terminal->pvt->outgoing, count);
	}
...
static void
deepinvte_terminal_send(DeepinvteTerminal *terminal, const char *encoding,
		  ...
		  gboolean local_echo, gboolean newline_stuff)
{
	...
	icount = length;
	ibuf =  data;
	ocount = ((length + 1) * DEEPINVTE_UTF8_BPC) + 1;
	_deepinvte_buffer_set_minimum_size(terminal->pvt->conv_buffer, ocount);
	obuf = obufptr = terminal->pvt->conv_buffer->data;

	if (_deepinvte_conv(conv, &ibuf, &icount, &obuf, &ocount) == (gsize)-1) {
		...
		if (cooked_length > 0) {
			deepinvte_terminal_emit_commit(terminal,
		...
		if ((cooked_length > 0) && local_echo) {
			...
			if (ucs4 != NULL) {
				...
				for (i = 0; i < len; i++) {
					_deepinvte_terminal_insert_char(terminal,
								 ucs4[i],
								 FALSE,
								 TRUE);
				}
		...
		if ((cooked_length > 0) && (terminal->pvt->pty != NULL)) {
			_deepinvte_buffer_append(terminal->pvt->outgoing,
					   cooked, cooked_length);
			_DEEPINVTE_DEBUG_IF(DEEPINVTE_DEBUG_KEYBOARD) {
			...
			_deepinvte_terminal_connect_pty_write(terminal);
		}
		if (crcount > 0) {
			g_free(cooked);
		}
 ...
 */
void
deepinvte_terminal_feed_child(DeepinvteTerminal *terminal, const char *text, glong length)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
	if (length == ((gssize)-1)) {
		length = strlen(text);
	...
	if (length > 0) {
		deepinvte_terminal_send(terminal, "UTF-8", text, length,
				  FALSE, FALSE);
	}
}

/**
 * deepinvte_terminal_feed_child_binary:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_feed_child_binary(DeepinvteTerminal *terminal, const char *data, glong length)
{
	g_assert(DEEPINVTE_IS_TERMINAL(terminal));
...
static void
deepinvte_terminal_feed_child_using_modes(DeepinvteTerminal *terminal,
				    const char *data, glong length)
{
	...
	if (length > 0) {
		deepinvte_terminal_send(terminal, "UTF-8", data, length,
...
static void
deepinvte_terminal_im_commit(GtkIMContext *im_context, gchar *text, DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS,
			"Input method committed `%s'.\n", text);
	deepinvte_terminal_feed_child_using_modes(terminal, text, -1);
...
static void
deepinvte_terminal_im_preedit_start(GtkIMContext *im_context, DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS,
...
static void
deepinvte_terminal_im_preedit_end(GtkIMContext *im_context, DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS,
...
static void
deepinvte_terminal_im_preedit_changed(GtkIMContext *im_context, DeepinvteTerminal *terminal)
{
	gchar *str;
	PangoAttrList *attrs;
	gint cursor;
...
static gboolean
deepinvte_terminal_configure_toplevel(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Top level parent configured.\n");
...
static void
deepinvte_terminal_hierarchy_changed(GtkWidget *widget, GtkWidget *old_toplevel,
			       gpointer data)
{
	GtkWidget *toplevel;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Hierarchy changed.\n");
	...
	if (toplevel != NULL) {
		g_signal_connect_swapped (toplevel, "configure-event",
				 G_CALLBACK (deepinvte_terminal_configure_toplevel),
				 widget);
	}
}

static void
deepinvte_terminal_set_inner_border(DeepinvteTerminal *terminal)
{
        DeepinvteTerminalPrivate *pvt = terminal->pvt;
...
static void
deepinvte_terminal_style_set (GtkWidget      *widget,
			GtkStyle       *prev_style)
{
	DeepinvteTerminal *terminal = DEEPINVTE_TERMINAL(widget);
        float aspect;

        GTK_WIDGET_CLASS (deepinvte_terminal_parent_class)->style_set (widget, prev_style);

        if (gtk_widget_get_style(widget) == prev_style)
                return;

        deepinvte_terminal_set_font_full_internal(terminal, terminal->pvt->fontdesc,
                                            terminal->pvt->fontantialias);

        deepinvte_terminal_set_inner_border(terminal);
...
static void
add_cursor_timeout (DeepinvteTerminal *terminal)
{
...
static void
remove_cursor_timeout (DeepinvteTerminal *terminal)
{
...
static void
_deepinvte_check_cursor_blink(DeepinvteTerminal *terminal)
{
...
void
_deepinvte_terminal_audible_beep(DeepinvteTerminal *terminal)
{
	GdkDisplay *display;
...
void
_deepinvte_terminal_visible_beep(DeepinvteTerminal *terminal)
{
	GtkWidget *widget;
	GtkAllocation allocation;
	GtkStyle *style;
	...
	if (gtk_widget_get_realized (widget)) {
		...
		_deepinvte_draw_start(terminal->pvt->draw);
		_deepinvte_draw_fill_rectangle(terminal->pvt->draw,
					 0, 0,
					 allocation.width, allocation.height,
					 &color, DEEPINVTE_DRAW_OPAQUE);
		_deepinvte_draw_end(terminal->pvt->draw);
...
void
_deepinvte_terminal_beep(DeepinvteTerminal *terminal)
{
	if (terminal->pvt->audible_bell) {
		_deepinvte_terminal_audible_beep (terminal);
	}
...
static guint
deepinvte_translate_ctrlkey (GdkEventKey *event)
{
	guint keyval;
	GdkKeymap *keymap;
	unsigned int i;
...
static gint
deepinvte_terminal_key_press(GtkWidget *widget, GdkEventKey *event)
{
	DeepinvteTerminal *terminal;
	GdkModifierType modifiers;
	...
	guint keyval = 0;
	gunichar keychar = 0;
	char keybuf[DEEPINVTE_UTF8_BPC];

	terminal = DEEPINVTE_TERMINAL(widget);

	/* First, check if GtkWidget's behavior already does something with
	 * this key. */
	if (GTK_WIDGET_CLASS(deepinvte_terminal_parent_class)->key_press_event) {
		if ((GTK_WIDGET_CLASS(deepinvte_terminal_parent_class))->key_press_event(widget,
								      event)) {
			return TRUE;
		}
	}
	...
	if (event->type == GDK_KEY_PRESS) {
		...
		modifier = _deepinvte_keymap_key_is_modifier(keyval);

		/* Unless it's a modifier key, hide the pointer. */
		if (!modifier) {
			_deepinvte_terminal_set_pointer_visible(terminal, FALSE);
		}
		...
		if (!terminal->pvt->im_preedit_active) {
			...
			if (terminal->pvt->modifiers & DEEPINVTE_META_MASK) {
				steal = TRUE;
			}
			switch (keyval) {
	...
	if ((event->type == GDK_KEY_PRESS) && !modifier) {
		...
		case GDK_KEY (BackSpace):
			...
			case DEEPINVTE_ERASE_ASCII_BACKSPACE:
				normal = g_strdup("");
				normal_length = 1;
				suppress_meta_esc = FALSE;
				break;
			case DEEPINVTE_ERASE_ASCII_DELETE:
				normal = g_strdup("");
				normal_length = 1;
				suppress_meta_esc = FALSE;
				break;
			case DEEPINVTE_ERASE_DELETE_SEQUENCE:
				special = "kD";
				suppress_meta_esc = TRUE;
				break;
			case DEEPINVTE_ERASE_TTY:
				if (terminal->pvt->pty != NULL &&
				    tcgetattr(deepinvte_pty_get_fd(terminal->pvt->pty), &tio) != -1)
				{
...
#define _POSIX_VDISABLE '\0'
		...
		case GDK_KEY (Insert):
			if (modifiers & GDK_SHIFT_MASK) {
				if (modifiers & GDK_CONTROL_MASK) {
					deepinvte_terminal_paste_clipboard(terminal);
					handled = TRUE;
					suppress_meta_esc = TRUE;
				} else {
					deepinvte_terminal_paste_primary(terminal);
					handled = TRUE;
					suppress_meta_esc = TRUE;
				}
			} else if (modifiers & GDK_CONTROL_MASK) {
				deepinvte_terminal_copy_clipboard(terminal);
				handled = TRUE;
				suppress_meta_esc = TRUE;
			}
			break;
		...
		case GDK_KEY (Up):
			if (modifiers & GDK_CONTROL_MASK 
				...
				deepinvte_terminal_scroll_lines(terminal, -1);
				scrolled = TRUE;
				handled = TRUE;
				suppress_meta_esc = TRUE;
			}
		...
		case GDK_KEY (Page_Up):
			if (modifiers & GDK_SHIFT_MASK) {
				deepinvte_terminal_scroll_pages(terminal, -1);
				scrolled = TRUE;
				handled = TRUE;
				suppress_meta_esc = TRUE;
			}
		...
		case GDK_KEY (Home):
			if (modifiers & GDK_SHIFT_MASK) {
				deepinvte_terminal_maybe_scroll_to_top(terminal);
				scrolled = TRUE;
				handled = TRUE;
			}
			break;
		...
		case GDK_KEY (KP_Subtract):
			if (modifiers &
				...
				switch (keyval) {
				case GDK_KEY (KP_Add):
					deepinvte_terminal_emit_increase_font_size(terminal);
					handled = TRUE;
					suppress_meta_esc = TRUE;
					break;
				case GDK_KEY (KP_Subtract):
					deepinvte_terminal_emit_decrease_font_size(terminal);
					handled = TRUE;
					suppress_meta_esc = TRUE;
					break;
				}
		...
		if (modifiers & GDK_CONTROL_MASK)
			keyval = deepinvte_translate_ctrlkey(event);
		...
		if (normal != NULL) {
			if (terminal->pvt->meta_sends_escape &&
				...
				deepinvte_terminal_feed_child(terminal,
							_DEEPINVTE_CAP_ESC,
							1);
			}
			if (normal_length > 0) {
				deepinvte_terminal_feed_child_using_modes(terminal,
								    normal,
								    normal_length);
			}
			g_free(normal);
		...
		if (special != NULL && terminal->pvt->termcap != NULL) {
			...
			normal = _deepinvte_termcap_find_string_length(termcap,
								 tterm,
								 special,
								 &normal_length);
			_deepinvte_keymap_key_add_key_modifiers(keyval,
							  modifiers,
							  terminal->pvt->sun_fkey_mode,
							  terminal->pvt->hp_fkey_mode,
							  terminal->pvt->legacy_fkey_mode,
...
static gboolean
deepinvte_terminal_key_release(GtkWidget *widget, GdkEventKey *event)
{
	DeepinvteTerminal *terminal;
...
/**
 * deepinvte_terminal_is_word_char:
 * @terminal: a #DeepinvteTerminal
 * @c: a candidate Unicode code point
 *
 * Checks if a particular character is considered to be part of a word or not,
 * based on the values last passed to deepinvte_terminal_set_word_chars().
 *
 * Returns: %TRUE if the character is considered to be part of a word
 */
gboolean
deepinvte_terminal_is_word_char(DeepinvteTerminal *terminal, gunichar c)
{
	guint i;
	DeepinvteWordCharRange *range;
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), FALSE);
...
static gboolean
deepinvte_same_class(DeepinvteTerminal *terminal, glong acol, glong arow,
	       glong bcol, glong brow)
{
	const DeepinvteCell *pcell = NULL;
	gboolean word_char;
	if ((pcell = deepinvte_terminal_find_charcell(terminal, acol, arow)) != NULL && pcell->c != 0) {
		word_char = deepinvte_terminal_is_word_char(terminal, _deepinvte_unistr_get_base (pcell->c));

		/* Lets not group non-wordchars together (bug #25290) */
		if (!word_char)
			return FALSE;

		pcell = deepinvte_terminal_find_charcell(terminal, bcol, brow);
		if (pcell == NULL || pcell->c == 0) {
			return FALSE;
		}
		if (word_char != deepinvte_terminal_is_word_char(terminal, _deepinvte_unistr_get_base (pcell->c))) {
			return FALSE;
		}
		return TRUE;
	}
...
static gboolean
deepinvte_line_is_wrappable(DeepinvteTerminal *terminal, glong row)
{
	const DeepinvteRowData *rowdata;
	rowdata = _deepinvte_terminal_find_row_data(terminal, row);
...
static gboolean
deepinvte_cell_is_between(glong col, glong row,
...
static gboolean
deepinvte_cell_is_selected(DeepinvteTerminal *terminal, glong col, glong row, gpointer data)
{
	DeepinvteVisualPosition ss, se;
	...
	return deepinvte_cell_is_between(col, row, ss.col, ss.row, se.col, se.row, TRUE);
}

/* Once we get text data, actually paste it in. */
static void
deepinvte_terminal_paste_cb(GtkClipboard *clipboard, const gchar *text, gpointer data)
{
	DeepinvteTerminal *terminal;
	gchar *paste, *p;
	long length;
	...
	if (text != NULL) {
		...
		if (terminal->pvt->screen->bracketed_paste_mode)
			deepinvte_terminal_feed_child(terminal, "\e[200~", -1);
		deepinvte_terminal_feed_child(terminal, paste, length);
		if (terminal->pvt->screen->bracketed_paste_mode)
			deepinvte_terminal_feed_child(terminal, "\e[201~", -1);
		g_free(paste);
	}
}

static void
deepinvte_terminal_get_mouse_tracking_info (DeepinvteTerminal   *terminal,
				      int            button,
				      long           col,
				      long           row,
				      unsigned char *pb,
...
static void
deepinvte_terminal_send_mouse_button_internal(DeepinvteTerminal *terminal,
					int          button,
					long         x,
					long         y)
{
	...
	deepinvte_terminal_get_mouse_tracking_info (terminal,
					      button, col, row,
					      &cb, &cx, &cy);

	/* Send event direct to the child, this is binary not text data */
	len = g_snprintf(buf, sizeof(buf), _DEEPINVTE_CAP_CSI "M%c%c%c", cb, cx, cy);
	deepinvte_terminal_feed_child_binary(terminal, buf, len);
}

/* Send a mouse button click/release notification. */
static void
deepinvte_terminal_maybe_send_mouse_button(DeepinvteTerminal *terminal,
				     GdkEventButton *event)
{
	...
	deepinvte_terminal_send_mouse_button_internal(terminal,
...
static void
deepinvte_terminal_maybe_send_mouse_drag(DeepinvteTerminal *terminal, GdkEventMotion *event)
{
	unsigned char cb, cx, cy;
	char buf[LINE_MAX];
	gint len;
...
static void
deepinvte_terminal_match_hilite_clear(DeepinvteTerminal *terminal)
{
...
static gboolean
cursor_inside_match (DeepinvteTerminal *terminal, long x, long y)
{
...
static void
deepinvte_terminal_match_hilite_show(DeepinvteTerminal *terminal, long x, long y)
{
...
static void
deepinvte_terminal_match_hilite_hide(DeepinvteTerminal *terminal)
{
	if(terminal->pvt->match != NULL && terminal->pvt->show_match){
		_deepinvte_invalidate_region (terminal,
				terminal->pvt->match_start.col,
...
static void
deepinvte_terminal_match_hilite_update(DeepinvteTerminal *terminal, long x, long y)
{
	int start, end, width, height;
	char *match;
	struct _DeepinvteCharAttributes *attr;
	DeepinvteScreen *screen;
	long delta;
	...
	match = deepinvte_terminal_match_check_internal(terminal,
						  x / width,
						  y / height + delta,
						  &terminal->pvt->match_tag,
						  &start,
...
static void
deepinvte_terminal_match_hilite(DeepinvteTerminal *terminal, long x, long y)
{
	int width, height;
	GtkAllocation allocation;
	...
	deepinvte_terminal_match_hilite_update(terminal, x, y);
}


/* Note that the clipboard has cleared. */
static void
deepinvte_terminal_clear_cb(GtkClipboard *clipboard, gpointer owner)
{
	DeepinvteTerminal *terminal;
	terminal = owner;
	if (terminal->pvt->has_selection) {
		_deepinvte_debug_print(DEEPINVTE_DEBUG_SELECTION, "Lost selection.\n");
		deepinvte_terminal_deselect_all(terminal);
	}
}
...
static void
deepinvte_terminal_copy_cb(GtkClipboard *clipboard, GtkSelectionData *data,
		     guint info, gpointer owner)
{
	DeepinvteTerminal *terminal;
	terminal = owner;
	if (terminal->pvt->selection != NULL) {
		_DEEPINVTE_DEBUG_IF(DEEPINVTE_DEBUG_SELECTION) {
			int i;
...
/**
 * deepinvte_terminal_get_text_range:
 * @terminal: a #DeepinvteTerminal
 ...
 * @is_selected: a #DeepinvteSelectionFunc callback
 * @user_data: (closure): user data to be passed to the callback
 * @attributes: (out) (transfer full) (array) (element-type Deepinvte.CharAttributes): location for storing text attributes
 *
 * Extracts a view of the visible part of the terminal.  If @is_selected is not
 * %NULL, characters will only be read if @is_selected returns %TRUE after being
 * passed the column and row, respectively.  A #DeepinvteCharAttributes structure
 ...
 */
char *
deepinvte_terminal_get_text_range(DeepinvteTerminal *terminal,
...
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
	return deepinvte_terminal_get_text_range_maybe_wrapped(terminal,
							 start_row, start_col,
							 end_row, end_col,
							 TRUE,
							 is_selected,
...
static char *
deepinvte_terminal_get_text_range_maybe_wrapped(DeepinvteTerminal *terminal,
					  glong start_row, glong start_col,
					  glong end_row, glong end_col,
					  gboolean wrap,
					  DeepinvteSelectionFunc is_selected,
					  ...
					  gboolean include_trailing_spaces)
{
	...
	for (row = start_row; row < end_row + 1; row++, col = 0) {
		...
		pcell = NULL;
		if (row_data != NULL) {
			while ((pcell = _deepinvte_row_data_get (row_data, col))) {
				...
				if (!pcell->attr.fragment && is_selected(terminal, col, row, data)) {
					...
					if (pcell->c == 0) {
						...
						_deepinvte_unistr_append_to_string (pcell->c, string);
					...
					if (attributes) {
						deepinvte_g_array_fill(attributes,
								&attr, string->len);
					}
				}
		...
		* off of the line. */
		 ...
		 * not soft-wrapped, append a newline. */
			...
			if (!deepinvte_line_is_wrappable(terminal, row)) {
				string = g_string_append_c(string, '\n');
			}
		}
		...
		if (attributes) {
			deepinvte_g_array_fill (attributes, &attr, string->len);
		}
	}
	/* Sanity check. */
...
static char *
deepinvte_terminal_get_text_maybe_wrapped(DeepinvteTerminal *terminal,
				    gboolean wrap,
				    DeepinvteSelectionFunc is_selected,
				    gpointer data,
				    GArray *attributes,
 ...
 */
char *
deepinvte_terminal_get_text(DeepinvteTerminal *terminal,
		      DeepinvteSelectionFunc is_selected,
		      gpointer user_data,
		      GArray *attributes)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
	return deepinvte_terminal_get_text_maybe_wrapped(terminal,
						   TRUE,
						   is_selected,
						   user_data,
						   attributes,
...
/**
 * deepinvte_terminal_get_text_include_trailing_spaces:
 * @terminal: a #DeepinvteTerminal
 * @is_selected: a #DeepinvteSelectionFunc callback
 * @user_data: (closure): user data to be passed to the callback
 * @attributes: (out) (transfer full) (array) (element-type Deepinvte.CharAttributes): location for storing text attributes
 ...
 * differs from deepinvte_terminal_get_text() in that trailing spaces at the end of
 * lines are included.
 *
 * Returns: (transfer full): a newly allocated text string, or %NULL.
 *
 ...
 */
char *
deepinvte_terminal_get_text_include_trailing_spaces(DeepinvteTerminal *terminal,
					      DeepinvteSelectionFunc is_selected,
					      gpointer user_data,
					      GArray *attributes)
{
...
/**
 * deepinvte_terminal_get_cursor_position:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_get_cursor_position(DeepinvteTerminal *terminal,
...
static void
deepinvte_terminal_copy(DeepinvteTerminal *terminal, GdkAtom board)
{
	GtkClipboard *clipboard;
	static GtkTargetEntry *targets = NULL;
	static gint n_targets = 0;

	clipboard = deepinvte_terminal_clipboard_get(terminal, board);
...
static void
deepinvte_terminal_paste(DeepinvteTerminal *terminal, GdkAtom board)
{
	GtkClipboard *clipboard;
	clipboard = deepinvte_terminal_clipboard_get(terminal, board);
	if (clipboard != NULL) {
		...
		gtk_clipboard_request_text(clipboard,
					   deepinvte_terminal_paste_cb,
					   terminal);
	}
}

static void
deepinvte_terminal_invalidate_selection (DeepinvteTerminal *terminal)
{
	_deepinvte_invalidate_region (terminal,
...
static void
deepinvte_terminal_start_selection(DeepinvteTerminal *terminal, GdkEventButton *event,
			     enum deepinvte_selection_type selection_type)
{
	long delta;
	...
	_deepinvte_terminal_disconnect_pty_read(terminal);
}

static gboolean
_deepinvte_terminal_maybe_end_selection (DeepinvteTerminal *terminal)
{
	if (terminal->pvt->selecting) {
		...
		if (terminal->pvt->has_selection &&
			...
			deepinvte_terminal_copy_primary(terminal);
			deepinvte_terminal_emit_selection_changed(terminal);
		}
		terminal->pvt->selecting = FALSE;

		/* Reconnect to input from the child if we paused it. */
		_deepinvte_terminal_connect_pty_read(terminal);
...
/* Helper */
static void
deepinvte_terminal_extend_selection_expand (DeepinvteTerminal *terminal)
{
	long i, j;
	DeepinvteScreen *screen;
	const DeepinvteRowData *rowdata;
	const DeepinvteCell *cell;
	DeepinvteVisualPosition *sc, *ec;
	...
	rowdata = _deepinvte_terminal_find_row_data(terminal, sc->row);
	if (rowdata != NULL) {
		/* Find the last non-empty character on the first line. */
		for (i = _deepinvte_row_data_length (rowdata); i > 0; i--) {
			cell = _deepinvte_row_data_get (rowdata, i - 1);
	...
	rowdata = _deepinvte_terminal_find_row_data(terminal, ec->row);
	if (rowdata != NULL) {
		...
		if (ec->col >= i) {
			ec->col = MAX(ec->col,
					...
					(long) _deepinvte_row_data_length (rowdata)));
		}
	} else {
	...
	case selection_type_word:
		...
		i = sc->col;
		j = sc->row;
		while (_deepinvte_ring_contains (screen->row_data, j)) {
			/* Get the data for the row we're looking at. */
			rowdata = _deepinvte_ring_index(screen->row_data, j);
			if (rowdata == NULL) {
				break;
			}
			/* Back up. */
			for (i = (j == sc->row) ?
			     ...
			     i > 0;
			     i--) {
				if (deepinvte_same_class(terminal,
						   i - 1,
						   j,
						   i,
						   j)) {
			...
			if (i > 0) {
				...
				if (deepinvte_line_is_wrappable(terminal, j - 1) &&
				    deepinvte_same_class(terminal,
		...
		while (_deepinvte_ring_contains (screen->row_data, j)) {
			...
			if (i < terminal->column_count - 1) {
				...
				if (deepinvte_line_is_wrappable(terminal, j) &&
				    deepinvte_same_class(terminal,
						   terminal->column_count - 1,
						   j,
						   0,
	...
	case selection_type_line:
		...
		j = sc->row;
		while (_deepinvte_ring_contains (screen->row_data, j - 1) &&
		       deepinvte_line_is_wrappable(terminal, j - 1)) {
			j--;
			sc->row = j;
		}
		...
		j = ec->row;
		while (_deepinvte_ring_contains (screen->row_data, j) &&
		       deepinvte_line_is_wrappable(terminal, j)) {
			j++;
			ec->row = j;
		}
		/* Make sure we include all of the last line. */
		ec->col = terminal->column_count;
		if (_deepinvte_ring_contains (screen->row_data, ec->row)) {
			rowdata = _deepinvte_ring_index(screen->row_data, ec->row);
			if (rowdata != NULL) {
				ec->col = MAX(ec->col, (long) _deepinvte_row_data_length (rowdata));
			}
		}
		break;
	}
...
static void
deepinvte_terminal_extend_selection(DeepinvteTerminal *terminal, long x, long y,
			      gboolean always_grow, gboolean force)
{
	...
	DeepinvteVisualPosition old_start, old_end, *sc, *ec, *so, *eo;
	...
	if (terminal->pvt->selection_block_mode) {
		...
		if (always_grow)
			deepinvte_terminal_invalidate_selection (terminal);
	...
	deepinvte_terminal_extend_selection_expand (terminal);
	...
	if (invalidate_selected || !had_selection) {
		_deepinvte_debug_print(DEEPINVTE_DEBUG_SELECTION, "Invalidating selection.");
		deepinvte_terminal_invalidate_selection (terminal);
	}
 ...
 */
void
deepinvte_terminal_select_all (DeepinvteTerminal *terminal)
{
	g_return_if_fail (DEEPINVTE_IS_TERMINAL (terminal));
	...
	terminal->pvt->selection_start.row = _deepinvte_ring_delta (terminal->pvt->screen->row_data);
	terminal->pvt->selection_start.col = 0;
	terminal->pvt->selection_end.row = _deepinvte_ring_next (terminal->pvt->screen->row_data);
	terminal->pvt->selection_end.col = -1;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_SELECTION, "Selecting *all* text.\n");
...
/**
 * deepinvte_terminal_select_none:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
void
deepinvte_terminal_select_none (DeepinvteTerminal *terminal)
{
	g_return_if_fail (DEEPINVTE_IS_TERMINAL (terminal));

	_deepinvte_debug_print(DEEPINVTE_DEBUG_SELECTION, "Clearing selection.\n");
...
/* Autoscroll a bit. */
static gboolean
deepinvte_terminal_autoscroll(DeepinvteTerminal *terminal)
{
	...
	if (terminal->pvt->mouse_last_y < 0) {
		if (terminal->adjustment) {
			...
			deepinvte_terminal_queue_adjustment_value_changed_clamped (terminal, adj);
			extend = TRUE;
		}
		_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Autoscrolling down.\n");
	}
	if (terminal->pvt->mouse_last_y >=
		...
		_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Autoscrolling up.\n");
	}
	if (extend) {
		...
		deepinvte_terminal_extend_selection(terminal, x, y, FALSE, TRUE);
	} else {
		/* Stop autoscrolling. */
...
/* Start autoscroll. */
static void
deepinvte_terminal_start_autoscroll(DeepinvteTerminal *terminal)
{
	if (terminal->pvt->mouse_autoscroll_tag == 0) {
		terminal->pvt->mouse_autoscroll_tag =
			g_timeout_add_full(G_PRIORITY_LOW,
					   666 / terminal->row_count,
					   (GSourceFunc)deepinvte_terminal_autoscroll,
					   terminal,
					   NULL);
	}
}
...
/* Stop autoscroll. */
static void
deepinvte_terminal_stop_autoscroll(DeepinvteTerminal *terminal)
{
	if (terminal->pvt->mouse_autoscroll_tag != 0) {
...
static gboolean
deepinvte_terminal_motion_notify(GtkWidget *widget, GdkEventMotion *event)
{
	DeepinvteTerminal *terminal;
	int width, height;
	long x, y;
	...
	if (terminal->pvt->mouse_last_button) {
		deepinvte_terminal_match_hilite_hide (terminal);
	} else {
		/* Hilite any matches. */
		deepinvte_terminal_match_hilite(terminal, x, y);
		/* Show the cursor. */
	...
	case GDK_MOTION_NOTIFY:
		...
		{
			_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Mousing drag 1.\n");
			deepinvte_terminal_extend_selection(terminal,
						      x, y, FALSE, FALSE);
                                        ...
                                        terminal->pvt->inner_border.top)
			{
				...
				deepinvte_terminal_autoscroll(terminal);
				/* Start a timed autoscroll if we're not doing it
				 * already. */
				deepinvte_terminal_start_autoscroll(terminal);
			}
		...
		if (!handled)
			deepinvte_terminal_maybe_send_mouse_drag(terminal, event);
		break;
	default:
		break;
	}
...
static gint
deepinvte_terminal_button_press(GtkWidget *widget, GdkEventButton *event)
{
	DeepinvteTerminal *terminal;
	...
	case GDK_BUTTON_PRESS:
		...
		case 1:
			...
			if (terminal->pvt->mouse_tracking_mode) {
				...
				if ((terminal->pvt->modifiers & GDK_SHIFT_MASK) &&
				    (terminal->pvt->has_selection ||
				     terminal->pvt->selecting_restart) &&
				    !deepinvte_cell_is_selected(terminal,
							  cellx,
							  celly,
							  NULL)) {
			...
			if (start_selecting) {
				deepinvte_terminal_deselect_all(terminal);
				deepinvte_terminal_start_selection(terminal,
							     event,
							     selection_type_char);
				handled = TRUE;
			}
		 ...
		 * event to the app. */
		if (handled == FALSE) {
			deepinvte_terminal_maybe_send_mouse_button(terminal, event);
			handled = TRUE;
		}
		break;
...
static gint
deepinvte_terminal_button_release(GtkWidget *widget, GdkEventButton *event)
{
	DeepinvteTerminal *terminal;
	gboolean handled = FALSE;
	int x, y;
	...
	deepinvte_terminal_stop_autoscroll(terminal);
	...
	case GDK_BUTTON_RELEASE:
		...
		case 1:
			...
			if ((terminal->pvt->modifiers & GDK_SHIFT_MASK) ||
			    !terminal->pvt->mouse_tracking_mode)
				handled = _deepinvte_terminal_maybe_end_selection (terminal);
			break;
		case 2:
			if ((terminal->pvt->modifiers & GDK_SHIFT_MASK) ||
			    !terminal->pvt->mouse_tracking_mode) {
...
static gboolean
deepinvte_terminal_focus_in(GtkWidget *widget, GdkEventFocus *event)
{
	DeepinvteTerminal *terminal;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Focus in.\n");
...
static gboolean
deepinvte_terminal_focus_out(GtkWidget *widget, GdkEventFocus *event)
{
	DeepinvteTerminal *terminal;
	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Focus out.\n");
	terminal = DEEPINVTE_TERMINAL(widget);
	/* Read the keyboard modifiers, though they're probably garbage. */
	deepinvte_terminal_read_modifiers (terminal, (GdkEvent*) event);
	/* We only have an IM context when we're realized, and there's not much
	...
	if (gtk_widget_get_realized (widget)) {
		_deepinvte_terminal_maybe_end_selection (terminal);
...
static gboolean
deepinvte_terminal_enter(GtkWidget *widget, GdkEventCrossing *event)
{
	gboolean ret = FALSE;
	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Enter.\n");
	if (GTK_WIDGET_CLASS (deepinvte_terminal_parent_class)->enter_notify_event) {
		ret = GTK_WIDGET_CLASS (deepinvte_terminal_parent_class)->enter_notify_event (widget, event);
	}
	if (gtk_widget_get_realized (widget)) {
		DeepinvteTerminal *terminal = DEEPINVTE_TERMINAL (widget);
		/* Hilite any matches. */
		deepinvte_terminal_match_hilite_show(terminal,
...
static gboolean
deepinvte_terminal_leave(GtkWidget *widget, GdkEventCrossing *event)
{
	gboolean ret = FALSE;
	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Leave.\n");
	if (GTK_WIDGET_CLASS (deepinvte_terminal_parent_class)->leave_notify_event) {
		ret = GTK_WIDGET_CLASS (deepinvte_terminal_parent_class)->leave_notify_event (widget, event);
	}
	if (gtk_widget_get_realized (widget)) {
		DeepinvteTerminal *terminal = DEEPINVTE_TERMINAL (widget);
		deepinvte_terminal_match_hilite_hide (terminal);
...
static void
deepinvte_terminal_set_visibility (DeepinvteTerminal *terminal, GdkVisibilityState state)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_MISC, "change visibility: %s -> %s.\n",
...
static gboolean
deepinvte_terminal_visibility_notify(GtkWidget *widget, GdkEventVisibility *event)
{
	DeepinvteTerminal *terminal;
	terminal = DEEPINVTE_TERMINAL(widget);

	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS, "Visibility (%s -> %s).\n",
			visibility_state_str(terminal->pvt->visibility_state),
			visibility_state_str(event->state));
	deepinvte_terminal_set_visibility(terminal, event->state);
...
static void
deepinvte_terminal_apply_metrics(DeepinvteTerminal *terminal,
			   gint width, gint height, gint ascent, gint descent)
{
	...
	if (cresize) {
		deepinvte_terminal_emit_char_size_changed(terminal,
						    terminal->char_width,
...
static void
deepinvte_terminal_ensure_font (DeepinvteTerminal *terminal)
{
	if (terminal->pvt->draw != NULL) {
		/* Load default fonts, if no fonts have been loaded. */
		if (!terminal->pvt->has_fonts) {
			deepinvte_terminal_set_font_full_internal(terminal,
		...
		if (terminal->pvt->fontdirty) {
			...
			_deepinvte_draw_set_text_font (terminal->pvt->draw,
					terminal->pvt->fontdesc,
					terminal->pvt->fontantialias);
			_deepinvte_draw_get_text_metrics (terminal->pvt->draw,
						    &width, &height, &ascent);
			deepinvte_terminal_apply_metrics(terminal,
...
/**
 * deepinvte_terminal_set_font_full:
 * @terminal: a #DeepinvteTerminal
 ...
 * Since: 0.11.11
 * 
 * Deprecated: 0.20: Use deepinvte_terminal_set_font()
 */
void
deepinvte_terminal_set_font_full(DeepinvteTerminal *terminal,
			   const PangoFontDescription *font_desc,
			   DeepinvteTerminalAntiAlias antialias)
{
        deepinvte_terminal_set_font_full_internal(terminal, font_desc, antialias);
}

static void
deepinvte_terminal_set_font_full_internal(DeepinvteTerminal *terminal,
                                    const PangoFontDescription *font_desc,
                                    DeepinvteTerminalAntiAlias antialias)
{
        GObject *object;
	...
	if (gtk_widget_get_realized (&terminal->widget)) {
		deepinvte_terminal_ensure_font (terminal);
	}
 ...
 */
void
deepinvte_terminal_set_font(DeepinvteTerminal *terminal,
		      const PangoFontDescription *font_desc)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
	deepinvte_terminal_set_font_full_internal(terminal, font_desc,
                                            DEEPINVTE_ANTI_ALIAS_USE_DEFAULT);
}

static void
deepinvte_terminal_set_font_from_string_full_internal(DeepinvteTerminal *terminal,
                                                const char *name,
                                                DeepinvteTerminalAntiAlias antialias)
{
 ...
 * passes it to deepinvte_terminal_set_font_full().
 *
 * Since: 0.11.11
 *
 * Deprecated: 0.20: Use deepinvte_terminal_set_font()
 */
void
deepinvte_terminal_set_font_from_string_full(DeepinvteTerminal *terminal, const char *name,
				       DeepinvteTerminalAntiAlias antialias)
{
        deepinvte_terminal_set_font_from_string_full_internal(terminal, name, antialias);
}

/**
 * deepinvte_terminal_set_font_from_string:
 * @terminal: a #DeepinvteTerminal
 * @name: (type utf8): a pango font description in string form
 *
 * A convenience function which converts @name into a #PangoFontDescription and
 * passes it to deepinvte_terminal_set_font().
 */
void
deepinvte_terminal_set_font_from_string(DeepinvteTerminal *terminal, const char *name)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
	g_return_if_fail(name != NULL);
	deepinvte_terminal_set_font_from_string_full_internal(terminal, name,
                                                        DEEPINVTE_ANTI_ALIAS_USE_DEFAULT);
}

/**
 * deepinvte_terminal_get_font:
 * @terminal: a #DeepinvteTerminal
 *
...
deepinvte_terminal_get_font(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
...
static void
deepinvte_terminal_refresh_size(DeepinvteTerminal *terminal)
{
        DeepinvteTerminalPrivate *pvt = terminal->pvt;
	int rows, columns;
        GError *error = NULL;
        ...
        if (deepinvte_pty_get_size(pvt->pty, &rows, &columns, &error)) {
 ...
 */
void
deepinvte_terminal_set_size(DeepinvteTerminal *terminal, glong columns, glong rows)
{
	...
	if (terminal->pvt->pty != NULL) {
		 ...
		 * in case something went awry.
                 */
		if (!deepinvte_pty_set_size(terminal->pvt->pty, rows, columns, &error)) {
			g_warning("%s\n", error->message);
                        g_error_free(error);
		}
		deepinvte_terminal_refresh_size(terminal);
	} else {
		terminal->row_count = rows;
		terminal->column_count = columns;
	}
	if (old_rows != terminal->row_count || old_columns != terminal->column_count) {
		DeepinvteScreen *screen = terminal->pvt->screen;
		glong visible_rows = MIN (old_rows, _deepinvte_ring_length (screen->row_data));
...
/* Redraw the widget. */
static void
deepinvte_terminal_handle_scroll(DeepinvteTerminal *terminal)
{
	long dy, adj;
	DeepinvteScreen *screen;
	...
	if (dy != 0) {
		_deepinvte_debug_print(DEEPINVTE_DEBUG_ADJ,
			    "Scrolling by %ld\n", dy);
		_deepinvte_terminal_scroll_region(terminal, screen->scroll_delta,
					   terminal->row_count, -dy);
		deepinvte_terminal_emit_text_scrolled(terminal, dy);
		_deepinvte_terminal_queue_contents_changed(terminal);
	} else {
		_deepinvte_debug_print(DEEPINVTE_DEBUG_ADJ, "Not scrolling\n");
	}
}

#if GTK_CHECK_VERSION (2, 91, 2)
static void
deepinvte_terminal_set_hadjustment(DeepinvteTerminal *terminal,
...
static void
deepinvte_terminal_set_vadjustment(DeepinvteTerminal *terminal,
                             GtkAdjustment *adjustment)
{
	...
	if (terminal->adjustment != NULL) {
		...
		g_signal_handlers_disconnect_by_func(terminal->adjustment,
						     deepinvte_terminal_handle_scroll,
						     terminal);
	...
	g_signal_connect_swapped(terminal->adjustment,
				 "value-changed",
				 G_CALLBACK(deepinvte_terminal_handle_scroll),
				 terminal);
}
...
#if !GTK_CHECK_VERSION (2, 91, 2)
static void
deepinvte_terminal_set_scroll_adjustments(GtkWidget *widget,
                                   GtkAdjustment *hadjustment G_GNUC_UNUSED,
                                   GtkAdjustment *vadjustment)
{
        deepinvte_terminal_set_vadjustment(DEEPINVTE_TERMINAL(widget), vadjustment);
}
#endif /* GTK 2.x */

/**
 * deepinvte_terminal_set_emulation:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_emulation(DeepinvteTerminal *terminal, const char *emulation)
{
        DeepinvteTerminalPrivate *pvt;
        GObject *object;
	int columns, rows;
	...
	if (emulation == NULL) {
		emulation = deepinvte_terminal_get_default_emulation(terminal);
	}
	terminal->pvt->emulation = g_intern_string(emulation);
	_deepinvte_debug_print(DEEPINVTE_DEBUG_MISC,
			"Setting emulation to `%s'...\n", emulation);
	/* Find and read the right termcap file. */
	deepinvte_terminal_set_termcap(terminal, NULL, FALSE);

	/* Create a table to hold the control sequences. */
	if (terminal->pvt->matcher != NULL) {
		_deepinvte_matcher_free(terminal->pvt->matcher);
	}
	terminal->pvt->matcher = _deepinvte_matcher_new(emulation, terminal->pvt->termcap);

	if (terminal->pvt->termcap != NULL) {
		/* Read emulation flags. */
		terminal->pvt->flags.am = _deepinvte_termcap_find_boolean(terminal->pvt->termcap,
								    terminal->pvt->emulation,
								    "am");
		terminal->pvt->flags.bw = _deepinvte_termcap_find_boolean(terminal->pvt->termcap,
								    terminal->pvt->emulation,
								    "bw");
		terminal->pvt->flags.LP = _deepinvte_termcap_find_boolean(terminal->pvt->termcap,
								    terminal->pvt->emulation,
								    "LP");
		terminal->pvt->flags.ul = _deepinvte_termcap_find_boolean(terminal->pvt->termcap,
								    terminal->pvt->emulation,
								    "ul");
		terminal->pvt->flags.xn = _deepinvte_termcap_find_boolean(terminal->pvt->termcap,
								    terminal->pvt->emulation,
								    "xn");

		/* Resize to the given default. */
		columns = _deepinvte_termcap_find_numeric(terminal->pvt->termcap,
						    terminal->pvt->emulation,
						    "co");
		if (columns <= 0) {
			columns = DEEPINVTE_COLUMNS;
		}
		terminal->pvt->default_column_count = columns;

		rows = _deepinvte_termcap_find_numeric(terminal->pvt->termcap,
						 terminal->pvt->emulation,
						 "li");
		if (rows <= 0 ) {
			rows = DEEPINVTE_ROWS;
	...
	deepinvte_terminal_emit_emulation_changed(terminal);
...
/**
 * deepinvte_terminal_get_default_emulation:
 * @terminal: a #DeepinvteTerminal
 *
 * Queries the terminal for its default emulation, which is attempted if the
 * terminal type passed to deepinvte_terminal_set_emulation() is %NULL.
 *
 ...
 */
const char *
deepinvte_terminal_get_default_emulation(DeepinvteTerminal *terminal)
{
	return g_intern_static_string(DEEPINVTE_DEFAULT_EMULATION);
}

/**
 * deepinvte_terminal_get_emulation:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
const char *
deepinvte_terminal_get_emulation(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
...
void
_deepinvte_terminal_inline_error_message(DeepinvteTerminal *terminal, const char *format, ...)
{
	va_list ap;
	char *str;
	...
	deepinvte_terminal_feed (terminal, "*** DEEPINVTE ***: ", 13);
	deepinvte_terminal_feed (terminal, str, -1);
	deepinvte_terminal_feed (terminal, "\r\n", 2);
	g_free (str);
}

/* Set the path to the termcap file we read, and read it in. */
static void
deepinvte_terminal_set_termcap(DeepinvteTerminal *terminal, const char *path,
			 gboolean reset)
{
        GObject *object = G_OBJECT(terminal);
	struct stat st;
	...
	if (path == NULL) {
		wpath = g_build_filename(TERMCAPDIR,
					 terminal->pvt->emulation ?
					 terminal->pvt->emulation :
					 deepinvte_terminal_get_default_emulation(terminal),
					 NULL);
	...
	_deepinvte_debug_print(DEEPINVTE_DEBUG_MISC, "Loading termcap `%s'...",
			terminal->pvt->termcap_path);
	if (terminal->pvt->termcap != NULL) {
		_deepinvte_termcap_free(terminal->pvt->termcap);
	}
	terminal->pvt->termcap = _deepinvte_termcap_new(terminal->pvt->termcap_path);
	_deepinvte_debug_print(DEEPINVTE_DEBUG_MISC, "\n");
	if (terminal->pvt->termcap == NULL) {
		_deepinvte_terminal_inline_error_message(terminal,
				"Failed to load terminal capabilities from '%s'",
				terminal->pvt->termcap_path);
	}
	if (reset) {
		deepinvte_terminal_set_emulation(terminal, terminal->pvt->emulation);
	}
...
static void
_deepinvte_terminal_codeset_changed_cb(struct _deepinvte_iso2022_state *state, gpointer p)
{
	deepinvte_terminal_set_encoding(p, _deepinvte_iso2022_state_get_codeset(state));
}
...
static void
deepinvte_terminal_init(DeepinvteTerminal *terminal)
{
	DeepinvteTerminalPrivate *pvt;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_LIFECYCLE, "deepinvte_terminal_init()\n");

	/* Initialize private data. */
	pvt = terminal->pvt = G_TYPE_INSTANCE_GET_PRIVATE (terminal, DEEPINVTE_TYPE_TERMINAL, DeepinvteTerminalPrivate);
	...
	/* Set an adjustment for the application to use to control scrolling. */
        ...
        deepinvte_terminal_set_hadjustment(terminal, NULL);
#endif

	deepinvte_terminal_set_vadjustment(terminal, NULL);
	...
	_deepinvte_ring_init (pvt->alternate_screen.row_data, terminal->row_count);
	...
	_deepinvte_ring_init (pvt->normal_screen.row_data,  DEEPINVTE_SCROLLBACK_INIT);
	...
	pvt->iso2022 = _deepinvte_iso2022_state_new(pvt->encoding,
					      &_deepinvte_terminal_codeset_changed_cb,
					      terminal);
	pvt->incoming = NULL;
	pvt->pending = g_array_new(FALSE, TRUE, sizeof(gunichar));
	pvt->max_input_bytes = DEEPINVTE_MAX_INPUT_READ;
	pvt->cursor_blink_tag = 0;
	pvt->outgoing = _deepinvte_buffer_new();
	pvt->outgoing_conv = DEEPINVTE_INVALID_CONV;
	pvt->conv_buffer = _deepinvte_buffer_new();
	deepinvte_terminal_set_encoding(terminal, NULL);
	g_assert(terminal->pvt->encoding != NULL);

	/* Load the termcap data and set up the emulation. */
	pvt->keypad_mode = DEEPINVTE_KEYMODE_NORMAL;
	pvt->cursor_mode = DEEPINVTE_KEYMODE_NORMAL;
	pvt->dec_saved = g_hash_table_new(NULL, NULL);
	pvt->default_column_count = DEEPINVTE_COLUMNS;
	pvt->default_row_count = DEEPINVTE_ROWS;

	/* Setting the terminal type and size requires the PTY master to
	 * be set up properly first. */
        pvt->pty = NULL;
	deepinvte_terminal_set_emulation(terminal, NULL);
	deepinvte_terminal_set_size(terminal,
	...
	pvt->scroll_on_keystroke = TRUE;
        pvt->scrollback_lines = -1; /* force update in deepinvte_terminal_set_scrollback_lines */
	deepinvte_terminal_set_scrollback_lines(terminal, DEEPINVTE_SCROLLBACK_INIT);

	/* Selection info. */
	deepinvte_terminal_set_word_chars(terminal, NULL);

	/* Miscellaneous options. */
	deepinvte_terminal_set_backspace_binding(terminal, DEEPINVTE_ERASE_AUTO);
	deepinvte_terminal_set_delete_binding(terminal, DEEPINVTE_ERASE_AUTO);
	pvt->meta_sends_escape = TRUE;
	pvt->audible_bell = TRUE;
	pvt->bell_margin = 10;
	pvt->allow_bold = TRUE;
	pvt->nrc_mode = TRUE;
	deepinvte_terminal_set_default_tabstops(terminal);

	/* Cursor shape. */
	pvt->cursor_shape = DEEPINVTE_CURSOR_SHAPE_BLOCK;
	...
	pvt->cursor_blink_timeout = 500;
        ...
        pvt->cursor_blink_mode = DEEPINVTE_CURSOR_BLINK_SYSTEM;

	/* Matching data. */
        pvt->match_regex_mode = DEEPINVTE_REGEX_UNDECIDED;
	pvt->match_regexes = g_array_new(FALSE, TRUE,
					 sizeof(struct deepinvte_match_regex));
	deepinvte_terminal_match_hilite_clear(terminal);

	/* Rendering data.  Try everything. */
	pvt->draw = _deepinvte_draw_new(&terminal->widget);

	/* The font description. */
	pvt->fontantialias = DEEPINVTE_ANTI_ALIAS_USE_DEFAULT;
	gtk_widget_ensure_style(&terminal->widget);
	...
	pvt->bg_saturation = 0.4 * DEEPINVTE_SATURATION_MAX;
	...
	g_signal_connect(terminal, "hierarchy-changed",
			 G_CALLBACK(deepinvte_terminal_hierarchy_changed),
			 NULL);
...
#if GTK_CHECK_VERSION (2, 99, 0)
{
        ...
        gtk_style_context_add_provider (context,
                                        DEEPINVTE_TERMINAL_GET_CLASS (terminal)->priv->style_provider,
...
static void
deepinvte_terminal_get_preferred_width(GtkWidget *widget,
				 int       *minimum_width,
				 int       *natural_width)
{
	DeepinvteTerminal *terminal;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_LIFECYCLE, "deepinvte_terminal_get_preferred_width()\n");
	...
	*natural_width += terminal->pvt->inner_border.left +
	...
	_deepinvte_debug_print(DEEPINVTE_DEBUG_WIDGET_SIZE,
...
static void
deepinvte_terminal_get_preferred_height(GtkWidget *widget,
				  int       *minimum_height,
				  int       *natural_height)
{
	DeepinvteTerminal *terminal;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_LIFECYCLE, "deepinvte_terminal_get_preferred_height()\n");
...
#else /* GTK+ 2.x */
static void
deepinvte_terminal_size_request(GtkWidget *widget, GtkRequisition *requisition)
{
	DeepinvteTerminal *terminal;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_LIFECYCLE, "deepinvte_terminal_size_request()\n");
...
static void
deepinvte_terminal_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
	DeepinvteTerminal *terminal;
	glong width, height;
	...
	_deepinvte_debug_print(DEEPINVTE_DEBUG_LIFECYCLE,
			"deepinvte_terminal_size_allocate()\n");
			...
			|| update_scrollback)
	{
		...
		deepinvte_terminal_set_size(terminal, width, height);
		...
		screen->cursor_current.row = CLAMP (screen->cursor_current.row,
				_deepinvte_ring_delta (screen->row_data),
				MAX (_deepinvte_ring_delta (screen->row_data),
					_deepinvte_ring_next (screen->row_data) - 1));
...
static void
root_pixmap_changed_cb(DeepinvteBg *bg, DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print (DEEPINVTE_DEBUG_EVENTS, "Root pixmap changed.\n");
	if (terminal->pvt->bg_transparent) {
		deepinvte_terminal_queue_background_update(terminal);
	}
}
...
static void
deepinvte_terminal_unrealize(GtkWidget *widget)
{
	GdkWindow *window;
	DeepinvteTerminal *terminal;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_LIFECYCLE, "deepinvte_terminal_unrealize()\n");
	...
	if (terminal->pvt->root_pixmap_changed_tag != 0) {
		DeepinvteBg       *bg;
		bg = deepinvte_bg_get_for_screen(gtk_widget_get_screen(widget));
	...
	if (terminal->pvt->im_context != NULL) {
	        g_signal_handlers_disconnect_by_func (terminal->pvt->im_context,
						      ...
						      terminal);
		deepinvte_terminal_im_reset(terminal);
	...
	if (terminal->pvt->draw != NULL) {
		_deepinvte_draw_free(terminal->pvt->draw);
...
static void
deepinvte_terminal_sync_settings (GtkSettings *settings,
                            GParamSpec *pspec,
                            DeepinvteTerminal *terminal)
{
        DeepinvteTerminalPrivate *pvt = terminal->pvt;
        ...
        if (pvt->cursor_blink_mode == DEEPINVTE_CURSOR_BLINK_SYSTEM)
                deepinvte_terminal_set_cursor_blinks_internal(terminal, blink);
}

static void
deepinvte_terminal_screen_changed (GtkWidget *widget,
                             GdkScreen *previous_screen)
{
        ...
        if (GTK_WIDGET_CLASS (deepinvte_terminal_parent_class)->screen_changed) {
                GTK_WIDGET_CLASS (deepinvte_terminal_parent_class)->screen_changed (widget, previous_screen);
        }
        ...
        deepinvte_terminal_sync_settings (settings, NULL, terminal);
        g_signal_connect (settings, "notify::gtk-cursor-blink",
                          G_CALLBACK (deepinvte_terminal_sync_settings), widget);
...
static void
deepinvte_terminal_finalize(GObject *object)
{
    	GtkWidget *widget = GTK_WIDGET (object);
    	DeepinvteTerminal *terminal = DEEPINVTE_TERMINAL (object);
	GtkWidget *toplevel;
	GtkClipboard *clipboard;
        GtkSettings *settings;
	struct deepinvte_match_regex *regex;
	...
	_deepinvte_debug_print(DEEPINVTE_DEBUG_LIFECYCLE, "deepinvte_terminal_finalize()\n");
	...
	/* The NLS maps. */
	_deepinvte_iso2022_state_free(terminal->pvt->iso2022);
	...
	terminal->pvt->fontantialias = DEEPINVTE_ANTI_ALIAS_USE_DEFAULT;
	...
	if (terminal->pvt->selection != NULL) {
		clipboard = deepinvte_terminal_clipboard_get(terminal,
	...
	_deepinvte_ring_fini(terminal->pvt->normal_screen.row_data);
	_deepinvte_ring_fini(terminal->pvt->alternate_screen.row_data);
	...
	if (terminal->pvt->outgoing_conv != DEEPINVTE_INVALID_CONV) {
		_deepinvte_conv_close(terminal->pvt->outgoing_conv);
		terminal->pvt->outgoing_conv = DEEPINVTE_INVALID_CONV;
	}
	...
	deepinvte_terminal_stop_processing (terminal);

	/* Discard any pending data. */
	_deepinvte_incoming_chunks_release (terminal->pvt->incoming);
	_deepinvte_buffer_free(terminal->pvt->outgoing);
	g_array_free(terminal->pvt->pending, TRUE);
	_deepinvte_buffer_free(terminal->pvt->conv_buffer);
	...
	_deepinvte_terminal_disconnect_pty_read(terminal);
	_deepinvte_terminal_disconnect_pty_write(terminal);
	if (terminal->pvt->pty_channel != NULL) {
		g_io_channel_unref (terminal->pvt->pty_channel);
	}
	if (terminal->pvt->pty != NULL) {
                deepinvte_pty_close(terminal->pvt->pty);
	...
	G_OBJECT_CLASS(deepinvte_terminal_parent_class)->finalize(object);
}

/* Handle realizing the widget.  Most of this is copy-paste from GGAD. */
static void
deepinvte_terminal_realize(GtkWidget *widget)
{
	GdkWindow *window;
	DeepinvteTerminal *terminal;
	...
	_deepinvte_debug_print(DEEPINVTE_DEBUG_LIFECYCLE, "deepinvte_terminal_realize()\n");
	...
	if (terminal->pvt->draw == NULL) {
		terminal->pvt->draw = _deepinvte_draw_new(&terminal->widget);
	}

	/* Create the stock cursors. */
	terminal->pvt->mouse_cursor_visible = TRUE;
	terminal->pvt->mouse_default_cursor =
		deepinvte_terminal_cursor_new(terminal, DEEPINVTE_DEFAULT_CURSOR);
	terminal->pvt->mouse_mousing_cursor =
		deepinvte_terminal_cursor_new(terminal, DEEPINVTE_MOUSING_CURSOR);
	...
	_DEEPINVTE_DEBUG_IF (DEEPINVTE_DEBUG_UPDATES) gdk_window_set_debug_updates (TRUE);
	...
	if (!terminal->pvt->palette_initialized) {
		deepinvte_terminal_set_default_colors(terminal);
	}
	...
	g_signal_connect(terminal->pvt->im_context, "commit",
			 G_CALLBACK(deepinvte_terminal_im_commit), terminal);
	g_signal_connect(terminal->pvt->im_context, "preedit-start",
			 G_CALLBACK(deepinvte_terminal_im_preedit_start),
			 terminal);
	g_signal_connect(terminal->pvt->im_context, "preedit-changed",
			 G_CALLBACK(deepinvte_terminal_im_preedit_changed),
			 terminal);
	g_signal_connect(terminal->pvt->im_context, "preedit-end",
			 G_CALLBACK(deepinvte_terminal_im_preedit_end),
			 terminal);
	...
	deepinvte_terminal_background_update(terminal);
}
...
static void
deepinvte_terminal_determine_colors_internal(DeepinvteTerminal *terminal,
				       const DeepinvteCell *cell,
				       gboolean selected,
				       gboolean cursor,
				       guint *pfore, guint *pback)
{
	...
	if (G_UNLIKELY (terminal->pvt->screen->reverse_mode)) {
		if (fore == DEEPINVTE_DEF_FG)
			fore = DEEPINVTE_DEF_BG;
		if (back == DEEPINVTE_DEF_BG)
			back = DEEPINVTE_DEF_FG;
	}

	/* Handle bold by using set bold color or brightening */
	if (cell->attr.bold) {
		if (fore == DEEPINVTE_DEF_FG)
			fore = DEEPINVTE_BOLD_FG;
		else if (fore < DEEPINVTE_LEGACY_COLOR_SET_SIZE) {
			fore += DEEPINVTE_COLOR_BRIGHT_OFFSET;
		}
	}
	...
	if (cell->attr.standout) {
		if (back < DEEPINVTE_LEGACY_COLOR_SET_SIZE)
			back += DEEPINVTE_COLOR_BRIGHT_OFFSET;
	}
...
static inline void
deepinvte_terminal_determine_colors (DeepinvteTerminal *terminal,
			       const DeepinvteCell *cell,
			       gboolean highlight,
			       guint *fore, guint *back)
{
	return deepinvte_terminal_determine_colors_internal (terminal, cell,
						       highlight, FALSE,
						       fore, back);
}

static inline void
deepinvte_terminal_determine_cursor_colors (DeepinvteTerminal *terminal,
				      const DeepinvteCell *cell,
				      gboolean highlight,
				      guint *fore, guint *back)
{
...
static gboolean
deepinvte_unichar_is_local_graphic(deepinvteunistr c)
{
...
static gboolean
deepinvte_terminal_unichar_is_local_graphic(DeepinvteTerminal *terminal, deepinvteunistr c, gboolean bold)
{
	return deepinvte_unichar_is_local_graphic (c) &&
		!_deepinvte_draw_has_char (terminal->pvt->draw, c, bold);
}

static void
deepinvte_terminal_fill_rectangle(DeepinvteTerminal *terminal,
...
static void
deepinvte_terminal_draw_line(DeepinvteTerminal *terminal,
		       const PangoColor *color,
		       gint x,
		       gint y,
		       gint xp,
		       gint yp)
{
	deepinvte_terminal_fill_rectangle(terminal, color,
				    x, y,
				    MAX(DEEPINVTE_LINE_WIDTH, xp - x + 1), MAX(DEEPINVTE_LINE_WIDTH, yp - y + 1));
}

static void
deepinvte_terminal_draw_rectangle(DeepinvteTerminal *terminal,
			    const PangoColor *color,
			    gint x,
			    gint y,
			    gint width,
...
{
	_deepinvte_draw_start(terminal->pvt->draw);
	_deepinvte_draw_draw_rectangle(terminal->pvt->draw,
				 x + terminal->pvt->inner_border.left,
                                 y + terminal->pvt->inner_border.top,
				 width, height,
				 color, DEEPINVTE_DRAW_OPAQUE);
...
static void
deepinvte_terminal_draw_point(DeepinvteTerminal *terminal,
			const PangoColor *color,
			gint x,
			gint y)
{
	deepinvte_terminal_fill_rectangle(terminal, color, x, y, 1, 1);
}

/* Draw the graphic representation of a line-drawing or special graphics
 * character. */
static gboolean
deepinvte_terminal_draw_graphic(DeepinvteTerminal *terminal, deepinvteunistr c,
			  ...
			  gboolean bold)
{
	...
	if ((back != DEEPINVTE_DEF_BG) || draw_default_bg) {
		deepinvte_terminal_fill_rectangle(terminal,
					    &terminal->pvt->palette[back],
					    x, y,
					    column_width * columns, row_height);
	}

	if (_deepinvte_draw_char(terminal->pvt->draw, &request,
			   &terminal->pvt->palette[fore], DEEPINVTE_DRAW_OPAQUE, bold)) {
	...
	case 124:
		...
		ybottom--;
		/* != */
		deepinvte_terminal_draw_line(terminal,
	...
	case 0x00b0: /* f */
		/* litle circle */
		deepinvte_terminal_draw_point(terminal,
					&terminal->pvt->palette[fore],
					xcenter - 1, ycenter);
		deepinvte_terminal_draw_point(terminal,
					&terminal->pvt->palette[fore],
	...
	case 0x2510: /* k */
		deepinvte_terminal_fill_rectangle(terminal,
					    ...
					    x,
					    ycenter,
					    xcenter - x + DEEPINVTE_LINE_WIDTH,
					    DEEPINVTE_LINE_WIDTH);
		deepinvte_terminal_fill_rectangle(terminal,
					    &terminal->pvt->palette[fore],
					    xcenter,
	...
	case 0x2513:
		deepinvte_terminal_fill_rectangle(terminal,
					    ...
					    x,
					    ycenter,
					    xcenter - x + DEEPINVTE_LINE_WIDTH * 2,
					    DEEPINVTE_LINE_WIDTH * 2);
		deepinvte_terminal_fill_rectangle(terminal,
					    &terminal->pvt->palette[fore],
					    xcenter,
	...
	case 0x2514: /* m */
		...
		deepinvte_terminal_fill_rectangle(terminal,
					    ...
					    y,
					    DEEPINVTE_LINE_WIDTH,
					    ycenter - y + DEEPINVTE_LINE_WIDTH);
		break;
	case 0x2517:
		deepinvte_terminal_fill_rectangle(terminal,
					    &terminal->pvt->palette[fore],
		...
		deepinvte_terminal_fill_rectangle(terminal,
					    ...
					    y,
					    DEEPINVTE_LINE_WIDTH * 2,
					    ycenter - y + DEEPINVTE_LINE_WIDTH * 2);
		break;
	case 0x2518: /* j */
		deepinvte_terminal_fill_rectangle(terminal,
					    &terminal->pvt->palette[fore],
...
static void
deepinvte_terminal_draw_cells(DeepinvteTerminal *terminal,
			struct _deepinvte_draw_text_request *items, gssize n,
			...
			gint column_width, gint row_height)
{
	...
	g_assert(n > 0);
	_DEEPINVTE_DEBUG_IF(DEEPINVTE_DEBUG_CELLS) {
	...
	defbg = &terminal->pvt->palette[DEEPINVTE_DEF_BG];
	...
	do {
		...
		if (clear && (draw_default_bg || bg != defbg)) {
			_deepinvte_draw_fill_rectangle(terminal->pvt->draw,
					...
					row_height,
					bg, DEEPINVTE_DRAW_OPAQUE);
		}
	} while (i < n);
	_deepinvte_draw_text(terminal->pvt->draw,
			items, n,
			fg, DEEPINVTE_DRAW_OPAQUE, bold);
	for (i = 0; i < n; i++) {
	...
	if (underline | strikethrough | hilite | boxed) {
		...
		do {
			...
			if (boxed) {
				deepinvte_terminal_draw_rectangle(terminal,
						&terminal->pvt->palette[fore],
						x, y,
...
static guint
_deepinvte_terminal_map_pango_color(DeepinvteTerminal *terminal, PangoColor *color)
{
 ...
 * the right thing. */
static void
_deepinvte_terminal_fudge_pango_colors(DeepinvteTerminal *terminal, GSList *attributes,
				 DeepinvteCell *cells, gssize n)
{
	int i, sumlen = 0;
...
static void
_deepinvte_terminal_apply_pango_attr(DeepinvteTerminal *terminal, PangoAttribute *attr,
			       DeepinvteCell *cells, guint n_cells)
{
	guint i, ival;
	...
	case PANGO_ATTR_BACKGROUND:
		...
		ival = _deepinvte_terminal_map_pango_color(terminal,
						     &attrcolor->color);
...
static void
_deepinvte_terminal_pango_attribute_destroy(gpointer attr, gpointer data)
{
	pango_attribute_destroy(attr);
}
static void
_deepinvte_terminal_translate_pango_cells(DeepinvteTerminal *terminal, PangoAttrList *attrs,
				    DeepinvteCell *cells, guint n_cells)
{
	PangoAttribute *attr;
		...
		do {
			...
			if (list != NULL) {
				for (listiter = list;
					...
					attr = listiter->data;
					_deepinvte_terminal_apply_pango_attr(terminal,
								       attr,
								       cells,
								       n_cells);
				}
				attr = list->data;
				_deepinvte_terminal_fudge_pango_colors(terminal,
								 list,
								 cells +
								 attr->start_index,
								 attr->end_index -
				...
				g_slist_foreach(list,
						_deepinvte_terminal_pango_attribute_destroy,
						NULL);
				g_slist_free(list);
			}
...
static void
deepinvte_terminal_draw_cells_with_attributes(DeepinvteTerminal *terminal,
					struct _deepinvte_draw_text_request *items,
					gssize n,
					PangoAttrList *attrs,
					gboolean draw_default_bg,
					gint column_width, gint height)
{
	int i, j, cell_count;
	DeepinvteCell *cells;
	char scratch_buf[DEEPINVTE_UTF8_BPC];
	guint fore, back;

	/* Note: since this function is only called with the pre-edit text,
	 * all the items contain gunichar only, not deepinvteunistr. */

	for (i = 0, cell_count = 0; i < n; i++) {
		cell_count += g_unichar_to_utf8(items[i].c, scratch_buf);
	}
	cells = g_new(DeepinvteCell, cell_count);
	_deepinvte_terminal_translate_pango_cells(terminal, attrs, cells, cell_count);
	for (i = 0, j = 0; i < n; i++) {
		deepinvte_terminal_determine_colors(terminal, &cells[j], FALSE, &fore, &back);
		deepinvte_terminal_draw_cells(terminal, items + i, 1,
					fore,
					back,
...
static void
deepinvte_terminal_draw_rows(DeepinvteTerminal *terminal,
		      DeepinvteScreen *screen,
		      ...
		      gint column_width, gint row_height)
{
	struct _deepinvte_draw_text_request items[4*DEEPINVTE_DRAW_MAX_LENGTH];
	...
	do {
		...
		i = start_column;
		if (row_data != NULL) {
			cell = _deepinvte_row_data_get (row_data, i);
			if (cell != NULL) {
			...
			do {
				...
				selected = deepinvte_cell_is_selected(terminal, i, row, NULL);
				deepinvte_terminal_determine_colors(terminal, cell, selected, &fore, &back);
				...
				while (j < end_column){
					/* Retrieve the cell. */
					cell = _deepinvte_row_data_get (row_data, j);
					 ...
					 * in this chunk. */
					selected = deepinvte_cell_is_selected(terminal, j, row, NULL);
					deepinvte_terminal_determine_colors(terminal, cell, selected, &nfore, &nback);
					if (nback != back) {
						break;
					}
					bold = cell && cell->attr.bold;
				...
				if (back != DEEPINVTE_DEF_BG) {
					_deepinvte_draw_fill_rectangle (
							terminal->pvt->draw,
							x + i * column_width,
							y,
							(j - i) * column_width + bold,
							row_height,
							&terminal->pvt->palette[back], DEEPINVTE_DRAW_OPAQUE);
				}
			...
			do {
				...
				j = i + 1;
				while (j < end_column){
					nselected = deepinvte_cell_is_selected(terminal, j, row, NULL);
					if (nselected != selected) {
						break;
					}
					j++;
				}
				deepinvte_terminal_determine_colors(terminal, NULL, selected, &fore, &back);
				if (back != DEEPINVTE_DEF_BG) {
					_deepinvte_draw_fill_rectangle (terminal->pvt->draw,
								  x + i *column_width,
								  y,
	...
	do {
		...
		do {
			...
			bold = cell->attr.bold;
			if (terminal->pvt->show_match) {
				hilite = deepinvte_cell_is_between(i, row,
						terminal->pvt->match_start.col,
						terminal->pvt->match_start.row,
						terminal->pvt->match_end.col,
						terminal->pvt->match_end.row,
			...
			if (deepinvte_terminal_unichar_is_local_graphic(terminal, cell->c, cell->attr.bold)) {
				if (deepinvte_terminal_draw_graphic(terminal,
							items[0].c,
							fore, back,
							FALSE,
							items[0].x,
			...
			do {
				while (j < end_column &&
					...
					nhilite = FALSE;
					if (terminal->pvt->show_match) {
						nhilite = deepinvte_cell_is_between(j, row,
								terminal->pvt->match_start.col,
								terminal->pvt->match_start.row,
								terminal->pvt->match_end.col,
								terminal->pvt->match_end.row,
...
fg_draw:
			/* Draw the cells. */
			deepinvte_terminal_draw_cells(terminal,
					items,
					item_count,
...
static void
deepinvte_terminal_expand_region (DeepinvteTerminal *terminal, GdkRegion *region, const GdkRectangle *area)
{
	DeepinvteScreen *screen;
	int width, height;
	...
	_deepinvte_debug_print (DEEPINVTE_DEBUG_UPDATES,
			"deepinvte_terminal_expand_region"
			"	(%d,%d)x(%d,%d) pixels,"
			" (%d,%d)x(%d,%d) cells"
			" [(%d,%d)x(%d,%d) pixels]\n",
...
static void
deepinvte_terminal_paint_area (DeepinvteTerminal *terminal, const GdkRectangle *area)
{
	DeepinvteScreen *screen;
	 ...
	 * need to draw. */
	delta = screen->scroll_delta;
	deepinvte_terminal_draw_rows(terminal,
			      screen,
...
static void
deepinvte_terminal_paint_cursor(DeepinvteTerminal *terminal)
{
	DeepinvteScreen *screen;
	const DeepinvteCell *cell;
	struct _deepinvte_draw_text_request item;
	int row, drow, col;
	...
	cell = deepinvte_terminal_find_charcell(terminal, col, drow);
	...
	if (cell && cell->c != 0) {
		gint cw = _deepinvte_draw_get_char_width (terminal->pvt->draw,
				cell->c, cell->attr.columns, cell->attr.bold);
		cursor_width = MAX(cursor_width, cw);
	}

	selected = deepinvte_cell_is_selected(terminal, col, drow, NULL);

	deepinvte_terminal_determine_cursor_colors(terminal, cell, selected, &fore, &back);
	...
	switch (terminal->pvt->cursor_shape) {
		...
		case DEEPINVTE_CURSOR_SHAPE_IBEAM: {
                        int stem_width;

                        stem_width = (int) (((float) height) * terminal->pvt->cursor_aspect_ratio + 0.5);
                        stem_width = CLAMP (stem_width, DEEPINVTE_LINE_WIDTH, cursor_width);
		 	
			deepinvte_terminal_fill_rectangle(terminal, &terminal->pvt->palette[back],
						     x, y, stem_width, height);
			break;
                }

		case DEEPINVTE_CURSOR_SHAPE_UNDERLINE: {
                        int line_height;

                        line_height = (int) (((float) width) * terminal->pvt->cursor_aspect_ratio + 0.5);
                        line_height = CLAMP (line_height, DEEPINVTE_LINE_WIDTH, height);
		...
		case DEEPINVTE_CURSOR_SHAPE_BLOCK:
			...
			if (focus) {
				...
				if (!deepinvte_terminal_unichar_is_local_graphic(terminal, item.c, cell ? cell->attr.bold : FALSE) ||
				    !deepinvte_terminal_draw_graphic(terminal,
							       item.c,
							       fore, back,
							       TRUE,
							       item.x,
					...
					if (cell && terminal->pvt->show_match) {
						hilite = deepinvte_cell_is_between(col, row,
								terminal->pvt->match_start.col,
								terminal->pvt->match_start.row,
								terminal->pvt->match_end.col,
								terminal->pvt->match_end.row,
				...
				deepinvte_terminal_draw_rectangle (terminal,
							    &terminal->pvt->palette[back],
							     x - DEEPINVTE_LINE_WIDTH,
							     y - DEEPINVTE_LINE_WIDTH,
							     cursor_width + 2*DEEPINVTE_LINE_WIDTH,
							     height + 2*DEEPINVTE_LINE_WIDTH);
			}
...
static void
deepinvte_terminal_paint_im_preedit_string(DeepinvteTerminal *terminal)
{
	DeepinvteScreen *screen;
	...
	columns = deepinvte_terminal_preedit_width(terminal, FALSE);
	len = deepinvte_terminal_preedit_length(terminal, FALSE);
	...
	if (len > 0) {
		...
		items = g_new(struct _deepinvte_draw_text_request, len);
		for (i = columns = 0; i < len; i++) {
			items[i].c = g_utf8_get_char(preedit);
			items[i].columns = _deepinvte_iso2022_unichar_width(terminal->pvt->iso2022,
								      items[i].c);
		...
		_deepinvte_draw_clear(terminal->pvt->draw,
				col * width + terminal->pvt->inner_border.left,
				row * height + terminal->pvt->inner_border.top,
				width * columns,
				height);
		...
		deepinvte_terminal_draw_cells_with_attributes(terminal,
							items, len,
...
/* Draw the widget. */
static void
deepinvte_terminal_paint(GtkWidget *widget, GdkRegion *region)
{
	DeepinvteTerminal *terminal;
	GtkAllocation allocation;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_LIFECYCLE, "deepinvte_terminal_paint()\n");
	_deepinvte_debug_print(DEEPINVTE_DEBUG_WORK, "=");
	...
	if (terminal->pvt->bg_transparent) {
		...
		_deepinvte_draw_set_background_scroll(terminal->pvt->draw, x, y);
	} else {
		if (terminal->pvt->scroll_background) {
			_deepinvte_draw_set_background_scroll(terminal->pvt->draw,
							0,
							terminal->pvt->screen->scroll_delta *
							terminal->char_height);
		} else {
			_deepinvte_draw_set_background_scroll(terminal->pvt->draw, 0, 0);
		}
	}

	_DEEPINVTE_DEBUG_IF (DEEPINVTE_DEBUG_UPDATES) {
		DeepinvteRegionRectangle clip;
		gdk_region_get_clipbox (region, &clip);
		g_printerr ("deepinvte_terminal_paint"
				"	(%d,%d)x(%d,%d) pixels\n",
				clip.x, clip.y, clip.width, clip.height);
	}

	_deepinvte_draw_clip(terminal->pvt->draw, region);
	gtk_widget_get_allocation(&terminal->widget, &allocation);
	_deepinvte_draw_clear (terminal->pvt->draw, 0, 0,
			 allocation.width, allocation.height);

	/* Calculate the bounding rectangle. */
	{
		DeepinvteRegionRectangle *rectangles;
		gint n, n_rectangles;
		...
		if (!(n_rectangles == 1
			...
			for (n = 0; n < n_rectangles; n++) {
				deepinvte_terminal_expand_region (terminal, rr, rectangles + n);
			}
			g_free (rectangles);
		...
		/* and now paint them */
		for (n = 0; n < n_rectangles; n++) {
			deepinvte_terminal_paint_area (terminal, rectangles + n);
		}
		g_free (rectangles);
	}

	deepinvte_terminal_paint_cursor(terminal);

	deepinvte_terminal_paint_im_preedit_string(terminal);
...
deepinvte_cairo_get_clip_region (cairo_t *cr)
{
...
static gboolean
deepinvte_terminal_draw(GtkWidget *widget,
                  cairo_t *cr)
{
        DeepinvteTerminal *terminal = DEEPINVTE_TERMINAL (widget);
        cairo_rectangle_int_t clip_rect;
        ...
        _deepinvte_debug_print (DEEPINVTE_DEBUG_WORK, "+");
        _deepinvte_debug_print (DEEPINVTE_DEBUG_EVENTS, "Draw (%d,%d)x(%d,%d)\n",
                          clip_rect.x, clip_rect.y,
                          clip_rect.width, clip_rect.height);

        region = deepinvte_cairo_get_clip_region (cr);
        if (region == NULL)
                return FALSE;

        deepinvte_terminal_paint(widget, region);
        cairo_region_destroy (region);
...
static gboolean
deepinvte_terminal_expose(GtkWidget *widget,
                    GdkEventExpose *event)
{
	...
	_deepinvte_debug_print (DEEPINVTE_DEBUG_WORK, "+");
	_deepinvte_debug_print (DEEPINVTE_DEBUG_EVENTS, "Expose (%d,%d)x(%d,%d)\n",
	...
	if (terminal->pvt->active != NULL &&
		...
		deepinvte_terminal_paint(widget, event->region);
		terminal->pvt->invalidated_all = FALSE;
	}
	return FALSE;
}
...
static gboolean
deepinvte_terminal_scroll(GtkWidget *widget, GdkEventScroll *event)
{
	GtkAdjustment *adj;
	DeepinvteTerminal *terminal;
	gdouble v;
	...
	if (terminal->pvt->alternate_screen_scroll &&
		...
		deepinvte_terminal_queue_adjustment_value_changed_clamped (terminal, v);
	}
 ...
 * it to the caller. */
static AtkObject *
deepinvte_terminal_get_accessible(GtkWidget *widget)
{
	DeepinvteTerminal *terminal;
	...
	if (first_time) {
		 ...
		 * the parent type of DeepinvteTerminal.
		 */
		derived_type = g_type_parent (DEEPINVTE_TYPE_TERMINAL);
		...
		if (g_type_is_a (derived_atk_type, GTK_TYPE_ACCESSIBLE)) {
			atk_registry_set_factory_type (registry,
						       DEEPINVTE_TYPE_TERMINAL,
						       deepinvte_terminal_accessible_factory_get_type ());
		}
		first_time = FALSE;
	}

	return GTK_WIDGET_CLASS (deepinvte_terminal_parent_class)->get_accessible (widget);
}

static void
deepinvte_terminal_get_property (GObject *object,
                           guint prop_id,
                           GValue *value,
                           GParamSpec *pspec)
{
	...
	switch (prop_id)
	{
                ...
                case PROP_ALLOW_BOLD:
                        g_value_set_boolean (value, deepinvte_terminal_get_allow_bold (terminal));
                        break;
                case PROP_AUDIBLE_BELL:
                        g_value_set_boolean (value, deepinvte_terminal_get_audible_bell (terminal));
                        break;
                ...
                case PROP_BACKGROUND_SATURATION:
                        g_value_set_double (value, (double) pvt->bg_saturation / (double) DEEPINVTE_SATURATION_MAX);
                        break;
                ...
                case PROP_CURSOR_BLINK_MODE:
                        g_value_set_enum (value, deepinvte_terminal_get_cursor_blink_mode (terminal));
                        break;
                case PROP_CURSOR_SHAPE:
                        g_value_set_enum (value, deepinvte_terminal_get_cursor_shape (terminal));
                        break;
                case PROP_DELETE_BINDING:
                        g_value_set_enum (value, pvt->delete_binding);
                        break;
                case PROP_EMULATION:
                        g_value_set_string (value, deepinvte_terminal_get_emulation (terminal));
                        break;
                case PROP_ENCODING:
                        g_value_set_string (value, deepinvte_terminal_get_encoding (terminal));
                        break;
                case PROP_FONT_DESC:
                        g_value_set_boxed (value, deepinvte_terminal_get_font (terminal));
                        break;
                case PROP_ICON_TITLE:
                        g_value_set_string (value, deepinvte_terminal_get_icon_title (terminal));
                        break;
                case PROP_MOUSE_POINTER_AUTOHIDE:
                        g_value_set_boolean (value, deepinvte_terminal_get_mouse_autohide (terminal));
                        break;
                case PROP_PTY:
                        g_value_set_int (value, pvt->pty != NULL ? deepinvte_pty_get_fd(pvt->pty) : -1);
                        break;
                case PROP_PTY_OBJECT:
                        g_value_set_object (value, deepinvte_terminal_get_pty_object(terminal));
                        break;
                ...
                case PROP_WINDOW_TITLE:
                        g_value_set_string (value, deepinvte_terminal_get_window_title (terminal));
                        break;
                case PROP_WORD_CHARS:
                        g_value_set_string (value, NULL /* FIXME */);
                        break;
                case PROP_VISIBLE_BELL:
                        g_value_set_boolean (value, deepinvte_terminal_get_visible_bell (terminal));
                        break;
...
static void
deepinvte_terminal_set_property (GObject *object,
                           guint prop_id,
                           const GValue *value,
                           GParamSpec *pspec)
{
...
#if GTK_CHECK_VERSION (2, 91, 2)
                case PROP_HADJUSTMENT:
                        deepinvte_terminal_set_hadjustment (terminal, g_value_get_object (value));
                        break;
                case PROP_VADJUSTMENT:
                        deepinvte_terminal_set_vadjustment (terminal, g_value_get_object (value));
                        break;
                case PROP_HSCROLL_POLICY:
...
#endif
                case PROP_ALLOW_BOLD:
                        deepinvte_terminal_set_allow_bold (terminal, g_value_get_boolean (value));
                        break;
                case PROP_AUDIBLE_BELL:
                        deepinvte_terminal_set_audible_bell (terminal, g_value_get_boolean (value));
                        break;
                case PROP_BACKGROUND_IMAGE_FILE:
                        deepinvte_terminal_set_background_image_file (terminal, g_value_get_string (value));
                        break;
                case PROP_BACKGROUND_IMAGE_PIXBUF:
                        deepinvte_terminal_set_background_image (terminal, g_value_get_object (value));
                        break;
                case PROP_BACKGROUND_OPACITY:
                        deepinvte_terminal_set_opacity (terminal, g_value_get_double (value) * (double) G_MAXUINT16);
                        break;
                case PROP_BACKGROUND_SATURATION:
                        deepinvte_terminal_set_background_saturation (terminal, g_value_get_double (value));
                        break;
                case PROP_BACKGROUND_TINT_COLOR:
                        deepinvte_terminal_set_background_tint_color (terminal, g_value_get_boxed (value));
                        break;
                case PROP_BACKGROUND_TRANSPARENT:
                        deepinvte_terminal_set_background_transparent (terminal, g_value_get_boolean (value));
                        break;
                case PROP_BACKSPACE_BINDING:
                        deepinvte_terminal_set_backspace_binding (terminal, g_value_get_enum (value));
                        break;
                case PROP_CURSOR_BLINK_MODE:
                        deepinvte_terminal_set_cursor_blink_mode (terminal, g_value_get_enum (value));
                        break;
                case PROP_CURSOR_SHAPE:
                        deepinvte_terminal_set_cursor_shape (terminal, g_value_get_enum (value));
                        break;
                case PROP_DELETE_BINDING:
                        deepinvte_terminal_set_delete_binding (terminal, g_value_get_enum (value));
                        break;
                case PROP_EMULATION:
                        deepinvte_terminal_set_emulation (terminal, g_value_get_string (value));
                        break;
                case PROP_ENCODING:
                        deepinvte_terminal_set_encoding (terminal, g_value_get_string (value));
                        break;
                case PROP_FONT_DESC:
                        deepinvte_terminal_set_font_full_internal (terminal, g_value_get_boxed (value), pvt->fontantialias);
                        break;
                case PROP_MOUSE_POINTER_AUTOHIDE:
                        deepinvte_terminal_set_mouse_autohide (terminal, g_value_get_boolean (value));
                        break;
                case PROP_PTY:
                        deepinvte_terminal_set_pty (terminal, g_value_get_int (value));
                        break;
                case PROP_PTY_OBJECT:
                        deepinvte_terminal_set_pty_object (terminal, g_value_get_object (value));
                        break;
                case PROP_SCROLL_BACKGROUND:
                        deepinvte_terminal_set_scroll_background (terminal, g_value_get_boolean (value));
                        break;
                case PROP_SCROLLBACK_LINES:
                        deepinvte_terminal_set_scrollback_lines (terminal, g_value_get_uint (value));
                        break;
                case PROP_SCROLL_ON_KEYSTROKE:
                        deepinvte_terminal_set_scroll_on_keystroke(terminal, g_value_get_boolean (value));
                        break;
                case PROP_SCROLL_ON_OUTPUT:
                        deepinvte_terminal_set_scroll_on_output (terminal, g_value_get_boolean (value));
                        break;
                case PROP_WORD_CHARS:
                        deepinvte_terminal_set_word_chars (terminal, g_value_get_string (value));
                        break;
                case PROP_VISIBLE_BELL:
                        deepinvte_terminal_set_visible_bell (terminal, g_value_get_boolean (value));
                        break;
...
/* Initialize methods. */
static void
deepinvte_terminal_class_init(DeepinvteTerminalClass *klass)
{
	...
	g_type_class_add_private(klass, sizeof (DeepinvteTerminalPrivate));

	gobject_class = G_OBJECT_CLASS(klass);
	widget_class = GTK_WIDGET_CLASS(klass);

	/* Override some of the default handlers. */
	gobject_class->finalize = deepinvte_terminal_finalize;
        gobject_class->get_property = deepinvte_terminal_get_property;
        gobject_class->set_property = deepinvte_terminal_set_property;
	widget_class->realize = deepinvte_terminal_realize;
	widget_class->scroll_event = deepinvte_terminal_scroll;
#if GTK_CHECK_VERSION (2, 90, 8)
        widget_class->draw = deepinvte_terminal_draw;
#else
	widget_class->expose_event = deepinvte_terminal_expose;
#endif
	widget_class->key_press_event = deepinvte_terminal_key_press;
	widget_class->key_release_event = deepinvte_terminal_key_release;
	widget_class->button_press_event = deepinvte_terminal_button_press;
	widget_class->button_release_event = deepinvte_terminal_button_release;
	widget_class->motion_notify_event = deepinvte_terminal_motion_notify;
	widget_class->enter_notify_event = deepinvte_terminal_enter;
	widget_class->leave_notify_event = deepinvte_terminal_leave;
	widget_class->focus_in_event = deepinvte_terminal_focus_in;
	widget_class->focus_out_event = deepinvte_terminal_focus_out;
	widget_class->visibility_notify_event = deepinvte_terminal_visibility_notify;
	widget_class->unrealize = deepinvte_terminal_unrealize;
	widget_class->style_set = deepinvte_terminal_style_set;
#if GTK_CHECK_VERSION (2, 91, 0)
	widget_class->get_preferred_width = deepinvte_terminal_get_preferred_width;
	widget_class->get_preferred_height = deepinvte_terminal_get_preferred_height;
#else
	widget_class->size_request = deepinvte_terminal_size_request;
#endif
	widget_class->size_allocate = deepinvte_terminal_size_allocate;
	widget_class->get_accessible = deepinvte_terminal_get_accessible;
        widget_class->screen_changed = deepinvte_terminal_screen_changed;
	...
	klass->copy_clipboard = deepinvte_terminal_real_copy_clipboard;
	klass->paste_clipboard = deepinvte_terminal_real_paste_clipboard;
        ...
        klass->set_scroll_adjustments = deepinvte_terminal_set_scroll_adjustments;

        /**
         * DeepinvteTerminal::set-scroll-adjustments:
         * @deepinvteterminal: the object which received the signal
         * @horizontal: (allow-none): the horizontal #GtkAdjustment (unused in #DeepinvteTerminal)
         ...
         * of #GtkScrollbar to the scroll directions of the #DeepinvteTerminal.
         *
         * Since: 0.17.1
         */
	widget_class->set_scroll_adjustments_signal =
		g_signal_new(I_("set-scroll-adjustments"),
			     ...
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET (DeepinvteTerminalClass, set_scroll_adjustments),
			     NULL, NULL,
			     _deepinvte_marshal_VOID__OBJECT_OBJECT,
			     G_TYPE_NONE, 2,
...
#define OBSOLETE_SIGNAL(str) str
        ...
        /**
         * DeepinvteTerminal::eof:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted when the terminal receives an end-of-file from a child which
         * is running in the terminal.  This signal is frequently (but not
         * always) emitted with a #DeepinvteTerminal::child-exited signal.
         */
        OBSOLETE_SIGNAL (klass->eof_signal =)
                g_signal_new(I_("eof"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, eof),
			     NULL,
			     NULL,
        ...
        /**
         * DeepinvteTerminal::window-title-changed:
         * @deepinvteterminal: the object which received the signal
         *
        ...
        OBSOLETE_SIGNAL (klass->window_title_changed_signal =)
                g_signal_new(I_("window-title-changed"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, window_title_changed),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::icon-title-changed:
         * @deepinvteterminal: the object which received the signal
         *
        ...
        OBSOLETE_SIGNAL (klass->icon_title_changed_signal =)
                g_signal_new(I_("icon-title-changed"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, icon_title_changed),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::encoding-changed:
         * @deepinvteterminal: the object which received the signal
         *
        ...
        OBSOLETE_SIGNAL (klass->encoding_changed_signal =)
                g_signal_new(I_("encoding-changed"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, encoding_changed),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        OBSOLETE_SIGNAL (klass->commit_signal =)
                g_signal_new(I_("commit"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, commit),
			     NULL,
			     NULL,
			     _deepinvte_marshal_VOID__STRING_UINT,
			     G_TYPE_NONE, 2, G_TYPE_STRING, G_TYPE_UINT);

        /**
         * DeepinvteTerminal::emulation-changed:
         * @deepinvteterminal: the object which received the signal
         *
        ...
        OBSOLETE_SIGNAL (klass->emulation_changed_signal =)
                g_signal_new(I_("emulation-changed"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, emulation_changed),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::char-size-changed:
         * @deepinvteterminal: the object which received the signal
        ...
        OBSOLETE_SIGNAL (klass->char_size_changed_signal =)
                g_signal_new(I_("char-size-changed"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, char_size_changed),
			     NULL,
			     NULL,
			     _deepinvte_marshal_VOID__UINT_UINT,
			     G_TYPE_NONE, 2, G_TYPE_UINT, G_TYPE_UINT);

        /**
         * DeepinvteTerminal::selection-changed:
         * @deepinvteterminal: the object which received the signal
         *
        ...
        OBSOLETE_SIGNAL (klass->selection_changed_signal =)
                g_signal_new (I_("selection-changed"),
			      G_OBJECT_CLASS_TYPE(klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET(DeepinvteTerminalClass, selection_changed),
			      NULL,
			      NULL,
                              g_cclosure_marshal_VOID__VOID,
			      G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::contents-changed:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted whenever the visible appearance of the terminal has changed.
         * Used primarily by #DeepinvteTerminalAccessible.
         */
        OBSOLETE_SIGNAL (klass->contents_changed_signal =)
                g_signal_new(I_("contents-changed"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, contents_changed),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::cursor-moved:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted whenever the cursor moves to a new character cell.  Used
         * primarily by #DeepinvteTerminalAccessible.
         */
        OBSOLETE_SIGNAL (klass->cursor_moved_signal =)
                g_signal_new(I_("cursor-moved"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, cursor_moved),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::deiconify-window:
         * @deepinvteterminal: the object which received the signal
         *
        ...
        OBSOLETE_SIGNAL (klass->deiconify_window_signal =)
                g_signal_new(I_("deiconify-window"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, deiconify_window),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::iconify-window:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted at the child application's request.
         */
        OBSOLETE_SIGNAL (klass->iconify_window_signal =)
                g_signal_new(I_("iconify-window"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, iconify_window),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::raise-window:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted at the child application's request.
         */
        OBSOLETE_SIGNAL (klass->raise_window_signal =)
                g_signal_new(I_("raise-window"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, raise_window),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::lower-window:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted at the child application's request.
         */
        OBSOLETE_SIGNAL (klass->lower_window_signal =)
                g_signal_new(I_("lower-window"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, lower_window),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::refresh-window:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted at the child application's request.
         */
        OBSOLETE_SIGNAL (klass->refresh_window_signal =)
                g_signal_new(I_("refresh-window"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, refresh_window),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::restore-window:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted at the child application's request.
         */
        OBSOLETE_SIGNAL (klass->restore_window_signal =)
                g_signal_new(I_("restore-window"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, restore_window),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::maximize-window:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted at the child application's request.
         */
        OBSOLETE_SIGNAL (klass->maximize_window_signal =)
                g_signal_new(I_("maximize-window"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, maximize_window),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::resize-window:
         * @deepinvteterminal: the object which received the signal
        ...
        OBSOLETE_SIGNAL (klass->resize_window_signal =)
                g_signal_new(I_("resize-window"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, resize_window),
			     NULL,
			     NULL,
			     _deepinvte_marshal_VOID__UINT_UINT,
			     G_TYPE_NONE, 2, G_TYPE_UINT, G_TYPE_UINT);
        ...
        OBSOLETE_SIGNAL (klass->move_window_signal =)
                g_signal_new(I_("move-window"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, move_window),
			     NULL,
			     NULL,
			     _deepinvte_marshal_VOID__UINT_UINT,
			     G_TYPE_NONE, 2, G_TYPE_UINT, G_TYPE_UINT);
        ...
        /**
         * DeepinvteTerminal::status-line-changed:
         * @deepinvteterminal: the object which received the signal
         *
        ...
        OBSOLETE_SIGNAL (klass->status_line_changed_signal =)
                g_signal_new(I_("status-line-changed"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, status_line_changed),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::increase-font-size:
         * @deepinvteterminal: the object which received the signal
         *
        ...
        OBSOLETE_SIGNAL (klass->increase_font_size_signal =)
                g_signal_new(I_("increase-font-size"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, increase_font_size),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::decrease-font-size:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted when the user hits the '-' key while holding the Control key.
         */
        OBSOLETE_SIGNAL (klass->decrease_font_size_signal =)
                g_signal_new(I_("decrease-font-size"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, decrease_font_size),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::text-modified:
         * @deepinvteterminal: the object which received the signal
         *
        ...
        OBSOLETE_SIGNAL (klass->text_modified_signal =)
                g_signal_new(I_("text-modified"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, text_modified),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::text-inserted:
         * @deepinvteterminal: the object which received the signal
         *
         * An internal signal used for communication between the terminal and
         * its accessibility peer. May not be emitted under certain
        ...
        OBSOLETE_SIGNAL (klass->text_inserted_signal =)
                g_signal_new(I_("text-inserted"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, text_inserted),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::text-deleted:
         * @deepinvteterminal: the object which received the signal
         *
         * An internal signal used for communication between the terminal and
         * its accessibility peer. May not be emitted under certain
        ...
        OBSOLETE_SIGNAL (klass->text_deleted_signal =)
                g_signal_new(I_("text-deleted"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, text_deleted),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::text-scrolled:
         * @deepinvteterminal: the object which received the signal
       ...
       OBSOLETE_SIGNAL (klass->text_scrolled_signal =)
                g_signal_new(I_("text-scrolled"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, text_scrolled),
			     NULL,
			     NULL,
...
#undef OBSOLETE_SIGNAL
        ...
        /**
         * DeepinvteTerminal::copy-clipboard:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted whenever deepinvte_terminal_copy_clipboard() is called.
         */
	signals[COPY_CLIPBOARD] =
                g_signal_new(I_("copy-clipboard"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, copy_clipboard),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal::paste-clipboard:
         * @deepinvteterminal: the object which received the signal
         *
         * Emitted whenever deepinvte_terminal_paste_clipboard() is called.
         */
	signals[PASTE_CLIPBOARD] =
                g_signal_new(I_("paste-clipboard"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, paste_clipboard),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        g_signal_new(I_("beep"),
			     G_OBJECT_CLASS_TYPE(klass),
			     G_SIGNAL_RUN_LAST,
			     G_STRUCT_OFFSET(DeepinvteTerminalClass, beep),
			     NULL,
			     NULL,
                             g_cclosure_marshal_VOID__VOID,
			     G_TYPE_NONE, 0);
        ...
        /**
         * DeepinvteTerminal:audible-bell:
         *
        ...
        /**
         * DeepinvteTerminal:background-image-file: (type filename):
         *
         * Sets a background image file for the widget.  If specified by
         * #DeepinvteTerminal:background-saturation:, the terminal will tint its
        ...
        /**
         * DeepinvteTerminal:background-image-pixbuf:
         *
        ...
        /**
         * DeepinvteTerminal:background-opacity:
         *
        ...
        /**
         * DeepinvteTerminal:background-saturation:
         *
         * If a background image has been set using #DeepinvteTerminal:background-image-file: or
         * #DeepinvteTerminal:background-image-pixbuf:, or #DeepinvteTerminal:background-transparent:,
        ...
        /**
         * DeepinvteTerminal:background-tint-color:
         *
         * If a background image has been set using #DeepinvteTerminal:background-image-file: or
         * #DeepinvteTerminal:background-image-pixbuf:, or #DeepinvteTerminal:background-transparent:, and
         * and the value set by DeepinvteTerminal:background-saturation: is less than 1.0,
         * the terminal
        ...
        /**
         * DeepinvteTerminal:background-transparent:
         *
        ...
        /**
         * DeepinvteTerminal:backspace-binding:
         *
        ...
        g_object_class_install_property
                (gobject_class,
                 ...
                 g_param_spec_enum ("backspace-binding", NULL, NULL,
                                    DEEPINVTE_TYPE_TERMINAL_ERASE_BINDING,
                                    DEEPINVTE_ERASE_AUTO,
                                    G_PARAM_READWRITE | STATIC_PARAMS));

        /**
         * DeepinvteTerminal:cursor-blink-mode:
         *
         * Sets whether or not the cursor will blink. Using %DEEPINVTE_CURSOR_BLINK_SYSTEM
        ...
        g_object_class_install_property
                (gobject_class,
                 ...
                 g_param_spec_enum ("cursor-blink-mode", NULL, NULL,
                                    DEEPINVTE_TYPE_TERMINAL_CURSOR_BLINK_MODE,
                                    DEEPINVTE_CURSOR_BLINK_SYSTEM,
                                    G_PARAM_READWRITE | STATIC_PARAMS));
     
        /**
         * DeepinvteTerminal:cursor-shape:
         *
        ...
        g_object_class_install_property
                (gobject_class,
                 ...
                 g_param_spec_enum ("cursor-shape", NULL, NULL,
                                    DEEPINVTE_TYPE_TERMINAL_CURSOR_SHAPE,
                                    DEEPINVTE_CURSOR_SHAPE_BLOCK,
                                    G_PARAM_READWRITE | STATIC_PARAMS));
     
        /**
         * DeepinvteTerminal:delete-binding:
         *
         * Controls what string or control sequence the terminal sends to its child
        ...
        g_object_class_install_property
                (gobject_class,
                 ...
                 g_param_spec_string ("emulation", NULL, NULL,
                                      DEEPINVTE_DEFAULT_EMULATION,
                                      G_PARAM_READWRITE | STATIC_PARAMS));
     
        /**
         * DeepinvteTerminal:encoding:
         *
        ...
        /**
         * DeepinvteTerminal:pointer-autohide:
         *
        ...
        /**
         * DeepinvteTerminal:pty:
         *
         * The file descriptor of the master end of the terminal's PTY.
         * 
         * Since: 0.20
         *
         * Deprecated: 0.26: Use the #DeepinvteTerminal:pty-object property instead
         */
        g_object_class_install_property
                (gobject_class,
                 PROP_PTY,
        ...
        /**
         * DeepinvteTerminal:scroll-background:
         *
        ...
        /**
         * DeepinvteTerminal:scrollback-lines:
         *
        ...
        g_object_class_install_property
                (gobject_class,
                 ...
                 g_param_spec_uint ("scrollback-lines", NULL, NULL,
                                    0, G_MAXUINT,
                                    DEEPINVTE_SCROLLBACK_INIT,
                                    G_PARAM_READWRITE | STATIC_PARAMS));
     
        /**
         * DeepinvteTerminal:scroll-on-keystroke:
         *
        ...
        /**
         * DeepinvteTerminal:scroll-on-output:
         *
         * Controls whether or not the terminal will forcibly scroll to the bottom of
        ...
        /**
         * DeepinvteTerminal:window-title:
         *
        ...
        /**
         * DeepinvteTerminal:visible-bell:
         *
        ...
        /**
         * DeepinvteTerminal:inner-border:
         *
        ...
        gtk_rc_parse_string("style \"deepinvte-default-style\" {\n"
                              "DeepinvteTerminal::inner-border = { 1, 1, 1, 1 }\n"
                            "}\n"
                            "class \"DeepinvteTerminal\" style : gtk \"deepinvte-default-style\"\n");
#endif
...
#if GTK_CHECK_VERSION (2, 99, 0)
        klass->priv = G_TYPE_CLASS_GET_PRIVATE (klass, DEEPINVTE_TYPE_TERMINAL, DeepinvteTerminalClassPrivate);

        klass->priv->style_provider = GTK_STYLE_PROVIDER (gtk_css_provider_new ());
        gtk_css_provider_load_from_data (GTK_CSS_PROVIDER (klass->priv->style_provider),
                                         "DeepinvteTerminal {\n"
                                           "-DeepinvteTerminal-inner-border: 1;\n"
                                         "}\n",
                                         -1, NULL);
#endif /* GTK 3.0 */
}
...
/**
 * deepinvte_terminal_set_audible_bell:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_audible_bell(DeepinvteTerminal *terminal, gboolean is_audible)
{
        DeepinvteTerminalPrivate *pvt;
...
/**
 * deepinvte_terminal_get_audible_bell:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
gboolean
deepinvte_terminal_get_audible_bell(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), FALSE);
...
/**
 * deepinvte_terminal_set_visible_bell:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_visible_bell(DeepinvteTerminal *terminal, gboolean is_visible)
{
        DeepinvteTerminalPrivate *pvt;
...
/**
 * deepinvte_terminal_get_visible_bell:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
gboolean
deepinvte_terminal_get_visible_bell(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), FALSE);
...
/**
 * deepinvte_terminal_set_allow_bold:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_allow_bold(DeepinvteTerminal *terminal, gboolean allow_bold)
{
        DeepinvteTerminalPrivate *pvt;
...
/**
 * deepinvte_terminal_get_allow_bold:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
gboolean
deepinvte_terminal_get_allow_bold(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), FALSE);
 ...
 */
void
deepinvte_terminal_set_scroll_background(DeepinvteTerminal *terminal, gboolean scroll)
{
        DeepinvteTerminalPrivate *pvt;
...
/**
 * deepinvte_terminal_set_scroll_on_output:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_scroll_on_output(DeepinvteTerminal *terminal, gboolean scroll)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
...
/**
 * deepinvte_terminal_set_scroll_on_keystroke:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_set_scroll_on_keystroke(DeepinvteTerminal *terminal, gboolean scroll)
{
        DeepinvteTerminalPrivate *pvt;
...
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SELECTION, "Copying to CLIPBOARD.\n");
	if (terminal->pvt->selection != NULL) {
		GtkClipboard *clipboard;
		clipboard = deepinvte_terminal_clipboard_get(terminal,
...
/**
 * deepinvte_terminal_copy_clipboard:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
void
deepinvte_terminal_copy_clipboard(DeepinvteTerminal *terminal)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
...
static void
deepinvte_terminal_real_paste_clipboard(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SELECTION, "Pasting CLIPBOARD.\n");
	deepinvte_terminal_paste(terminal, GDK_SELECTION_CLIPBOARD);
}
 ...
 */
void
deepinvte_terminal_paste_clipboard(DeepinvteTerminal *terminal)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
...
/**
 * deepinvte_terminal_copy_primary:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
void
deepinvte_terminal_copy_primary(DeepinvteTerminal *terminal)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SELECTION, "Copying to PRIMARY.\n");
	deepinvte_terminal_copy(terminal, GDK_SELECTION_PRIMARY);
}

/**
 * deepinvte_terminal_paste_primary:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
void
deepinvte_terminal_paste_primary(DeepinvteTerminal *terminal)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SELECTION, "Pasting PRIMARY.\n");
	deepinvte_terminal_paste(terminal, GDK_SELECTION_PRIMARY);
}

/**
 * deepinvte_terminal_im_append_menuitems:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_im_append_menuitems(DeepinvteTerminal *terminal, GtkMenuShell *menushell)
{
...
static gboolean
deepinvte_terminal_background_update(DeepinvteTerminal *terminal)
{
	double saturation;
	...
	_deepinvte_debug_print(DEEPINVTE_DEBUG_MISC|DEEPINVTE_DEBUG_EVENTS,
			"Updating background image.\n");

	entry = &terminal->pvt->palette[DEEPINVTE_DEF_BG];
	_deepinvte_debug_print(DEEPINVTE_DEBUG_BG,
	...
	saturation /= DEEPINVTE_SATURATION_MAX;
	if (terminal->pvt->bg_transparent) {
		if (terminal->pvt->root_pixmap_changed_tag == 0) {
			DeepinvteBg *bg;

			/* Connect to background-change events. */
			bg = deepinvte_bg_get_for_screen (gtk_widget_get_screen (&terminal->widget));
			terminal->pvt->root_pixmap_changed_tag =
		...
		_deepinvte_draw_set_background_image(terminal->pvt->draw,
...
static void
deepinvte_terminal_queue_background_update(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_EVENTS,
 ...
 * deepinvte_terminal_set_background_image_file(), or
 * deepinvte_terminal_set_background_transparent(), and the saturation value is less
 * than 1.0, the terminal will adjust the colors of the image before drawing
 * the image.  To do so, the terminal will create a copy of the background
 * image (or snapshot of the root window) and modify its pixel values.
 */
void
deepinvte_terminal_set_background_saturation(DeepinvteTerminal *terminal, double saturation)
{
        DeepinvteTerminalPrivate *pvt;
	guint v;
	...
	v = CLAMP(saturation * DEEPINVTE_SATURATION_MAX, 0, DEEPINVTE_SATURATION_MAX);
        if (v == pvt->bg_saturation)
                return;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_MISC,
			"Setting background saturation to %d/%d.\n",
			v, DEEPINVTE_SATURATION_MAX);
 ...
 * deepinvte_terminal_set_background_image(),
 * deepinvte_terminal_set_background_image_file(), or
 * deepinvte_terminal_set_background_transparent(), and the value set by
 * deepinvte_terminal_set_background_saturation() is less than one, the terminal
 * will adjust the color of the image before drawing the image.  To do so,
 * the terminal will create a copy of the background image (or snapshot of
 * the root window) and modify its pixel values.  The initial tint color
 * is black.
 ...
 */
void
deepinvte_terminal_set_background_tint_color(DeepinvteTerminal *terminal,
 ...
 */
void
deepinvte_terminal_set_background_transparent(DeepinvteTerminal *terminal,
 ...
 * specified image.  If necessary, the image will be tiled to cover the
 * widget's entire visible area. If specified by
 * deepinvte_terminal_set_background_saturation(), the terminal will tint its
 * in-memory copy of the image before applying it to the terminal.
 */
void
deepinvte_terminal_set_background_image(DeepinvteTerminal *terminal, GdkPixbuf *image)
{
        DeepinvteTerminalPrivate *pvt;
        GObject *object;
 ...
 */
void
deepinvte_terminal_set_background_image_file(DeepinvteTerminal *terminal, const char *path)
{
        DeepinvteTerminalPrivate *pvt;
        GObject *object;
...
/**
 * deepinvte_terminal_get_has_selection:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
gboolean
deepinvte_terminal_get_has_selection(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), FALSE);
...
/**
 * deepinvte_terminal_get_using_xft:
 * @terminal: a #DeepinvteTerminal
 *
 * A #DeepinvteTerminal can use multiple methods to draw text.  This function
 ...
 */
gboolean
deepinvte_terminal_get_using_xft(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), TRUE);
	return TRUE;
}

static void
deepinvte_terminal_set_cursor_blinks_internal(DeepinvteTerminal *terminal, gboolean blink)
{
        DeepinvteTerminalPrivate *pvt = terminal->pvt;
 ...
 *
 * Deprecated: 0.17.1 Use deepinvte_terminal_set_cursor_blink_mode() instead.
 */
void
deepinvte_terminal_set_cursor_blinks(DeepinvteTerminal *terminal, gboolean blink)
{
        deepinvte_terminal_set_cursor_blink_mode(terminal, blink ? DEEPINVTE_CURSOR_BLINK_ON : DEEPINVTE_CURSOR_BLINK_OFF);
}

/**
 * deepinvte_terminal_set_cursor_blink_mode:
 * @terminal: a #DeepinvteTerminal
 * @mode: the #DeepinvteTerminalCursorBlinkMode to use
 *
 * Sets whether or not the cursor will blink. Using %DEEPINVTE_CURSOR_BLINK_SYSTEM
 * will use the #GtkSettings::gtk-cursor-blink setting.
 *
 ...
 */
void
deepinvte_terminal_set_cursor_blink_mode(DeepinvteTerminal *terminal, DeepinvteTerminalCursorBlinkMode mode)
{
        DeepinvteTerminalPrivate *pvt;
        gboolean blinks;
	...
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
        ...
        switch (mode) {
          case DEEPINVTE_CURSOR_BLINK_SYSTEM:
            g_object_get(gtk_widget_get_settings(GTK_WIDGET(terminal)),
                                                 "gtk-cursor-blink", &blinks,
                                                 NULL);
            break;
          case DEEPINVTE_CURSOR_BLINK_ON:
            blinks = TRUE;
            break;
          case DEEPINVTE_CURSOR_BLINK_OFF:
            blinks = FALSE;
            break;
        }

        deepinvte_terminal_set_cursor_blinks_internal(terminal, blinks);

        g_object_notify(G_OBJECT(terminal), "cursor-blink-mode");
}

/**
 * deepinvte_terminal_get_cursor_blink_mode:
 * @terminal: a #DeepinvteTerminal
 *
...
deepinvte_terminal_get_cursor_blink_mode(DeepinvteTerminal *terminal)
{
        g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), DEEPINVTE_CURSOR_BLINK_SYSTEM);
 ...
 * @terminal: a #DeepinvteTerminal
 * @shape: the #DeepinvteTerminalCursorShape to use
 *
 ...
 */
void
deepinvte_terminal_set_cursor_shape(DeepinvteTerminal *terminal, DeepinvteTerminalCursorShape shape)
{
        DeepinvteTerminalPrivate *pvt;
...
deepinvte_terminal_get_cursor_shape(DeepinvteTerminal *terminal)
{
        g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), DEEPINVTE_CURSOR_SHAPE_BLOCK);
 ...
 */
void
deepinvte_terminal_set_scrollback_lines(DeepinvteTerminal *terminal, glong lines)
{
        DeepinvteTerminalPrivate *pvt;
        GObject *object;
	glong scroll_delta;
	...
	if (screen == &terminal->pvt->normal_screen) {
		...
		next = MAX (screen->cursor_current.row + 1,
				_deepinvte_ring_next (screen->row_data));
		_deepinvte_ring_resize (screen->row_data, lines);
		low = _deepinvte_ring_delta (screen->row_data);
		...
		if (_deepinvte_ring_next (screen->row_data) > next){
			_deepinvte_ring_shrink (screen->row_data, next - low);
		}
	} else {
		_deepinvte_ring_resize (screen->row_data, terminal->row_count);
		scroll_delta = _deepinvte_ring_delta (screen->row_data);
		screen->insert_delta = _deepinvte_ring_delta (screen->row_data);
		if (_deepinvte_ring_next (screen->row_data) > screen->insert_delta + terminal->row_count){
			_deepinvte_ring_shrink (screen->row_data, terminal->row_count);
		}
	}

	/* Adjust the scrollbars to the new locations. */
	deepinvte_terminal_queue_adjustment_value_changed (terminal, scroll_delta);
 ...
 */
void
deepinvte_terminal_set_word_chars(DeepinvteTerminal *terminal, const char *spec)
{
	DeepinvteConv conv;
	gunichar *wbuf;
	...
	if (conv == DEEPINVTE_INVALID_CONV) {
		...
		g_warning(_("_deepinvte_conv_open() failed setting word characters"));
		return;
	}
	ilen = strlen(spec);
	ibuf = ibufptr = (guchar *)g_strdup(spec);
	olen = (ilen + 1) * sizeof(gunichar);
	_deepinvte_buffer_set_minimum_size(terminal->pvt->conv_buffer, olen);
	obuf = obufptr = terminal->pvt->conv_buffer->data;
	wbuf = (gunichar*) obuf;
	wbuf[ilen] = '\0';
	_deepinvte_conv(conv, (const guchar **)&ibuf, &ilen, &obuf, &olen);
...
/**
 * deepinvte_terminal_set_backspace_binding:
 * @terminal: a #DeepinvteTerminal
 * @binding: a #DeepinvteTerminalEraseBinding for the backspace key
 *
 ...
 */
void
deepinvte_terminal_set_backspace_binding(DeepinvteTerminal *terminal,
...
/**
 * deepinvte_terminal_set_delete_binding:
 * @terminal: a #DeepinvteTerminal
 * @binding: a #DeepinvteTerminalEraseBinding for the delete key
 *
 ...
 */
void
deepinvte_terminal_set_delete_binding(DeepinvteTerminal *terminal,
				DeepinvteTerminalEraseBinding binding)
{
        DeepinvteTerminalPrivate *pvt;
...
/**
 * deepinvte_terminal_set_mouse_autohide:
 * @terminal: a #DeepinvteTerminal
 ...
 * deepinvte_terminal_get_mouse_autohide().
 */
void
deepinvte_terminal_set_mouse_autohide(DeepinvteTerminal *terminal, gboolean setting)
{
        DeepinvteTerminalPrivate *pvt;
 ...
 * using deepinvte_terminal_set_mouse_autohide().
 *
 * Returns: %TRUE if autohiding is enabled, %FALSE if not
 */
gboolean
deepinvte_terminal_get_mouse_autohide(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), FALSE);
 ...
 */
void
deepinvte_terminal_reset(DeepinvteTerminal *terminal,
                   ...
                   gboolean clear_history)
{
	...
	_deepinvte_incoming_chunks_release (pvt->incoming);
	pvt->incoming = NULL;
	g_array_set_size(pvt->pending, 0);
	_deepinvte_buffer_clear(pvt->outgoing);
	/* Reset charset substitution state. */
	_deepinvte_iso2022_state_free(pvt->iso2022);
	pvt->iso2022 = _deepinvte_iso2022_state_new(NULL,
							&_deepinvte_terminal_codeset_changed_cb,
							terminal);
	_deepinvte_iso2022_state_set_codeset(pvt->iso2022,
				       pvt->encoding);
	...
	if (clear_history) {
		_deepinvte_ring_fini(pvt->normal_screen.row_data);
		_deepinvte_ring_init(pvt->normal_screen.row_data, pvt->scrollback_lines);
		_deepinvte_ring_fini(pvt->alternate_screen.row_data);
		_deepinvte_ring_init(pvt->alternate_screen.row_data, terminal->row_count);
 ...
 */
const char *
deepinvte_terminal_get_status_line(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
...
/**
 * deepinvte_terminal_get_padding:
 * @terminal: a #DeepinvteTerminal
 ...
 *
 * Deprecated: 0.26: Get the #DeepinvteTerminal:inner-border style property instead
 */
void
deepinvte_terminal_get_padding(DeepinvteTerminal *terminal, int *xpad, int *ypad)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
...
/**
 * deepinvte_terminal_get_adjustment:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
GtkAdjustment *
deepinvte_terminal_get_adjustment(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
...
/**
 * deepinvte_terminal_get_char_width:
 * @terminal: a #DeepinvteTerminal
 *
 * Returns: the width of a character cell
 */
glong
deepinvte_terminal_get_char_width(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), -1);
	deepinvte_terminal_ensure_font (terminal);
...
/**
 * deepinvte_terminal_get_char_height:
 * @terminal: a #DeepinvteTerminal
 *
 * Returns: the height of a character cell
 */
glong
deepinvte_terminal_get_char_height(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), -1);
	deepinvte_terminal_ensure_font (terminal);
...
/**
 * deepinvte_terminal_get_char_descent:
 * @terminal: a #DeepinvteTerminal
 *
 * An accessor function provided for the benefit of language bindings.
 *
 ...
 */
glong
deepinvte_terminal_get_char_descent(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), -1);
	deepinvte_terminal_ensure_font (terminal);
 ...
 */
glong
deepinvte_terminal_get_char_ascent(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), -1);
	deepinvte_terminal_ensure_font (terminal);
...
/**
 * deepinvte_terminal_get_row_count:
 * @terminal: a #DeepinvteTerminal
 *
 *
 ...
 */
glong
deepinvte_terminal_get_row_count(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), -1);
...
/**
 * deepinvte_terminal_get_column_count:
 * @terminal: a #DeepinvteTerminal
 *
 * Returns: the number of columns
 */
glong
deepinvte_terminal_get_column_count(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), -1);
...
/**
 * deepinvte_terminal_get_window_title:
 * @terminal: a #DeepinvteTerminal
 *
 * Returns: (transfer none): the window title
 */
const char *
deepinvte_terminal_get_window_title(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), "");
	return terminal->window_title;
}

/**
 * deepinvte_terminal_get_icon_title:
 * @terminal: a #DeepinvteTerminal
 *
 * Returns: (transfer none): the icon title
 */
const char *
deepinvte_terminal_get_icon_title(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), "");
 ...
 * instead of deepinvte_terminal_fork_command() or deepinvte_terminal_forkpty().
 *
 * Since: 0.12.1
 *
 * Deprecated: 0.26: Use deepinvte_pty_new_foreign() and deepinvte_terminal_set_pty_object()
 */
void
deepinvte_terminal_set_pty(DeepinvteTerminal *terminal, int pty_master)
{
        DeepinvtePty *pty;
        ...
        pty = deepinvte_pty_new_foreign(pty_master, NULL);
        if (pty == NULL)
                return;
...
/**
 * deepinvte_terminal_set_pty_object:
 * @terminal: a #DeepinvteTerminal
 * @pty: (allow-none): a #DeepinvtePty, or %NULL
 *
 ...
 */
void
deepinvte_terminal_set_pty_object(DeepinvteTerminal *terminal,
                            DeepinvtePty *pty)
{
        DeepinvteTerminalPrivate *pvt;
        GObject *object;
        ...
        g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
        g_return_if_fail(pty == NULL || DEEPINVTE_IS_PTY(pty));
		...
		if (terminal->pvt->incoming != NULL) {
			deepinvte_terminal_process_incoming(terminal);
			_deepinvte_incoming_chunks_release (terminal->pvt->incoming);
        ...
        _deepinvte_terminal_setup_utf8 (terminal);
 ...
 * Since: 0.20
 *
 * Deprecated: 0.26: Use deepinvte_terminal_get_pty_object() and deepinvte_pty_get_fd()
 */
int
deepinvte_terminal_get_pty(DeepinvteTerminal *terminal)
{
        DeepinvteTerminalPrivate *pvt;
        ...
        pvt = terminal->pvt;
        if (pvt->pty != NULL)
                return deepinvte_pty_get_fd(pvt->pty);

        return -1;
}

/**
 * deepinvte_terminal_get_pty_object:
 * @terminal: a #DeepinvteTerminal
 *
 * Returns the #DeepinvtePty of @terminal.
 *
 * Returns: (transfer none): a #DeepinvtePty, or %NULL
 *
 * Since: 0.26
 */
DeepinvtePty *
deepinvte_terminal_get_pty_object(DeepinvteTerminal *terminal)
{
        g_return_val_if_fail (DEEPINVTE_IS_TERMINAL (terminal), NULL);
...
/**
 * deepinvte_terminal_get_child_exit_status:
 * @terminal: a #DeepinvteTerminal
 *
 * Gets the exit status of the command started by deepinvte_terminal_fork_command().
 ...
 * the #DeepinvteTerminal::child-exited signal.
 * 
 ...
 */
int
deepinvte_terminal_get_child_exit_status(DeepinvteTerminal *terminal)
{
        g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), -1);
 ...
 * get signals. */
void
_deepinvte_terminal_accessible_ref(DeepinvteTerminal *terminal)
{
	g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
...
char *
_deepinvte_terminal_get_selection(DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
...
void
_deepinvte_terminal_get_start_selection(DeepinvteTerminal *terminal, long *col, long *row)
{
	DeepinvteVisualPosition ss;
...
void
_deepinvte_terminal_get_end_selection(DeepinvteTerminal *terminal, long *col, long *row)
{
	DeepinvteVisualPosition se;
...
void
_deepinvte_terminal_select_text(DeepinvteTerminal *terminal,
...
void
_deepinvte_terminal_remove_selection(DeepinvteTerminal *terminal)
{
	deepinvte_terminal_deselect_all (terminal);
}

static void
_deepinvte_terminal_select_empty_at(DeepinvteTerminal *terminal,
			      long col, long row)
{
	_deepinvte_terminal_select_text(terminal, col, row, col - 1, row, 0, 0);
}

static void
add_update_timeout (DeepinvteTerminal *terminal)
{
	if (update_timeout_tag == 0) {
		_deepinvte_debug_print (DEEPINVTE_DEBUG_TIMEOUT,
				"Starting update timeout\n");
		update_timeout_tag =
			g_timeout_add_full (GDK_PRIORITY_REDRAW,
					DEEPINVTE_UPDATE_TIMEOUT,
					update_timeout, NULL,
					NULL);
	}
...
static void
reset_update_regions (DeepinvteTerminal *terminal)
{
...
static void
remove_from_active_list (DeepinvteTerminal *terminal)
{
	if (terminal->pvt->active != NULL
			&& terminal->pvt->update_regions == NULL) {
		_deepinvte_debug_print(DEEPINVTE_DEBUG_TIMEOUT,
...
static void
remove_update_timeout (DeepinvteTerminal *terminal)
{
	reset_update_regions (terminal);
...
static void
deepinvte_terminal_add_process_timeout (DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_TIMEOUT,
			"Adding terminal to active list\n");
	terminal->pvt->active = active_terminals =
	...
	if (update_timeout_tag == 0 &&
		...
		process_timeout_tag =
			g_timeout_add (DEEPINVTE_DISPLAY_TIMEOUT,
					process_timeout, NULL);
	}
}
static inline gboolean
deepinvte_terminal_is_processing (DeepinvteTerminal *terminal)
{
	return terminal->pvt->active != NULL;
}
static inline void
deepinvte_terminal_start_processing (DeepinvteTerminal *terminal)
{
	if (!deepinvte_terminal_is_processing (terminal)) {
		deepinvte_terminal_add_process_timeout (terminal);
	}
...
static void
deepinvte_terminal_stop_processing (DeepinvteTerminal *terminal)
{
	remove_from_active_list (terminal);
}

static inline gboolean
need_processing (DeepinvteTerminal *terminal)
{
	return _deepinvte_incoming_chunks_length (terminal->pvt->incoming) != 0;
}

/* Emit an "icon-title-changed" signal. */
static void
deepinvte_terminal_emit_icon_title_changed(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
static void
deepinvte_terminal_emit_window_title_changed(DeepinvteTerminal *terminal)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS,
...
static void
deepinvte_terminal_emit_pending_signals(DeepinvteTerminal *terminal)
{
        GObject *object;
	GdkWindow *window;
	...
	deepinvte_terminal_emit_adjustment_changed (terminal);

	if (terminal->pvt->screen->status_line_changed) {
		_deepinvte_terminal_emit_status_line_changed (terminal);
	...
	if (terminal->pvt->window_title_changed) {
		...
		deepinvte_terminal_emit_window_title_changed(terminal);
	...
	if (terminal->pvt->icon_title_changed) {
		...
		deepinvte_terminal_emit_icon_title_changed(terminal);
                g_object_notify(object, "icon-title");
	}

	/* Flush any pending "inserted" signals. */
	deepinvte_terminal_emit_cursor_moved(terminal);
	deepinvte_terminal_emit_pending_text_signals(terminal, 0);
	deepinvte_terminal_emit_contents_changed (terminal);

        g_object_thaw_notify(object);
}

static void time_process_incoming (DeepinvteTerminal *terminal)
{
	gdouble elapsed;
	glong target;
	...
	target = DEEPINVTE_MAX_PROCESS_TIME / elapsed * terminal->pvt->input_bytes;
...
process_timeout (gpointer data)
{
	...
	for (l = active_terminals; l != NULL; l = next) {
		DeepinvteTerminal *terminal = l->data;
		gboolean active = FALSE;

		next = g_list_next (l);

		if (l != active_terminals) {
			_deepinvte_debug_print (DEEPINVTE_DEBUG_WORK, "T");
		}
		if (terminal->pvt->pty_channel != NULL) {
			if (terminal->pvt->pty_input_active ||
					terminal->pvt->pty_input_source == 0) {
				terminal->pvt->pty_input_active = FALSE;
				deepinvte_terminal_io_read (terminal->pvt->pty_channel,
						G_IO_IN, terminal);
			}
			_deepinvte_terminal_enable_input_source (terminal);
		}
		if (need_processing (terminal)) {
			active = TRUE;
			if (DEEPINVTE_MAX_PROCESS_TIME) {
...
static gboolean
update_regions (DeepinvteTerminal *terminal)
{
	GSList *l;
	GdkRegion *region;
	GdkWindow *window;
...
update_timeout (gpointer data)
{
	...
	update_timeout_tag =
		g_timeout_add_full (G_PRIORITY_DEFAULT_IDLE,
				    DEEPINVTE_UPDATE_REPEAT_TIMEOUT,
...
/**
 * deepinvte_terminal_write_contents:
 * @terminal: a #DeepinvteTerminal
 ...
 */
gboolean
deepinvte_terminal_write_contents (DeepinvteTerminal *terminal,
                             ...
                             GError **error)
{
	...
	return _deepinvte_ring_write_contents (terminal->pvt->screen->row_data,
					 stream, flags,
					 cancellable, error);
}
...
/**
 * deepinvte_terminal_search_set_gregex:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_search_set_gregex (DeepinvteTerminal *terminal,
				GRegex      *regex)
{
        g_return_if_fail(DEEPINVTE_IS_TERMINAL(terminal));
...
/**
 * deepinvte_terminal_search_get_gregex:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
GRegex *
deepinvte_terminal_search_get_gregex (DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), NULL);
...
/**
 * deepinvte_terminal_search_set_wrap_around:
 * @terminal: a #DeepinvteTerminal
 ...
 */
void
deepinvte_terminal_search_set_wrap_around (DeepinvteTerminal *terminal,
...
/**
 * deepinvte_terminal_search_get_wrap_around:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
gboolean
deepinvte_terminal_search_get_wrap_around (DeepinvteTerminal *terminal)
{
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL(terminal), FALSE);
...
static gboolean
deepinvte_terminal_search_rows (DeepinvteTerminal *terminal,
			  long start_row,
			  long end_row,
			  gboolean backward)
{
	...
	gchar *word;
	DeepinvteCharAttributes *ca;
	GArray *attrs;
	gdouble value, page_size;

	pvt = terminal->pvt;

	row_text = deepinvte_terminal_get_text_range (terminal, start_row, 0, end_row, -1, NULL, NULL, NULL);
	...
	g_free (row_text);
	if (!pvt->search_attrs)
		pvt->search_attrs = g_array_new (FALSE, TRUE, sizeof (DeepinvteCharAttributes));
	attrs = pvt->search_attrs;
	row_text = deepinvte_terminal_get_text_range (terminal, start_row, 0, end_row, -1, NULL, NULL, attrs);

	/* This gives us the offset in the buffer */
	g_match_info_fetch_pos (match_info, 0, &start, &end);

	ca = &g_array_index (attrs, DeepinvteCharAttributes, start);
	start_row = ca->row;
	start_col = ca->column;
	ca = &g_array_index (attrs, DeepinvteCharAttributes, end - 1);
	end_row = ca->row;
	end_col = ca->column;
	...
	_deepinvte_terminal_select_text (terminal, start_col, start_row, end_col, end_row, 0, 0);
	...
	if (backward) {
		if (end_row < value || end_row >= value + page_size)
			deepinvte_terminal_queue_adjustment_value_changed_clamped (terminal, end_row - page_size + 1);
	} else {
		if (start_row < value || start_row >= value + page_size)
			deepinvte_terminal_queue_adjustment_value_changed_clamped (terminal, start_row);
	}
...
static gboolean
deepinvte_terminal_search_rows_iter (DeepinvteTerminal *terminal,
			       long start_row,
			       long end_row,
			       gboolean backward)
{
	const DeepinvteRowData *row;
	...
	if (backward) {
		...
		while (iter_start_row > start_row) {
			...
			do {
				iter_start_row--;
				row = _deepinvte_terminal_find_row_data (terminal, iter_start_row);
			} while (row && row->attr.soft_wrapped);

			if (deepinvte_terminal_search_rows (terminal, iter_start_row, iter_end_row, backward))
				return TRUE;
		}
	} else {
		...
		while (iter_end_row < end_row) {
			...
			do {
				row = _deepinvte_terminal_find_row_data (terminal, iter_end_row);
				iter_end_row++;
			} while (row && row->attr.soft_wrapped);
...
static gboolean
deepinvte_terminal_search_find (DeepinvteTerminal *terminal,
			  gboolean     backward)
{
        DeepinvteTerminalPrivate *pvt;
	...
	buffer_start_row = _deepinvte_ring_delta (terminal->pvt->screen->row_data);
	buffer_end_row = _deepinvte_ring_next (terminal->pvt->screen->row_data);
	 ...
	 * position... */
	if (backward) {
		if (deepinvte_terminal_search_rows_iter (terminal, buffer_start_row, last_start_row, backward))
			return TRUE;
		if (pvt->search_wrap_around &&
		    deepinvte_terminal_search_rows_iter (terminal, last_end_row, buffer_end_row, backward))
			return TRUE;
		if (pvt->has_selection) {
			if (pvt->search_wrap_around)
			    _deepinvte_terminal_select_empty_at (terminal,
		...
		if (deepinvte_terminal_search_rows_iter (terminal, last_end_row, buffer_end_row, backward))
			return TRUE;
		if (pvt->search_wrap_around &&
		    deepinvte_terminal_search_rows_iter (terminal, buffer_start_row, last_start_row, backward))
			return TRUE;
		if (pvt->has_selection) {
			if (pvt->search_wrap_around)
			    _deepinvte_terminal_select_empty_at (terminal,
...
/**
 * deepinvte_terminal_search_find_previous:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
gboolean
deepinvte_terminal_search_find_previous (DeepinvteTerminal *terminal)
{
	return deepinvte_terminal_search_find (terminal, TRUE);
}

/**
 * deepinvte_terminal_search_find_next:
 * @terminal: a #DeepinvteTerminal
 *
 ...
 */
gboolean
deepinvte_terminal_search_find_next (DeepinvteTerminal *terminal)
{
	return deepinvte_terminal_search_find (terminal, FALSE);
}
