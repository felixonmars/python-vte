 ...
 */
typedef struct _deepinvte_termcap
{
  GMappedFile *file;
  GTree *tree;
  const char *end;
 ...
 */
static int
_deepinvte_termcap_strcmp (const char *a,
                     const char *b,
                     const char *enders)
{
 ...
 */
static const char *
_deepinvte_termcap_find_start (DeepinvteTermcap *termcap,
                         const char *tname,
                         const char *cap)
{
  const char *contents;
  ...
  while (contents != termcap->end)
  {
    ...
    {
      if (!_deepinvte_termcap_strcmp (start, cap, "=#:\n"))
        return start;
  ...
  chain = _deepinvte_termcap_find_string (termcap, tname, "tc");
  if (chain[0])
    start = _deepinvte_termcap_find_start (termcap, chain, cap);
  g_free (chain);
...
static int
_deepinvte_termcap_unescape_string(const char *string, char *result)
{
  int value = -1;
  int length = 0;
...
char *
_deepinvte_termcap_find_string_length (DeepinvteTermcap *termcap,
                                 const char *tname,
                                 const char *cap,
                                 gssize *length)
{
  const char *result = _deepinvte_termcap_find_start (termcap, tname, cap);
  char *string;
  ...
  *length = _deepinvte_termcap_unescape_string (result, NULL);
  string = g_malloc (*length);
  _deepinvte_termcap_unescape_string (result, string);
...
char *
_deepinvte_termcap_find_string (DeepinvteTermcap *termcap,
                          const char *tname,
                          const char *cap)
{
  gssize length;

  return _deepinvte_termcap_find_string_length (termcap, tname, cap, &length);
}

long
_deepinvte_termcap_find_numeric (DeepinvteTermcap *termcap,
                           const char *tname,
                           const char *cap)
{
  const char *result = _deepinvte_termcap_find_start (termcap, tname, cap);
...
gboolean
_deepinvte_termcap_find_boolean (DeepinvteTermcap *termcap,
                           const char *tname,
                           const char *cap)
{
  const char *result = _deepinvte_termcap_find_start (termcap, tname, cap);
 ...
 */
static void
_deepinvte_termcap_parse_entry (GTree *termcap, const char **cnt, const char *end)
{
  ...
  while (contents != end)
  {
    ...
    */
    ...
    else if (*contents == '|' || *contents == ':' || *contents == '\n')
    {
       ...
       * the tree to ensure that _deepinvte_termcap_strcmp will always
...
static GTree *
_deepinvte_termcap_parse_file (const char *contents, int length)
{
   ...
   * us to notice the end of strings passed to us by deepinvte.
   */
  termcap = g_tree_new_full ((GCompareDataFunc) _deepinvte_termcap_strcmp,
                             (gpointer)":|\n", NULL, NULL);
  ...
  while (contents != end)
  {
    switch (*contents++)
    {
      ...
      default:
        ...
        _deepinvte_termcap_parse_entry (termcap, &contents, end);
    }
  }
...
static DeepinvteTermcap *
_deepinvte_termcap_create (const char *filename)
{
  const char *contents;
  DeepinvteTermcap *termcap;
  GMappedFile *file;
  int length;
  ...
  termcap = g_slice_new (DeepinvteTermcap);
  termcap->file = file;
  termcap->tree = _deepinvte_termcap_parse_file (contents, length);
...
static void
_deepinvte_termcap_destroy (DeepinvteTermcap *termcap)
{
  if (!termcap)
    return;
  g_tree_destroy (termcap->tree);
  g_mapped_file_unref (termcap->file);
  g_slice_free (DeepinvteTermcap, termcap);
}

/*
 * --- cached interface to create/destroy termcap trees ---
 */
static GStaticMutex _deepinvte_termcap_mutex = G_STATIC_MUTEX_INIT;
static GCache *_deepinvte_termcap_cache = NULL;

DeepinvteTermcap *
_deepinvte_termcap_new(const char *filename)
{
  DeepinvteTermcap *result;

  g_static_mutex_lock (&_deepinvte_termcap_mutex);

  if (_deepinvte_termcap_cache == NULL)
    _deepinvte_termcap_cache = g_cache_new((GCacheNewFunc) _deepinvte_termcap_create,
                                     (GCacheDestroyFunc) _deepinvte_termcap_destroy,
                                     (GCacheDupFunc) g_strdup,
                                     (GCacheDestroyFunc) g_free,
                                     g_str_hash, g_direct_hash, g_str_equal);

  result = g_cache_insert (_deepinvte_termcap_cache, (gpointer) filename);

  g_static_mutex_unlock (&_deepinvte_termcap_mutex);

  return result;
}

void
_deepinvte_termcap_free (DeepinvteTermcap *termcap)
{
  g_static_mutex_lock (&_deepinvte_termcap_mutex);
  g_cache_remove (_deepinvte_termcap_cache, termcap);
  g_static_mutex_unlock (&_deepinvte_termcap_mutex);
}
...
main (int argc, char **argv)
{
  ...
  if (argc < 4)
  {
    g_printerr("deepinvtetc /path/to/termcap termname attrs...\n"
                     "  where attrs are\n"
                     "    :xx for boolean\n"
                     "    =xx for string\n"
  ...
  tc = _deepinvte_termcap_new (argv[1]);
  ...
  for (i = 3; i < argc; i++)
  {
    ...
    {
      case ':':
        printf ("%s\n", _deepinvte_termcap_find_boolean (tc, argv[2], argv[i] + 1)?
                        "true" : "false");
        break;

      case '=':
      case '+':
        str = _deepinvte_termcap_find_string_length (tc, argv[2], argv[i] + 1, &len);
      ...
      case '#':
        printf ("%ld\n", _deepinvte_termcap_find_numeric (tc, argv[2], argv[i] + 1));
        break;
