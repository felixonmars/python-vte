...
G_DEFINE_TYPE(DeepinvteBg, deepinvte_bg, G_TYPE_OBJECT)

struct _DeepinvteBgPrivate {
	GList *cache;
	GdkScreen *screen;
#ifdef GDK_WINDOWING_X11
...
} DeepinvteBgCacheItem;

static void deepinvte_bg_cache_item_free(DeepinvteBgCacheItem *item);
static void deepinvte_bg_cache_prune_int(DeepinvteBg *bg, gboolean root);
static const cairo_user_data_key_t item_surface_key;

#ifdef GDK_WINDOWING_X11

static void
_deepinvte_bg_display_sync(DeepinvteBg *bg)
{
        DeepinvteBgPrivate *pvt = bg->pvt;

	gdk_display_sync(pvt->native.display);
}

static gboolean
_deepinvte_property_get_pixmaps(GdkWindow *window, GdkAtom atom,
...
deepinvte_bg_root_surface(DeepinvteBg *bg)
{
        DeepinvteBgPrivate *pvt = bg->pvt;
	GdkAtom prop_type;
	int prop_size;
	...
	pixmaps = NULL;
	gdk_error_trap_push();
	if (!_deepinvte_property_get_pixmaps(pvt->native.window, pvt->native.atom,
                                       &prop_type, &prop_size,
                                       &pixmaps))
		goto out;
	...
	surface = cairo_xlib_surface_create (display,
        ...
        _deepinvte_debug_print(DEEPINVTE_DEBUG_BG|DEEPINVTE_DEBUG_EVENTS,
                         "DeepinvteBg new background image %dx%d\n", width, height);

 out_pixmaps:
	g_free(pixmaps);
 out:
	_deepinvte_bg_display_sync(bg);
...
static void
deepinvte_bg_set_root_surface(DeepinvteBg *bg, cairo_surface_t *surface)
{
        DeepinvteBgPrivate *pvt = bg->pvt;
	...
	deepinvte_bg_cache_prune_int (bg, TRUE);
	g_signal_emit_by_name(bg, "root-pixmap-changed");
}

static GdkFilterReturn
deepinvte_bg_root_filter(GdkXEvent *native, GdkEvent *event, gpointer data)
{
	...
	switch (xevent->type) {
	case PropertyNotify:
		bg = DEEPINVTE_BG(data);
                pvt = bg->pvt;
		if ((xevent->xproperty.window == pvt->native.native_window) &&
		    (xevent->xproperty.atom == pvt->native.native_atom)) {
			surface = deepinvte_bg_root_surface(bg);
			deepinvte_bg_set_root_surface(bg, surface);
		}
		break;
	default:
		break;
...
static void
deepinvte_bg_finalize (GObject *obj)
{
	DeepinvteBg *bg = DEEPINVTE_BG (obj);
        DeepinvteBgPrivate *pvt = bg->pvt;

        g_list_foreach (pvt->cache, (GFunc)deepinvte_bg_cache_item_free, NULL);
        g_list_free (pvt->cache);

	G_OBJECT_CLASS(deepinvte_bg_parent_class)->finalize (obj);
}

static void
deepinvte_bg_class_init(DeepinvteBgClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);

	gobject_class->finalize = deepinvte_bg_finalize;
	...
	g_signal_new("root-pixmap-changed",
                     ...
                     G_TYPE_NONE, 0);
	g_type_class_add_private(klass, sizeof (DeepinvteBgPrivate));
}

static void
deepinvte_bg_init(DeepinvteBg *bg)
{
	bg->pvt = G_TYPE_INSTANCE_GET_PRIVATE (bg, DEEPINVTE_TYPE_BG, DeepinvteBgPrivate);
}
 ...
 * @screen: a #GdkScreen
 *
 * Returns the global #DeepinvteBg object for @screen, creating it if necessary.
 *
 * Returns: (transfer none): a #DeepinvteBg
 */
DeepinvteBg *
deepinvte_bg_get_for_screen(GdkScreen *screen)
{
	DeepinvteBg       *bg;

	bg = g_object_get_data(G_OBJECT(screen), "deepinvte-bg");
	if (G_UNLIKELY(bg == NULL)) {
                DeepinvteBgPrivate *pvt;

		bg = g_object_new(DEEPINVTE_TYPE_BG, NULL);
		g_object_set_data_full(G_OBJECT(screen),
				"deepinvte-bg", bg, (GDestroyNotify)g_object_unref);
		...
		pvt->root_surface = deepinvte_bg_root_surface(bg);
		events = gdk_window_get_events(window);
		events |= GDK_PROPERTY_CHANGE_MASK;
		gdk_window_set_events(window, events);
		gdk_window_add_filter(window, deepinvte_bg_root_filter, bg);
            }
#endif /* GDK_WINDOWING_X11 */
	}
...
static gboolean
deepinvte_bg_colors_equal(const PangoColor *a, const PangoColor *b)
{
...
static void
deepinvte_bg_cache_item_free(DeepinvteBgCacheItem *item)
{
        _deepinvte_debug_print(DEEPINVTE_DEBUG_BG,
                         "DeepinvteBgCacheItem %p freed\n", item);
	...
	g_slice_free(DeepinvteBgCacheItem, item);
}

static void
deepinvte_bg_cache_prune_int(DeepinvteBg *bg, gboolean root)
{
	GList *i, *next;
	for (i = bg->pvt->cache; i != NULL; i = next) {
		DeepinvteBgCacheItem *item = i->data;
		next = g_list_next (i);
		...
		if ((root && (item->source_type == DEEPINVTE_BG_SOURCE_ROOT)) ||
		    item->surface == NULL) {
			deepinvte_bg_cache_item_free (item);
...
static void
deepinvte_bg_cache_prune(DeepinvteBg *bg)
{
	deepinvte_bg_cache_prune_int(bg, FALSE);
}

static void item_surface_destroy_func(void *data)
{
	DeepinvteBgCacheItem *item = data;

        _deepinvte_debug_print(DEEPINVTE_DEBUG_BG,
                         "DeepinvteBgCacheItem %p surface destroyed\n", item);

	item->surface = NULL;
}

/*
 * deepinvte_bg_cache_add:
 * @bg: a #DeepinvteBg
 * @item: a #DeepinvteBgCacheItem
 *
 ...
 */
static void
deepinvte_bg_cache_add(DeepinvteBg *bg, DeepinvteBgCacheItem *item)
{
	deepinvte_bg_cache_prune(bg);
...
/*
 * deepinvte_bg_cache_search:
 * @bg: a #DeepinvteBg
 * @source_type: a #DeepinvteBgSourceType
 ...
 */
static cairo_surface_t *
deepinvte_bg_cache_search(DeepinvteBg *bg,
		    DeepinvteBgSourceType source_type,
		    ...
		    double saturation)
{
	...
	for (i = bg->pvt->cache; i != NULL; i = g_list_next(i)) {
		DeepinvteBgCacheItem *item = i->data;
		if (deepinvte_bg_colors_equal(&item->tint_color, tint) &&
		    (saturation == item->saturation) &&
		    (source_type == item->source_type)) {
			switch (source_type) {
			case DEEPINVTE_BG_SOURCE_ROOT:
				break;
			case DEEPINVTE_BG_SOURCE_PIXBUF:
				if (item->source_pixbuf != source_pixbuf) {
					continue;
				}
				break;
			case DEEPINVTE_BG_SOURCE_FILE:
...
{
        DeepinvteBgPrivate *pvt;
	DeepinvteBgCacheItem *item;
	GdkPixbuf *pixbuf;
	cairo_surface_t *cached;
	cairo_t *cr;
	int width, height;

        g_return_val_if_fail(DEEPINVTE_IS_BG(bg), NULL);
        pvt = bg->pvt;

	if (source_type == DEEPINVTE_BG_SOURCE_NONE) {
		return NULL;
	}
#ifndef GDK_WINDOWING_X11
        if (source_type == DEEPINVTE_BG_SOURCE_ROOT) {
                return NULL;
        }
#endif

	cached = deepinvte_bg_cache_search(bg, source_type,
	...
	if (cached != NULL) {
         ...
         * tint and saturation matched. This means that for DEEPINVTE_BG_SOURCE_FILE,
         ...
         * of tint and saturation, and just create a new #DeepinvteBgCacheItem
         * with a new surface for it.
         */

	item = g_slice_new(DeepinvteBgCacheItem);
...
#ifdef GDK_WINDOWING_X11
        else if (source_type == DEEPINVTE_BG_SOURCE_ROOT &&
    ...
    out:
	deepinvte_bg_cache_add(bg, item);
