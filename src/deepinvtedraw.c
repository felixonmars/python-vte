 ...
 *
 *
 * This file implements deepinvte rendering using pangocairo.  Note that this does
 ...
 *     information needed to quickly draw a single deepinvteunistr.  The font_info
 ...
 *     the deepinvteunistr as determined by Pango consists of a single regular glyph
 ...
 *     weird and exceptional case that a single deepinvteunistr uses more than one
 ...
 * that's how deepinvte.c works and we only get called from there. */
 ...
 *
 * Moreover, setting it larger than %DEEPINVTE_DRAW_MAX_LENGTH is nonsensical,
...
#ifdef DEEPINVTE_DEBUG
	_deepinvte_debug_print (DEEPINVTE_DEBUG_PANGOCAIRO,
			  "deepinvtepangocairo: %p cached %d ASCII letters\n",
...
font_info_measure_font (struct font_info *info)
{
	...
	pango_layout_set_text (info->layout, DEEPINVTE_DRAW_SINGLE_WIDE_CHARACTERS, -1);
	...
	info->width  = PANGO_PIXELS (howmany (logical.width, strlen(DEEPINVTE_DRAW_SINGLE_WIDE_CHARACTERS)));
	...
	_deepinvte_debug_print (DEEPINVTE_DEBUG_MISC,
			  "deepinvtepangocairo: %p font metrics = %dx%d (%d)\n",
...
font_info_allocate (PangoContext *context)
{
	...
	_deepinvte_debug_print (DEEPINVTE_DEBUG_PANGOCAIRO,
			  "deepinvtepangocairo: %p allocating font_info\n",
			  info);
	...
	info->string = g_string_sized_new (DEEPINVTE_UTF8_BPC+1);
...
#ifdef DEEPINVTE_DEBUG
	_deepinvte_debug_print (DEEPINVTE_DEBUG_PANGOCAIRO,
			  "deepinvtepangocairo: %p freeing font_info.  coverages %d = %d + %d + %d\n",
			  info,
			  info->coverage_count[0],
...
fontconfig_timestamp_quark (void)
{
	...
	if (G_UNLIKELY (!quark))
		quark = g_quark_from_static_string ("deepinvte-fontconfig-timestamp");

	return quark;
}

static void
deepinvte_pango_context_set_fontconfig_timestamp (PangoContext *context,
...
static guint
deepinvte_pango_context_get_fontconfig_timestamp (PangoContext *context)
{
...
context_hash (PangoContext *context)
{
	return pango_units_from_double (pango_cairo_context_get_resolution (context))
	     ...
	     ^ deepinvte_pango_context_get_fontconfig_timestamp (context);
}
	       ...
	       PangoContext *b)
{
	return pango_cairo_context_get_resolution (a) == pango_cairo_context_get_resolution (b)
	    ...
	    && deepinvte_pango_context_get_fontconfig_timestamp (a) == deepinvte_pango_context_get_fontconfig_timestamp (b);
}
...
font_info_find_for_context (PangoContext *context)
{
	...
	if (G_LIKELY (info)) {
		_deepinvte_debug_print (DEEPINVTE_DEBUG_PANGOCAIRO,
				  "deepinvtepangocairo: %p found font_info in cache\n",
				  info);
			      ...
			      guint                       fontconfig_timestamp)
{
	...
	deepinvte_pango_context_set_fontconfig_timestamp (context, fontconfig_timestamp);
	...
	case DEEPINVTE_ANTI_ALIAS_FORCE_ENABLE:
	case DEEPINVTE_ANTI_ALIAS_FORCE_DISABLE:

		if (antialias == DEEPINVTE_ANTI_ALIAS_FORCE_ENABLE)
	...
	default:
	case DEEPINVTE_ANTI_ALIAS_USE_DEFAULT:
			   ...
			   deepinvteunistr c)
{
	...
	_deepinvte_unistr_append_to_string (c, info->string);
...
struct _deepinvte_draw *
_deepinvte_draw_new (GtkWidget *widget)
{
	struct _deepinvte_draw *draw;

	/* Create the structure. */
	draw = g_slice_new0 (struct _deepinvte_draw);
	draw->widget = g_object_ref (widget);

	_deepinvte_debug_print (DEEPINVTE_DEBUG_DRAW, "draw_new\n");

	return draw;
}

void
_deepinvte_draw_free (struct _deepinvte_draw *draw)
{
	_deepinvte_debug_print (DEEPINVTE_DEBUG_DRAW, "draw_free\n");
	...
	g_slice_free (struct _deepinvte_draw, draw);
}

void
_deepinvte_draw_start (struct _deepinvte_draw *draw)
{
	GdkWindow *window;

	g_return_if_fail (gtk_widget_get_realized (draw->widget));

	_deepinvte_debug_print (DEEPINVTE_DEBUG_DRAW, "draw_start\n");
...
void
_deepinvte_draw_end (struct _deepinvte_draw *draw)
{
	...
	_deepinvte_debug_print (DEEPINVTE_DEBUG_DRAW, "draw_end\n");
}

void
_deepinvte_draw_set_background_solid(struct _deepinvte_draw *draw,
			       double red,
			       double green,
			       double blue,
			       double opacity)
...
void
_deepinvte_draw_set_background_image (struct _deepinvte_draw *draw,
			        ...
			        double saturation)
{
	...
	_deepinvte_draw_start (draw);

	surface = deepinvte_bg_get_surface (deepinvte_bg_get_for_screen (gtk_widget_get_screen (draw->widget)),
				     type, pixbuf, filename,
				     color, saturation,
				     cairo_get_target(draw->cr));

	_deepinvte_draw_end (draw);
...
void
_deepinvte_draw_set_background_scroll (struct _deepinvte_draw *draw,
				 gint x, gint y)
{
	cairo_matrix_t matrix;

	_deepinvte_debug_print (DEEPINVTE_DEBUG_DRAW,
			"draw_set_scroll (%d, %d)\n",
			x, y);
...
void
_deepinvte_draw_clip (struct _deepinvte_draw *draw, GdkRegion *region)
{
	_deepinvte_debug_print (DEEPINVTE_DEBUG_DRAW, "draw_clip\n");
	gdk_cairo_region(draw->cr, region);
	cairo_clip (draw->cr);
}

void
_deepinvte_draw_clear (struct _deepinvte_draw *draw, gint x, gint y, gint width, gint height)
{
	g_return_if_fail (draw->bg_pattern != NULL);

	_deepinvte_debug_print (DEEPINVTE_DEBUG_DRAW, "draw_clear (%d, %d, %d, %d)\n",
			  x,y,width, height);
...
void
_deepinvte_draw_set_text_font (struct _deepinvte_draw *draw,
			const PangoFontDescription *fontdesc,
			DeepinvteTerminalAntiAlias antialias)
{
	PangoFontDescription *bolddesc = NULL;

	_deepinvte_debug_print (DEEPINVTE_DEBUG_DRAW, "draw_set_text_font (aa=%d)\n",
			  antialias);
...
void
_deepinvte_draw_get_text_metrics(struct _deepinvte_draw *draw,
...
int
_deepinvte_draw_get_char_width (struct _deepinvte_draw *draw, deepinvteunistr c, int columns,
			  gboolean bold)
{
	struct unistr_info *uinfo;
...
static gboolean
_deepinvte_draw_has_bold (struct _deepinvte_draw *draw)
{
...
static void
_deepinvte_draw_text_internal (struct _deepinvte_draw *draw,
			 ...
			 const PangoColor *color, guchar alpha, gboolean bold)
{
	...
	for (i = 0; i < n_requests; i++) {
		deepinvteunistr c = requests[i].c;
		int x = requests[i].x;
...
void
_deepinvte_draw_text (struct _deepinvte_draw *draw,
	       struct _deepinvte_draw_text_request *requests, gsize n_requests,
	       const PangoColor *color, guchar alpha, gboolean bold)
{
	g_return_if_fail (draw->started);

	if (_deepinvte_debug_on (DEEPINVTE_DEBUG_DRAW)) {
	...
	_deepinvte_draw_text_internal (draw, requests, n_requests, color, alpha, bold);

	/* handle fonts that lack a bold face by double-striking */
	if (bold && !_deepinvte_draw_has_bold (draw)) {
		gsize i;
		...
		_deepinvte_draw_text_internal (draw, requests,
...
gboolean
_deepinvte_draw_has_char (struct _deepinvte_draw *draw, deepinvteunistr c, gboolean bold)
{
	struct unistr_info *uinfo;

	_deepinvte_debug_print (DEEPINVTE_DEBUG_DRAW, "draw_has_char ('0x%04X', %s)\n", c,
			  bold ? "bold" : "normal");
...
gboolean
_deepinvte_draw_char (struct _deepinvte_draw *draw,
	       ...
	       const PangoColor *color, guchar alpha, gboolean bold)
{
	...
	has_char =_deepinvte_draw_has_char (draw, request->c, bold);
	if (has_char)
		_deepinvte_draw_text (draw, request, 1, color, alpha, bold);

	return has_char;
}

void
_deepinvte_draw_draw_rectangle (struct _deepinvte_draw *draw,
			 ...
			 const PangoColor *color, guchar alpha)
{
	...
	cairo_set_operator (draw->cr, CAIRO_OPERATOR_OVER);
	cairo_rectangle (draw->cr, x+DEEPINVTE_LINE_WIDTH/2., y+DEEPINVTE_LINE_WIDTH/2., width-DEEPINVTE_LINE_WIDTH, height-DEEPINVTE_LINE_WIDTH);
	set_source_color_alpha (draw->cr, color, alpha);
	cairo_set_line_width (draw->cr, DEEPINVTE_LINE_WIDTH);
	cairo_stroke (draw->cr);
}

void
_deepinvte_draw_fill_rectangle (struct _deepinvte_draw *draw,
			 gint x, gint y, gint width, gint height,
			 const PangoColor *color, guchar alpha)
{
	g_return_if_fail (draw->started);
