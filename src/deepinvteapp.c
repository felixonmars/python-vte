...
#undef DEEPINVTE_DISABLE_DEPRECATED
#include "deepinvte.h"
...
window_title_changed(GtkWidget *widget, gpointer win)
{
	...
	g_assert(DEEPINVTE_TERMINAL(widget));
	g_assert(GTK_IS_WINDOW(win));
	g_assert(DEEPINVTE_TERMINAL(widget)->window_title != NULL);
	window = GTK_WINDOW(win);

	gtk_window_set_title(window, DEEPINVTE_TERMINAL(widget)->window_title);
}
...
icon_title_changed(GtkWidget *widget, gpointer win)
{
	...
	g_assert(DEEPINVTE_TERMINAL(widget));
	g_assert(GTK_IS_WINDOW(win));
	g_assert(DEEPINVTE_TERMINAL(widget)->icon_title != NULL);
	window = GTK_WINDOW(win);

	g_message("Icon title changed to \"%s\".\n",
		  DEEPINVTE_TERMINAL(widget)->icon_title);
}
...
char_size_realized(GtkWidget *widget, gpointer data)
{
	...
	gtk_widget_style_get (widget, "inner-border", &inner_border, NULL);
	width = deepinvte_terminal_get_char_width (terminal);
	height = deepinvte_terminal_get_char_height (terminal);
	geometry.width_inc = width;
	geometry.height_inc = height;
	geometry.base_width = inner_border ? (inner_border->left + inner_border->right) : 0;
	geometry.base_height = inner_border ? (inner_border->top + inner_border->bottom) : 0;
...
static void
destroy_and_quit(DeepinvteTerminal *terminal, GtkWidget *window)
{
	...
	if (output_file) {
		...
		if (stream) {
			deepinvte_terminal_write_contents (terminal, stream,
						     DEEPINVTE_TERMINAL_WRITE_DEFAULT,
						     NULL, &error);
...
{
	destroy_and_quit(DEEPINVTE_TERMINAL (terminal), window);
}
static void
child_exited(GtkWidget *terminal, gpointer window)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_MISC, "Child exited with status %x\n",
			 deepinvte_terminal_get_child_exit_status (DEEPINVTE_TERMINAL (terminal)));
	destroy_and_quit(DEEPINVTE_TERMINAL (terminal), GTK_WIDGET (window));
}
...
{
	g_print("Status = `%s'.\n",
		deepinvte_terminal_get_status_line(DEEPINVTE_TERMINAL(widget)));
}
...
button_pressed(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
	...
	case 3:
		...
		match = deepinvte_terminal_match_check(terminal,
		...
		if (match != NULL) {
			...
			if (GPOINTER_TO_INT(data) != 0) {
				deepinvte_terminal_match_remove(terminal, tag);
			}
		}
		break;
	case 1:
...
resize_window(GtkWidget *widget, guint width, guint height, gpointer data)
{
	...
	if ((GTK_IS_WINDOW(data)) && (width >= 2) && (height >= 2)) {
		...
		char_width = deepinvte_terminal_get_char_width (terminal);
		char_height = deepinvte_terminal_get_char_height (terminal);
		column_count = deepinvte_terminal_get_column_count (terminal);
		row_count = deepinvte_terminal_get_row_count (terminal);
		gtk_widget_style_get (widget, "inner-border", &inner_border, NULL);
...
adjust_font_size(GtkWidget *widget, gpointer data, gint howmuch)
{
	...
	desired = pango_font_description_copy(deepinvte_terminal_get_font(terminal));
	...
	deepinvte_terminal_set_font(terminal, desired);
	gtk_window_resize(GTK_WINDOW(data),
...
read_and_feed(GIOChannel *source, GIOCondition condition, gpointer data)
{
	...
	gsize size;
	GIOStatus status;
	g_assert(DEEPINVTE_IS_TERMINAL(data));
	status = g_io_channel_read_chars(source, buf, sizeof(buf),
					 &size, NULL);
	if ((status == G_IO_STATUS_NORMAL) && (size > 0)) {
		deepinvte_terminal_feed(DEEPINVTE_TERMINAL(data), buf, size);
		return TRUE;
	}
	return FALSE;
}
		   ...
		   gpointer user_data)
{
  ...
  if (!pspec ||
      pspec->owner_type != DEEPINVTE_TYPE_TERMINAL)
    return;
...
static void
child_exit_cb(DeepinvteTerminal *terminal,
		 gpointer user_data)
{
}
...
main(int argc, char **argv)
{
	...
	const GOptionEntry options[]={
		...
		},
		{
			...
			"Print DeepinvteTerminal object notifications",
			NULL
		},
		{
			"output-file", 0, 0,
	...
	GError *error = NULL;
	DeepinvteTerminalCursorBlinkMode cursor_blink_mode = DEEPINVTE_CURSOR_BLINK_SYSTEM;
	DeepinvteTerminalCursorShape cursor_shape = DEEPINVTE_CURSOR_SHAPE_BLOCK;
	GtkPolicyType scrollbar_policy = GTK_POLICY_ALWAYS;
	DeepinvtePtyFlags pty_flags = DEEPINVTE_PTY_DEFAULT;

	/* Have to do this early. */
	if (getenv("DEEPINVTE_PROFILE_MEMORY")) {
		if (atol(getenv("DEEPINVTE_PROFILE_MEMORY")) != 0) {
			g_mem_set_vtable(glib_mem_profiler_table);
		}
	}

	context = g_option_context_new (" - test DEEPINVTE terminal emulation");
	...
	if (cursor_blink_mode_string) {
		cursor_blink_mode = parse_enum(DEEPINVTE_TYPE_TERMINAL_CURSOR_BLINK_MODE, cursor_blink_mode_string);
		g_free(cursor_blink_mode_string);
	}
	if (cursor_shape_string) {
		cursor_shape = parse_enum(DEEPINVTE_TYPE_TERMINAL_CURSOR_SHAPE, cursor_shape_string);
	...
	if (pty_flags_string) {
		pty_flags |= parse_flags(DEEPINVTE_TYPE_PTY_FLAGS, pty_flags_string);
	...
	/* Set some defaults. */
	deepinvte_terminal_set_audible_bell(terminal, audible);
	deepinvte_terminal_set_visible_bell(terminal, !audible);
	deepinvte_terminal_set_cursor_blink_mode(terminal, cursor_blink_mode);
	deepinvte_terminal_set_scroll_background(terminal, scroll);
	deepinvte_terminal_set_scroll_on_output(terminal, FALSE);
	deepinvte_terminal_set_scroll_on_keystroke(terminal, TRUE);
	deepinvte_terminal_set_scrollback_lines(terminal, lines);
	deepinvte_terminal_set_mouse_autohide(terminal, TRUE);
	if (background != NULL) {
		deepinvte_terminal_set_background_image_file(terminal,
						       background);
	}
	if (transparent) {
		deepinvte_terminal_set_background_transparent(terminal,
							TRUE);
	}
	deepinvte_terminal_set_background_tint_color(terminal, &tint);
	deepinvte_terminal_set_colors(terminal, &fore, &back, NULL, 0);
	deepinvte_terminal_set_opacity(terminal, 0xdddd);
	if (highlight_set) {
		deepinvte_terminal_set_color_highlight(terminal,
						 &highlight);
	}
	if (cursor_set) {
		deepinvte_terminal_set_color_cursor(terminal, &cursor);
	}
	if (termcap != NULL) {
		deepinvte_terminal_set_emulation(terminal, termcap);
	}
	deepinvte_terminal_set_cursor_shape(terminal, cursor_shape);

	/* Set the default font. */
	deepinvte_terminal_set_font_from_string_full(terminal, font,
					       antialias ? DEEPINVTE_ANTI_ALIAS_USE_DEFAULT : DEEPINVTE_ANTI_ALIAS_FORCE_DISABLE);
	...
	if (dingus) {
		...
		id = deepinvte_terminal_match_add_gregex(terminal, regex, 0);
		g_regex_unref (regex);
		deepinvte_terminal_match_set_cursor_type(terminal,
						   id, GDK_GUMBY);
	...
	if (!console) {
		if (shell) {
			...
			_DEEPINVTE_DEBUG_IF(DEEPINVTE_DEBUG_MISC)
				deepinvte_terminal_feed(terminal, message, -1);

                        if (command == NULL || *command == '\0')
                                command = deepinvte_get_user_shell ();
			...
			if (!g_shell_parse_argv(command, &command_argc, &command_argv, &err) ||
			    !deepinvte_terminal_fork_command_full(terminal,
							    pty_flags,
							    NULL,
							    command_argv,
							    env_add,
			...
			long i;
			i = deepinvte_terminal_forkpty(terminal,
						 env_add, working_directory,
						 TRUE, TRUE, TRUE);
			switch (i) {
			case -1:
				/* abnormal */
				g_warning("Error in deepinvte_terminal_forkpty(): %s",
					  strerror(errno));
				break;
			case 0:
				/* child */
	...
	if (geometry) {
		...
		gtk_window_set_default_geometry (GTK_WINDOW (window),
						 deepinvte_terminal_get_column_count (terminal),
						 deepinvte_terminal_get_row_count (terminal));
	}
#endif
