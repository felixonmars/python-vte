...
{
	const struct _deepinvte_regex_match *A, *B;
	A = a;
	B = b;
	if (B->rm_so != A->rm_so) {
...
static void
_deepinvte_regex_sort_matches(struct _deepinvte_regex_match *matches, gsize n_matches)
{
	GArray *array;
	if (n_matches <= 1) {
		return;
	}
	array = g_array_new(FALSE, FALSE, sizeof(struct _deepinvte_regex_match));
	g_array_append_vals(array, matches, n_matches);
	g_array_sort(array, compare_matches);
	memmove(matches, array->data,
		n_matches * sizeof(struct _deepinvte_regex_match));
...
struct _deepinvte_regex *
_deepinvte_regex_compile(const char *pattern)
{
	struct _deepinvte_regex *ret;
	int i;

	ret = g_slice_new(struct _deepinvte_regex);
	i = regcomp(&ret->posix_regex, pattern, REG_EXTENDED);
	if (i != 0) {
		g_slice_free(struct _deepinvte_regex, ret);
		return NULL;
	}
	return ret;
}
...
void
_deepinvte_regex_free(struct _deepinvte_regex *regex)
{
	regfree(&regex->posix_regex);
	g_slice_free(struct _deepinvte_regex, regex);
}

int
_deepinvte_regex_exec(struct _deepinvte_regex *regex, const char *string,
	...
	if (ret == 0) {
		...
		for (i = 0; i < nmatch; i++) {
			...
			if (matches[i].rm_so == -1) {
				_deepinvte_regex_sort_matches(matches, i);
				break;
			}
		}
	}
