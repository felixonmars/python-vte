...
/**
 * SECTION: deepinvte-access
 * @short_description: Accessibility peer of #DeepinvteTerminal
 *
 * The accessibility peer of a #DeepinvteTerminal, implementing GNOME's accessibility
 * framework.
 */
...
#define DEEPINVTE_TERMINAL_ACCESSIBLE_PRIVATE_DATA "DeepinvteTerminalAccessiblePrivateData"

typedef struct _DeepinvteTerminalAccessiblePrivate {
...
} DeepinvteTerminalAccessiblePrivate;
...
static gunichar deepinvte_terminal_accessible_get_character_at_offset(AtkText *text,
								gint offset);
static gpointer deepinvte_terminal_accessible_parent_class;

G_DEFINE_TYPE(DeepinvteTerminalAccessibleFactory, deepinvte_terminal_accessible_factory, ATK_TYPE_OBJECT_FACTORY)

static const char *deepinvte_terminal_accessible_action_names[] = {
        "menu",
        NULL
};

static const char *deepinvte_terminal_accessible_action_descriptions[] = {
        "Popup context menu",
        NULL
};

/* Create snapshot private data. */
static DeepinvteTerminalAccessiblePrivate *
deepinvte_terminal_accessible_new_private_data(void)
{
	DeepinvteTerminalAccessiblePrivate *priv;
	priv = g_slice_new0(DeepinvteTerminalAccessiblePrivate);
...
static void
deepinvte_terminal_accessible_free_private_data(DeepinvteTerminalAccessiblePrivate *priv)
{
	gint i;
	...
	g_slice_free(DeepinvteTerminalAccessiblePrivate, priv);
}

static gint
offset_from_xy (DeepinvteTerminalAccessiblePrivate *priv,
		gint x, gint y)
{
	gint offset;
	gint linebreak;
...
static void
xy_from_offset (DeepinvteTerminalAccessiblePrivate *priv,
...
static gboolean
all_selected(DeepinvteTerminal *terminal, glong column, glong row, gpointer data)
{
	return TRUE;
}
...
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_SIGNALS|DEEPINVTE_DEBUG_ALLY,
...
static void
deepinvte_terminal_accessible_update_private_data_if_needed(AtkObject *text,
						      char **old, glong *olen)
{
	DeepinvteTerminal *terminal;
	DeepinvteTerminalAccessiblePrivate *priv;
	struct _DeepinvteCharAttributes attrs;
	char *next, *tmp;
	long row, offset, caret;
	long ccol, crow;
	guint i;

	g_assert(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(text));

	/* Retrieve the private data structure.  It must already exist. */
	priv = g_object_get_data(G_OBJECT(text),
				 DEEPINVTE_TERMINAL_ACCESSIBLE_PRIVATE_DATA);
	g_assert(priv != NULL);
	...
	terminal = DEEPINVTE_TERMINAL(gtk_accessible_get_widget(GTK_ACCESSIBLE(text)));
	if (priv->snapshot_contents_invalid) {
		...
		priv->snapshot_attributes = g_array_new(FALSE, FALSE,
							sizeof(struct _DeepinvteCharAttributes));
		...
		tmp = deepinvte_terminal_get_text_include_trailing_spaces(terminal,
								    all_selected,
								    NULL,
		...
		for (i = 0, row = 0; i < priv->snapshot_characters->len; i++) {
			...
			if ((i == 0) || (attrs.row != row)) {
				_deepinvte_debug_print(DEEPINVTE_DEBUG_ALLY,
	...
	deepinvte_terminal_get_cursor_position(terminal, &ccol, &crow);
	_deepinvte_debug_print(DEEPINVTE_DEBUG_ALLY,
			"Cursor at (%ld, " "%ld).\n", ccol, crow);
...
static void
deepinvte_terminal_accessible_text_modified(DeepinvteTerminal *terminal, gpointer data)
{
	DeepinvteTerminalAccessiblePrivate *priv;
	char *old, *current;
	...
	g_assert(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(data));
	...
	priv->snapshot_contents_invalid = TRUE;
	deepinvte_terminal_accessible_update_private_data_if_needed(ATK_OBJECT(data),
							      &old, &olen);
	g_assert(old != NULL);
...
static void
deepinvte_terminal_accessible_text_scrolled(DeepinvteTerminal *terminal,
				      gint howmuch,
				      gpointer data)
{
	DeepinvteTerminalAccessiblePrivate *priv;
...
static void
deepinvte_terminal_accessible_invalidate_cursor(DeepinvteTerminal *terminal, gpointer data)
{
	DeepinvteTerminalAccessiblePrivate *priv;
...
static void
deepinvte_terminal_accessible_title_changed(DeepinvteTerminal *terminal, gpointer data)
{
	g_assert(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(data));
...
static gboolean
deepinvte_terminal_accessible_focus_in(DeepinvteTerminal *terminal, GdkEventFocus *event,
				 gpointer data)
{
	g_assert(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(data));
	g_assert(DEEPINVTE_IS_TERMINAL(terminal));
...
static gboolean
deepinvte_terminal_accessible_focus_out(DeepinvteTerminal *terminal, GdkEventFocus *event,
				  gpointer data)
{
	g_assert(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(data));
	g_assert(DEEPINVTE_IS_TERMINAL(terminal));
...
static gboolean
deepinvte_terminal_accessible_visibility_notify(DeepinvteTerminal *terminal,
...
static void
deepinvte_terminal_accessible_selection_changed (DeepinvteTerminal *terminal,
					   gpointer data)
{
	g_assert(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(data));
	g_assert(DEEPINVTE_IS_TERMINAL(terminal));
...
static void
deepinvte_terminal_initialize (AtkObject *obj, gpointer data)
{
	DeepinvteTerminal *terminal;
	AtkObject *parent;

	ATK_OBJECT_CLASS (deepinvte_terminal_accessible_parent_class)->initialize (obj, data);

	terminal = DEEPINVTE_TERMINAL (data);

	_deepinvte_terminal_accessible_ref(terminal);

	g_object_set_data(G_OBJECT(obj),
			  DEEPINVTE_TERMINAL_ACCESSIBLE_PRIVATE_DATA,
			  deepinvte_terminal_accessible_new_private_data());

	g_signal_connect(terminal, "text-inserted",
			 G_CALLBACK(deepinvte_terminal_accessible_text_modified),
			 obj);
	...
	g_signal_connect(terminal, "text-scrolled",
			 G_CALLBACK(deepinvte_terminal_accessible_text_scrolled),
			 obj);
	g_signal_connect(terminal, "cursor-moved",
			 G_CALLBACK(deepinvte_terminal_accessible_invalidate_cursor),
			 obj);
	g_signal_connect(terminal, "window-title-changed",
			 G_CALLBACK(deepinvte_terminal_accessible_title_changed),
			 obj);
	g_signal_connect(terminal, "focus-in-event",
			 G_CALLBACK(deepinvte_terminal_accessible_focus_in),
			 obj);
	g_signal_connect(terminal, "focus-out-event",
			 G_CALLBACK(deepinvte_terminal_accessible_focus_out),
			 obj);
	g_signal_connect(terminal, "visibility-notify-event",
			 G_CALLBACK(deepinvte_terminal_accessible_visibility_notify),
			 obj);
	g_signal_connect(terminal, "selection-changed",
			 G_CALLBACK(deepinvte_terminal_accessible_selection_changed),
			 obj);
...
/**
 * deepinvte_terminal_accessible_new:
 ...
 */
AtkObject *
deepinvte_terminal_accessible_new(DeepinvteTerminal *terminal)
{
	...
	object = g_object_new(DEEPINVTE_TYPE_TERMINAL_ACCESSIBLE, NULL);
...
static void
deepinvte_terminal_accessible_finalize(GObject *object)
{
	DeepinvteTerminalAccessiblePrivate *priv;
	GtkAccessible *accessible = NULL;
        GtkWidget *widget;

	_deepinvte_debug_print(DEEPINVTE_DEBUG_ALLY, "Finalizing accessible peer.\n");

	g_assert(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(object));
	...
	if (widget != NULL) {
		...
		g_signal_handlers_disconnect_matched(widget,
						     ...
						     G_SIGNAL_MATCH_DATA,
						     0, 0, NULL,
						     deepinvte_terminal_accessible_title_changed,
						     object);
		g_signal_handlers_disconnect_matched(widget,
						     G_SIGNAL_MATCH_FUNC |
						     G_SIGNAL_MATCH_DATA,
						     0, 0, NULL,
						     deepinvte_terminal_accessible_focus_in,
						     object);
		g_signal_handlers_disconnect_matched(widget,
						     G_SIGNAL_MATCH_FUNC |
						     G_SIGNAL_MATCH_DATA,
	...
	priv = g_object_get_data(object,
				 DEEPINVTE_TERMINAL_ACCESSIBLE_PRIVATE_DATA);
	if (priv != NULL) {
		deepinvte_terminal_accessible_free_private_data(priv);
		g_object_set_data(object,
				  DEEPINVTE_TERMINAL_ACCESSIBLE_PRIVATE_DATA,
				  NULL);
	}
	G_OBJECT_CLASS(deepinvte_terminal_accessible_parent_class)->finalize(object);
}

static gchar *
deepinvte_terminal_accessible_get_text(AtkText *text,
				 gint start_offset, gint end_offset)
{
	...
	deepinvte_terminal_accessible_update_private_data_if_needed(ATK_OBJECT(text),
							      NULL, NULL);
...
static gchar *
deepinvte_terminal_accessible_get_text_somewhere(AtkText *text,
					   gint offset,
					   ...
					   gint *end_offset)
{
	...
	switch (boundary_type) {
		...
		case ATK_TEXT_BOUNDARY_WORD_START:
			...
			while (offset > 0) {
				prev = deepinvte_terminal_accessible_get_character_at_offset(text, offset - 1);
				if (deepinvte_terminal_is_word_char(terminal, prev)) {
					offset--;
				} else {
					break;
				}
			...
			if (direction == direction_next) {
				while (offset < (int) priv->snapshot_characters->len) {
					next = deepinvte_terminal_accessible_get_character_at_offset(text, offset);
					if (deepinvte_terminal_is_word_char(terminal, next)) {
						offset++;
					} else {
						break;
					}
			...
			while (offset < (int) priv->snapshot_characters->len) {
				current = deepinvte_terminal_accessible_get_character_at_offset(text, offset);
				if (deepinvte_terminal_is_word_char(terminal, current)) {
					offset++;
				} else {
					break;
				}
		...
		case ATK_TEXT_BOUNDARY_WORD_END:
			...
			while (offset > 0) {
				prev = deepinvte_terminal_accessible_get_character_at_offset(text, offset - 1);
				if (deepinvte_terminal_is_word_char(terminal, prev) &&
				    !deepinvte_terminal_is_word_char(terminal, current)) {
					break;
				} else {
					offset--;
					current = prev;
	...
	*end_offset = end = CLAMP(end, start, priv->snapshot_characters->len);
	return deepinvte_terminal_accessible_get_text(text, start, end);
}

static gchar *
deepinvte_terminal_accessible_get_text_before_offset(AtkText *text, gint offset,
					       AtkTextBoundary boundary_type,
					       gint *start_offset,
					       gint *end_offset)
{
	...
	deepinvte_terminal_accessible_update_private_data_if_needed(ATK_OBJECT(text),
							      NULL, NULL);
	return deepinvte_terminal_accessible_get_text_somewhere(text,
							  offset,
							  boundary_type,
							  -1,
							  start_offset,
...
static gchar *
deepinvte_terminal_accessible_get_text_after_offset(AtkText *text, gint offset,
					      AtkTextBoundary boundary_type,
					      gint *start_offset,
					      gint *end_offset)
{
...
static gchar *
deepinvte_terminal_accessible_get_text_at_offset(AtkText *text, gint offset,
					   AtkTextBoundary boundary_type,
					   gint *start_offset,
					   gint *end_offset)
{
...
static gunichar
deepinvte_terminal_accessible_get_character_at_offset(AtkText *text, gint offset)
{
	DeepinvteTerminalAccessiblePrivate *priv;
	int mapped;
	char *unichar;
	...
	unichar = deepinvte_terminal_accessible_get_text(text, offset, offset + 1);
...
static gint
deepinvte_terminal_accessible_get_caret_offset(AtkText *text)
{
	DeepinvteTerminalAccessiblePrivate *priv;
...
get_attribute_set (struct _DeepinvteCharAttributes attr)
{
...
static AtkAttributeSet *
deepinvte_terminal_accessible_get_run_attributes(AtkText *text, gint offset,
					   gint *start_offset, gint *end_offset)
{
	DeepinvteTerminalAccessiblePrivate *priv;
	guint i;
	struct _DeepinvteCharAttributes cur_attr;
	struct _DeepinvteCharAttributes attr;
...
static AtkAttributeSet *
deepinvte_terminal_accessible_get_default_attributes(AtkText *text)
{
	return NULL;
}

static void
deepinvte_terminal_accessible_get_character_extents(AtkText *text, gint offset,
					      gint *x, gint *y,
					      ...
					      AtkCoordType coords)
{
	...
	char_width = deepinvte_terminal_get_char_width (terminal);
	char_height = deepinvte_terminal_get_char_height (terminal);
	*x *= char_width;
	*y *= char_height;
	*width = char_width;
	*height = char_height;
...
static gint
deepinvte_terminal_accessible_get_character_count(AtkText *text)
{
	DeepinvteTerminalAccessiblePrivate *priv;
...
static gint
deepinvte_terminal_accessible_get_offset_at_point(AtkText *text,
					    gint x, gint y,
					    AtkCoordType coords)
{
	DeepinvteTerminalAccessiblePrivate *priv;
...
static gint
deepinvte_terminal_accessible_get_n_selections(AtkText *text)
{
	GtkWidget *widget;
	DeepinvteTerminal *terminal;
	...
	g_assert(DEEPINVTE_IS_TERMINAL (widget));
	terminal = DEEPINVTE_TERMINAL (widget);
	return (deepinvte_terminal_get_has_selection (terminal)) ? 1 : 0;
}

static gchar *
deepinvte_terminal_accessible_get_selection(AtkText *text, gint selection_number,
				      gint *start_offset, gint *end_offset)
{
	GtkWidget *widget;
	DeepinvteTerminal *terminal;
	...
	g_assert(DEEPINVTE_IS_TERMINAL (widget));
	terminal = DEEPINVTE_TERMINAL (widget);
	if (!deepinvte_terminal_get_has_selection (terminal)) {
		return NULL;
	}
	...
	priv = g_object_get_data(G_OBJECT(text),
				 DEEPINVTE_TERMINAL_ACCESSIBLE_PRIVATE_DATA);
	_deepinvte_terminal_get_start_selection (terminal, &start_x, &start_y);
	*start_offset = offset_from_xy (priv, start_x, start_y);
	_deepinvte_terminal_get_end_selection (terminal, &end_x, &end_y);
	*end_offset = offset_from_xy (priv, end_x, end_y);
	return _deepinvte_terminal_get_selection (terminal);
}

static gboolean
deepinvte_terminal_accessible_add_selection(AtkText *text,
				      gint start_offset, gint end_offset)
{
	GtkWidget *widget;
	DeepinvteTerminal *terminal;
	...
	g_assert(DEEPINVTE_IS_TERMINAL (widget));
	terminal = DEEPINVTE_TERMINAL (widget);
	g_assert(!deepinvte_terminal_get_has_selection (terminal));
	priv = g_object_get_data(G_OBJECT(text),
				 DEEPINVTE_TERMINAL_ACCESSIBLE_PRIVATE_DATA);
	xy_from_offset (priv, start_offset, &start_x, &start_y);
	xy_from_offset (priv, end_offset, &end_x, &end_y);
	_deepinvte_terminal_select_text (terminal, start_x, start_y, end_x, end_y, start_offset, end_offset);
	return TRUE;
}

static gboolean
deepinvte_terminal_accessible_remove_selection(AtkText *text,
					 gint selection_number)
{
	...
	g_assert(DEEPINVTE_IS_TERMINAL (widget));
	terminal = DEEPINVTE_TERMINAL (widget);
	if (selection_number == 0 && deepinvte_terminal_get_has_selection (terminal)) {
		_deepinvte_terminal_remove_selection (terminal);
		return TRUE;
	} else {
		return FALSE;
	}
...
static gboolean
deepinvte_terminal_accessible_set_selection(AtkText *text, gint selection_number,
				      gint start_offset, gint end_offset)
{
	GtkWidget *widget;
	DeepinvteTerminal *terminal;
	...
	return deepinvte_terminal_accessible_add_selection (text, start_offset, end_offset);
}

static gboolean
deepinvte_terminal_accessible_set_caret_offset(AtkText *text, gint offset)
{
	g_assert(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(text));
	deepinvte_terminal_accessible_update_private_data_if_needed(ATK_OBJECT(text),
							      NULL, NULL);
...
static void
deepinvte_terminal_accessible_text_init(gpointer iface, gpointer data)
{
	AtkTextIface *text;
	...
	text->get_text = deepinvte_terminal_accessible_get_text;
	text->get_text_after_offset = deepinvte_terminal_accessible_get_text_after_offset;
	text->get_text_at_offset = deepinvte_terminal_accessible_get_text_at_offset;
	text->get_character_at_offset = deepinvte_terminal_accessible_get_character_at_offset;
	text->get_text_before_offset = deepinvte_terminal_accessible_get_text_before_offset;
	text->get_caret_offset = deepinvte_terminal_accessible_get_caret_offset;
	text->get_run_attributes = deepinvte_terminal_accessible_get_run_attributes;
	text->get_default_attributes = deepinvte_terminal_accessible_get_default_attributes;
	text->get_character_extents = deepinvte_terminal_accessible_get_character_extents;
	text->get_character_count = deepinvte_terminal_accessible_get_character_count;
	text->get_offset_at_point = deepinvte_terminal_accessible_get_offset_at_point;
	text->get_n_selections = deepinvte_terminal_accessible_get_n_selections;
	text->get_selection = deepinvte_terminal_accessible_get_selection;
	text->add_selection = deepinvte_terminal_accessible_add_selection;
	text->remove_selection = deepinvte_terminal_accessible_remove_selection;
	text->set_selection = deepinvte_terminal_accessible_set_selection;
	text->set_caret_offset = deepinvte_terminal_accessible_set_caret_offset;
}

static AtkLayer
deepinvte_terminal_accessible_get_layer(AtkComponent *component)
{
	return ATK_LAYER_WIDGET;
}

static gint
deepinvte_terminal_accessible_get_mdi_zorder(AtkComponent *component)
{
	return G_MININT;
}

static gboolean
deepinvte_terminal_accessible_contains(AtkComponent *component,
				 gint x, gint y,
...
static void
deepinvte_terminal_accessible_get_extents(AtkComponent *component,
				    gint *x, gint *y,
				    gint *width, gint *height,
				    AtkCoordType coord_type)
{
...
static void
deepinvte_terminal_accessible_get_position(AtkComponent *component,
				     gint *x, gint *y,
				     AtkCoordType coord_type)
{
	GtkWidget *widget;
...
static void
deepinvte_terminal_accessible_get_size(AtkComponent *component,
				 gint *width, gint *height)
{
	GtkWidget *widget;
	GdkWindow *window;
...
static gboolean
deepinvte_terminal_accessible_set_extents(AtkComponent *component,
				    gint x, gint y,
				    gint width, gint height,
				    AtkCoordType coord_type)
{
...
static gboolean
deepinvte_terminal_accessible_set_position(AtkComponent *component,
				     gint x, gint y,
				     AtkCoordType coord_type)
{
...
static gboolean
deepinvte_terminal_accessible_set_size(AtkComponent *component,
				 gint width, gint height)
{
	DeepinvteTerminal *terminal;
	...
	deepinvte_terminal_set_size(terminal, columns, rows);
	return (deepinvte_terminal_get_row_count (terminal) == rows) &&
	       (deepinvte_terminal_get_column_count (terminal) == columns);
}


static AtkObject *
deepinvte_terminal_accessible_ref_accessible_at_point(AtkComponent *component,
						gint x, gint y,
						AtkCoordType coord_type)
{
...
static guint
deepinvte_terminal_accessible_add_focus_handler(AtkComponent *component,
					  AtkFocusHandler handler)
{
	guint signal_id;
	signal_id = g_signal_lookup("focus-event",
				    DEEPINVTE_TYPE_TERMINAL_ACCESSIBLE);
...
static void
deepinvte_terminal_accessible_remove_focus_handler(AtkComponent *component,
					     guint handler_id)
{
...
static void
deepinvte_terminal_accessible_component_init(gpointer iface, gpointer data)
{
	...
	component->add_focus_handler = deepinvte_terminal_accessible_add_focus_handler;
	component->contains = deepinvte_terminal_accessible_contains;
	component->ref_accessible_at_point = deepinvte_terminal_accessible_ref_accessible_at_point;
	component->get_extents = deepinvte_terminal_accessible_get_extents;
	component->get_position = deepinvte_terminal_accessible_get_position;
	component->get_size = deepinvte_terminal_accessible_get_size;
	component->remove_focus_handler = deepinvte_terminal_accessible_remove_focus_handler;
	component->set_extents = deepinvte_terminal_accessible_set_extents;
	component->set_position = deepinvte_terminal_accessible_set_position;
	component->set_size = deepinvte_terminal_accessible_set_size;
	component->get_layer = deepinvte_terminal_accessible_get_layer;
	component->get_mdi_zorder = deepinvte_terminal_accessible_get_mdi_zorder;
}

/* AtkAction interface */

static gboolean
deepinvte_terminal_accessible_do_action (AtkAction *accessible, int i)
{
	GtkWidget *widget;
	...
	if (!widget) {
		...
		g_signal_emit_by_name (widget, "popup_menu", &retval);
                break;
        default :
                g_warning ("Invalid action passed to DeepinvteTerminalAccessible::do_action");
                return FALSE;
        }
        return retval;
}
...
static int
deepinvte_terminal_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}

static const char *
deepinvte_terminal_accessible_action_get_description (AtkAction *accessible, int i)
{
        DeepinvteTerminalAccessiblePrivate *priv;

        g_return_val_if_fail (i < LAST_ACTION, NULL);

	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(accessible), NULL);
	...
	g_return_val_if_fail(priv != NULL, NULL);
        ...
        if (priv->action_descriptions[i]) {
                ...
                return deepinvte_terminal_accessible_action_descriptions[i];
        }
}

static const char *
deepinvte_terminal_accessible_action_get_name (AtkAction *accessible, int i)
{
        g_return_val_if_fail (i < LAST_ACTION, NULL);

        return deepinvte_terminal_accessible_action_names[i];
}

static const char *
deepinvte_terminal_accessible_action_get_keybinding (AtkAction *accessible, int i)
{
        g_return_val_if_fail (i < LAST_ACTION, NULL);
...
static gboolean
deepinvte_terminal_accessible_action_set_description (AtkAction *accessible,
                                                int i,
                                                const char *description)
{
	...
	g_return_val_if_fail(DEEPINVTE_IS_TERMINAL_ACCESSIBLE(accessible), FALSE);
...
static void
deepinvte_terminal_accessible_action_init(gpointer iface, gpointer data)
{
	...
	_deepinvte_debug_print(DEEPINVTE_DEBUG_ALLY,
			...
			"AtkAction interface.\n");
	/* Set our virtual functions. */
	action->do_action = deepinvte_terminal_accessible_do_action;
	action->get_n_actions = deepinvte_terminal_accessible_get_n_actions;
	action->get_description = deepinvte_terminal_accessible_action_get_description;
	action->get_name = deepinvte_terminal_accessible_action_get_name;
	action->get_keybinding = deepinvte_terminal_accessible_action_get_keybinding;
	action->set_description = deepinvte_terminal_accessible_action_set_description;
}
static void
deepinvte_terminal_accessible_class_init(gpointer *klass)
{
	GObjectClass *gobject_class;
	AtkObjectClass *class = ATK_OBJECT_CLASS (klass);

	deepinvte_terminal_accessible_parent_class = g_type_class_peek_parent (klass);

	gobject_class = G_OBJECT_CLASS(klass);

	class->initialize = deepinvte_terminal_initialize;
	/* Override the finalize method. */
	gobject_class->finalize = deepinvte_terminal_accessible_finalize;
}

GType
deepinvte_terminal_accessible_get_type(void)
{
	...
	if (G_UNLIKELY (terminal_accessible_type == 0)) {
		...
		GTypeInfo terminal_accessible_info = {
			...
			(GClassInitFunc)deepinvte_terminal_accessible_class_init,
		...
		parent_type = g_type_parent(DEEPINVTE_TYPE_TERMINAL);
		...
		atk_registry_set_factory_type(registry,
				...
				DEEPINVTE_TYPE_TERMINAL_ACCESSIBLE_FACTORY);
	}
...
static AtkObject *
deepinvte_terminal_accessible_factory_create_accessible(GObject *obj)
{
	GtkAccessible *accessible;
	DeepinvteTerminal *terminal;

	g_assert(DEEPINVTE_IS_TERMINAL(obj));

	terminal = DEEPINVTE_TERMINAL(obj);
	accessible = GTK_ACCESSIBLE(deepinvte_terminal_accessible_new(terminal));
...
static void
deepinvte_terminal_accessible_factory_class_init(DeepinvteTerminalAccessibleFactoryClass *klass)
{
	AtkObjectFactoryClass *class = ATK_OBJECT_FACTORY_CLASS(klass);
	/* Override the one method we care about. */
	class->create_accessible = deepinvte_terminal_accessible_factory_create_accessible;
}
static void
deepinvte_terminal_accessible_factory_init(DeepinvteTerminalAccessibleFactory *self)
{
	/* nothing to initialise */
}

AtkObjectFactory *
deepinvte_terminal_accessible_factory_new(void)
{
	_deepinvte_debug_print(DEEPINVTE_DEBUG_ALLY,
			"Creating a new DeepinvteTerminalAccessibleFactory.\n");
	return g_object_new(DEEPINVTE_TYPE_TERMINAL_ACCESSIBLE_FACTORY, NULL);
}
