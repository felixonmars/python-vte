...
struct _DeepinvteConv {
	...
	DeepinvteBuffer *in_scratch, *out_scratch;
};

/* We can't use g_utf8_strlen as that's not nul-safe :( */
static glong
_deepinvte_conv_utf8_strlen(const gchar *p, gssize max)
{
	const gchar *q = p + max;
	glong length = -1;
	do {
...
static size_t
_deepinvte_conv_utf8_utf8(GIConv converter,
		    const gchar **inbuf,
		    gsize *inbytes_left,
		    gchar **outbuf,
		    gsize *outbytes_left)
 ...
 * encoding. */
DeepinvteConv
_deepinvte_conv_open(const char *target, const char *source)
{
	DeepinvteConv ret;
	GIConv conv;
	...
	if (strcmp(target, DEEPINVTE_CONV_GUNICHAR_TYPE) == 0) {
		real_target = "UTF-8";
		out_unichar = TRUE;
	}

	/* Determine if we need to convert UTF-8 to gunichars on output. */
	if (strcmp(source, DEEPINVTE_CONV_GUNICHAR_TYPE) == 0) {
		real_source = "UTF-8";
		in_unichar = TRUE;
	}
	...
	if (!utf8) {
		...
		if (conv == ((GIConv) -1)) {
			return DEEPINVTE_INVALID_CONV;
		}
	}

	/* Set up the descriptor. */
	ret = g_slice_new0(struct _DeepinvteConv);
	if (utf8) {
		ret->conv = NULL;
		ret->convert = (convert_func) _deepinvte_conv_utf8_utf8;
		ret->close = NULL;
	} else {
	...
	ret->in_scratch = _deepinvte_buffer_new();
	ret->out_scratch = _deepinvte_buffer_new();

	return ret;
}

gint
_deepinvte_conv_close(DeepinvteConv converter)
{
	g_assert(converter != NULL);
	g_assert(converter != DEEPINVTE_INVALID_CONV);
	...
	_deepinvte_buffer_free(converter->in_scratch);
	_deepinvte_buffer_free(converter->out_scratch);

	/* Free the structure itself. */
	g_slice_free(struct _DeepinvteConv, converter);

	return 0;
}

size_t
_deepinvte_conv(DeepinvteConv converter,
	  ...
	  guchar **outbuf, gsize *outbytes_left)
{
	...
	if (converter->in_unichar) {
		...
		_deepinvte_buffer_set_minimum_size(converter->in_scratch,
					     (char_count + 1) * DEEPINVTE_UTF8_BPC);
		/* Convert the incoming text. */
		g = (gunichar*) *inbuf;
		p = converter->in_scratch->data;
		end = p + (char_count + 1) * DEEPINVTE_UTF8_BPC;
	...
	if (converter->out_unichar) {
		work_outbytes = *outbytes_left * DEEPINVTE_UTF8_BPC;
		_deepinvte_buffer_set_minimum_size(converter->out_scratch,
					     work_outbytes);
	...
	if (converter->in_unichar) {
		...
		int chars;
		chars = _deepinvte_conv_utf8_strlen((const gchar *)work_inbuf_start,
					      work_inbuf_working - work_inbuf_start);
...
size_t
_deepinvte_conv_cu(DeepinvteConv converter,
	     const guchar **inbuf, gsize *inbytes_left,
	     gunichar **outbuf, gsize *outbytes_left)
{
	return _deepinvte_conv(converter,
			 inbuf, inbytes_left,
			 (guchar**)outbuf, outbytes_left);
}

size_t
_deepinvte_conv_uu(DeepinvteConv converter,
...
size_t
_deepinvte_conv_uc(DeepinvteConv converter,
	     const gunichar **inbuf, gsize *inbytes_left,
	     guchar **outbuf, gsize *outbytes_left)
{
	return _deepinvte_conv(converter,
...
main(int argc, char **argv)
{
	...
	i = _deepinvte_conv_utf8_strlen("\0\0\0\0", 4);
	g_assert(i == 4);
	i = _deepinvte_conv_utf8_strlen("\0A\0\0", 4);
	g_assert(i == 4);
	i = _deepinvte_conv_utf8_strlen("\0A\0B", 4);
	g_assert(i == 4);
	i = _deepinvte_conv_utf8_strlen("A\0B\0", 4);
	g_assert(i == 4);
	i = _deepinvte_conv_utf8_strlen("ABCDE", 4);
	g_assert(i == 4);
	...
	outbuf = buf;
	outbytes = sizeof(buf);
	conv = _deepinvte_conv_open("UTF-8", "ISO-8859-1");
	i = _deepinvte_conv(conv, &inbuf, &inbytes, &outbuf, &outbytes);
	g_assert(inbytes == 0);
	...
	outbuf = buf;
	outbytes = sizeof(buf);
	conv = _deepinvte_conv_open("ISO-8859-1", "UTF-8");
	i = _deepinvte_conv(conv, &inbuf, &inbytes, &outbuf, &outbytes);
	g_assert(inbytes == 0);
	if (strcmp(narrow_test, buf) != 0) {
	...
	conv = _deepinvte_conv_open(DEEPINVTE_CONV_GUNICHAR_TYPE, "ISO-8859-1");
	i = _deepinvte_conv(conv, &inbuf, &inbytes, &outbuf, &outbytes);
	g_assert(inbytes == 0);
	...
	outbuf = buf;
	outbytes = sizeof(buf);
	conv = _deepinvte_conv_open("ISO-8859-1", DEEPINVTE_CONV_GUNICHAR_TYPE);
	i = _deepinvte_conv(conv, &inbuf, &inbytes, &outbuf, &outbytes);
	g_assert(inbytes == 0);
	...
	outbuf = buf;
	outbytes = sizeof(buf);
	conv = _deepinvte_conv_open("UTF-8", "UTF-8");
	i = _deepinvte_conv(conv, &inbuf, &inbytes, &outbuf, &outbytes);
	g_assert(inbytes == 0);
	if (strcmp(narrow_test, buf) != 0) {
	...
	for (i = 0; i < sizeof(mbyte_test); i++) {
		...
		outbytes = sizeof(buf);
		conv = _deepinvte_conv_open("UTF-8", "UTF-8");
		ret = _deepinvte_conv(conv, &inbuf, &inbytes, &outbuf, &outbytes);
		switch (i) {
		case 0:
